...
achieves a safety factor of approximately $11.9\times$ (Seg-DA, E16)
at $N=15$ LUT points, providing the strongest formal guarantee for compiled
KAN inference to date; plain DA alone certifies this checkpoint with a
...
E56& Deep KAN ($L{=}3$)       & [28,16,8,4] 120ep & \textbf{99.60\%}, 608/608 Z3, DA 15.3$\times$ linear \\
...
\begin{itemize}
    ...
    \item \textbf{DA error bound:} $\Delta_{\text{DA}}^{(L=3)} = 0.98$, a
          $1.5\times$ increase from the recomputed
          $\Delta_{\text{DA}}^{(L=2)} = 0.66$ (2026-08-03) —
         consistent with near-linear growth across the additional layer, not the
