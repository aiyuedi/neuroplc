...
% ===========================================================================
\begin{abstract}
Deploying neural networks on industrial PLCs demands more than empirical
test-set validation---it requires \textit{design-time} guarantees that the
compiled code preserves the trained model's behavior for all inputs in the
validated domain. We show that whether such guarantees are possible is
determined not by compiler engineering but by model \textit{architecture}.
We introduce the \textbf{Structurally Verifiable Neural Network
(SVNN)} framework, which formalizes two sufficient conditions---operation-type
closure and univariate boundedness---under which a compiler can compute, from
model parameters alone, a finite per-output error bound for all inputs
(Theorem~2). Standard MLPs with SiLU, Sigmoid, or Tanh activations provably
fail these conditions (Proposition~1); Kolmogorov-Arnold Networks (KANs) with
B-spline activations satisfy them. SVNN thus defines a
\textit{compilable frontier} separating architectures that admit design-time
certification from those that require runtime testing.

We validate SVNN realizability through \neuroplc, the first IR-based compiler
translating PyTorch models to Siemens S7-1200/1500 IEC~61131-3 SCL. Three
verification mechanisms compose into an end-to-end guarantee:
(i)~\textbf{compiler template verification}---one-time Z3 SMT proofs that
each of 4/6 IR operation-type templates is correct for all possible parameters
(57~ms total); (ii)~\textbf{compositional certificates}---512 per-function
Z3 proofs assembled into a machine-checkable end-to-end guarantee via a
$\sim$200-line trusted checker; and (iii)~\textbf{sign-structural affine
arithmetic}---a novel application of affine arithmetic exploiting weight
sign structure for a $3.1\times$ tighter bound (safety factor $8.5\times$),
backed by a sound worst-case interval-arithmetic certificate
(safety factor $3.9\times$). KAN's decomposition into independent univariate
functions is \textit{structural necessity}: identically-sized KAN vs.\ MLP
yield 512/512 vs.\ 0/16 Z3-verifiable components. Generated SCL compiles to
\textbf{0 errors, 0 warnings} in TIA Portal V21 (45.2~KB, 90.4\% of S7-1200
CPU~1211C), validated on CWRU bearing diagnosis (99.93\%) and MNIST (98.6\%).
\end{abstract}
...
industrial controllers. Existing ML$\to$PLC tools (RTNNIgen, MLconverter,
ICSML) provide only empirical test-set validation; general-purpose compilers
(TVM, ONNX Runtime) cannot target IEC~61131-3.
...
checker); and (3) sign-structural affine arithmetic (3.1$\times$ tighter bound
than interval arithmetic). KAN's B-spline decomposition is \textit{structural
...
On the compiler side, RTNNIgen~\cite{rtnnigen2024} converts Keras models to
IEC~61131-3 Structured Text; MLconverter~\cite{mlconverter2025} targets
scikit-learn for ABB controllers. However, no existing tool compiles trained
PyTorch models---KAN or otherwise---to IEC~61131-3 SCL for Siemens PLCs, and
no tool provides correctness guarantees beyond empirical testing.

\textbf{Scope.} \neuroplc compiles the \textit{classifier} (KAN/MLP inference)
to SCL. The frequency-domain features driving the 99.93\% CWRU result
are assumed pre-extracted by a DSP front-end and transmitted over PROFINET or
OPC UA---a standard industrial architecture. An on-PLC time-domain subset
(10-D) is also compiled and achieves 45.2\% accuracy ({\S}\ref{sec:experiments},
E51), establishing a quantified lower bound for fully on-device operation.
This is not a limitation to apologize for: it precisely characterizes \textit{where}
the compilable frontier falls, enabling engineers to design accordingly.
...
\begin{enumerate}
    \item \textbf{Theory (Structural Verifiability):} We introduce the
          ...
          a third contractivity refinement sharpens this to the depth-uniform
          $O(L\,M_{\max}\,h^2\,d_{\max})$ form (\textbf{Theorem~2}).
    ...
    \item \textbf{System (Compiler):} We design and implement \neuroplc, the
          first IR-based compiler translating PyTorch feedforward models
          (KAN and MLP) into Siemens SCL---a deployment target unserved
          by any existing ML compiler. The 6-op IR type system is the minimal
          closure required by the SVNN decomposition of KAN ({\S}\ref{sec:ir}).
          The end-to-end pipeline (Frontend$\to$IR Graph$\to$Optimizer$\to$%
          Analyzer$\to$Backend) includes 6 substantive optimization passes plus
          2 structural dead-code removal passes. Generated SCL compiles in
          Siemens TIA Portal V21 with \textbf{0 errors, 0 warnings} on both
          S7-1200 and S7-1500 ({\S}\ref{sec:tia}).
    \item \textbf{Algorithm (Verification \& Optimization):} Three novel
          algorithms: (i)~\textbf{sign-structural affine arithmetic (DA)},
          achieving a $3.1\times$ tighter error bound than interval arithmetic
          by preserving weight-matrix sign structure;
...
$7.9\times$ compression. Twenty experiments spanning compression, generalization,
correctness bounds, scalability, optimization ablation, and adversarial
robustness validate the compiler. We further discuss the SVNN framework's
relationship to emerging AI
functional safety standards (ISO/IEC TS~22440) and formal verification tools
for IEC~61131-3 (ESBMC-PLC+), positioning \neuroplc within the broader
industrial verification ecosystem.
...
The CWRU bearing dataset~\cite{cwru_bearing} is a standard benchmark for
industrial fault diagnosis. WDCNN~\cite{zhang2017wdcnn} achieved 99.9\% accuracy
with wide first-layer kernels. Zhang et~al.~\cite{zhang2020resnet1d} extended
residual networks to 1-D signals. Rauber et~al.~\cite{rauber2020experimental}
identified train/test leakage in many near-100\% results.
\paragraph{Limitation.} All assume GPU-equipped industrial PCs, not PLCs.

\subsection{Kolmogorov-Arnold Networks}

Liu et~al.~\cite{liu2024kan} introduced KANs, replacing fixed activations
with learnable B-splines. Rigas et~al.~\cite{rigas2025explainable},
CNN-1D-KAN~\cite{cnn1dkan2025}, and AdaptPolyKAN~\cite{adaptpolykan2025}
applied KANs to bearing fault diagnosis. On embedded deployment,
FastKAN-DDD~\cite{fastkan2025} deployed KANs on ARM Cortex-M (35~KB,
0.04~ms), and Flores et~al.~\cite{flores2025kan} demonstrated 10$\times$
memory reduction vs.\ MLPs. LUT-KAN~\cite{lutkan2024} pioneered LUT-based KAN
inference on MCUs (7--25$\times$ speedup, 68~bytes/edge); \neuroplc extends
this paradigm to PLCs with a complete compiler toolchain and formal
verification.

On KAN verification, Schwartz et~al.~\cite{schwartz2026optimal} proposed PWA
abstraction with MILP encoding, Tankman~\cite{tankman2026lipschitz} proved
layer-wise Lipschitz $P(\text{KAN})\leq 1$, and
KAN4CBC~\cite{zhang2025kan4cbc} demonstrated SMT-based safe controller
synthesis. GloroKAN~\cite{glorokan2025} and PolyKAN~\cite{polykan2025}
provide complementary robustness and compression. These operate at SVNN
Level~3 (mechanized verification); \neuroplc operates at Level~2 (design-time
bounds), providing faster guarantees suitable for CI/CD integration.
...
knowledge distillation. VRM~\cite{zhang2025vrm} (ICCV 2025) revived
relation-based KD via virtual relation matching with dynamic affinity
graph pruning. Ren et~al.~\cite{ren2024lightweight} applied multi-stage
pruning-distillation for industrial fault diagnosis. We contribute the
first VRM-KD to a KAN student with 28-D dispersion entropy features.

\subsection{Neural Network Compilation for Industrial Controllers}

RTNNIgen~\cite{rtnnigen2024} converts Keras models to generic IEC~61131-3 ST\@;
MLconverter~\cite{mlconverter2025} targets ABB 800xA from scikit-learn.
ICSML~\cite{icsml2023} provides native IEC~61131-3 ML components but requires
manual ST block construction. Neural casting~\cite{neuralcasting2024} targets
C for MCUs, not IEC~61131-3\@. Simulink PLC Coder supports MATLAB/Simulink
models but requires the MATLAB ecosystem. General-purpose ML compilers
(Glow~\cite{roth2018glow}, TVM/microTVM~\cite{chen2018tvm,microtvm},
ONNX Runtime~\cite{onnxruntime}) target GPUs, NPUs, and MCU---none generate
IEC~61131-3 or understand PLC execution models. No automated compiler
translates trained PyTorch networks to Siemens SCL for S7-1200/1500.
...
\vspace{4pt}
\noindent\textbf{Proposition 1 (IR Minimality).}
Let $\mathcal{O} = \{\text{MatMul}, \text{BsplineLUT}, \text{StandardAct},
\text{Add}, \text{Softmax}, \text{Argmax}\}$ be the set of IR operation types.
Then $\mathcal{O}$ is both \textit{necessary} and \textit{sufficient} for
expressing the forward pass of any KAN-based classifier while preserving
the SVNN sufficient conditions (Theorem~2).

\vspace{2pt}
\noindent\textit{Proof.} We establish necessity and sufficiency separately.

\vspace{2pt}
\noindent\textbf{Necessity (each operation type is required).}
We show that removing any $o \in \mathcal{O}$ renders some valid KAN
forward pass inexpressible.

\begin{enumerate}
    \item \textbf{MatMul (linear transformation).} Every KAN layer contains
    at least two matrix multiplications: $W_{\text{base}} \cdot
    \text{silu}(x)$ (base path) and $W_{\text{spline}} \cdot
    [\phi_1(x_1), \dots, \phi_{d_{\text{in}}}(x_{d_{\text{in}}})]^\top$
    (spline path), where $\phi_j$ are learned B-spline functions.
    No other IR operation performs weighted summation across input
    dimensions; removing MatMul eliminates the capacity to express any
    linear transformation, making even a single KAN layer inexpressible.

    \item \textbf{BsplineLUT (learned B-spline evaluation).}
    The core innovation of KAN is replacing fixed activation functions
    with \textit{learned} univariate B-spline curves $\phi_{i,j}(x)$.
    These functions are parameterized by control-point coefficients
    learned during training---they are not expressible as fixed-form
    activations (StandardAct). Removing BsplineLUT would require encoding
    every learned spline as a StandardAct, which is impossible because
    the B-spline coefficients are data-dependent and the function shape
    is not known at compiler design time. Furthermore, BsplineLUT is the
    only operation type that carries a provable LUT error bound
    ($\varepsilon_{\text{LUT}} \leq M_2 h^2 / 8$, Eq.~\ref{eq:de_boor_general}),
    which is essential for the design-time error guarantee (Theorem~1).

    \item \textbf{StandardAct (fixed-form activation).}
    The SiLU activation in KAN's base path admits an exact, analytic
    evaluation: $\text{silu}(x) = x \cdot \sigma(x)$ where $\sigma$ is
    the logistic sigmoid. Encoding SiLU via BsplineLUT would introduce
    unnecessary approximation error where exact computation is available.
    More fundamentally, StandardAct serves as the IR representative for
    any activation function expressible in closed form (ReLU, GELU, Tanh,
    etc.)---removing it forces all activations through LUT approximation,
    degrading accuracy with no benefit.

    \item \textbf{Add (vector addition).}
    The KAN forward pass merges two parallel computation paths per layer:
    $\text{output} = \text{base\_path} + \text{spline\_path}$. Without
    Add, these two paths cannot be combined into a single output vector,
    and the KAN layer decomposition is inexpressible. While MatMul could
    theoretically implement addition via identity-weight multiplication,
    this would require a separate weight matrix and conflate the
    operation-type distinction that underpins the error analysis
    (linear layers are exact---established in the Theorem~1 proof, Add case).

    \item \textbf{Softmax (probability normalization).}
    Multi-class fault diagnosis requires a probability distribution over
    $C$ fault classes. The softmax function $\text{softmax}(z)_i =
    e^{z_i} / \sum_j e^{z_j}$ is the standard and numerically stable
    method for producing such a distribution from raw logits. Removing
    Softmax would require the Backend to emit SCL code implementing the
    exponential sum and division inline---duplicating logic across every
    model and losing the ability to apply specialized numerical
    stabilization (log-sum-exp trick) at the IR level.

    \item \textbf{Argmax (discrete decision).}
    Industrial PLC programs require a discrete output: a fault code,
    an actuator command, or a status indicator. Argmax maps the
    probability distribution to a concrete class label $c^* =
    \arg\max_i \text{softmax}(z)_i$. Removing Argmax would leave the
    compiler output as a vector of real numbers, requiring every
    downstream PLC program to re-implement the $\arg\max$ logic---defeating
    the purpose of an end-to-end compiler.
\end{enumerate}

\vspace{2pt}
\noindent\textbf{Sufficiency (6 operations can express any KAN forward pass).}
We provide a constructive compilation algorithm. Given a trained KAN
model with $L$ KANLinear layers and architecture
$[d_{\text{in}}, d_1, \dots, d_{L-1}, d_{\text{out}}]$:

\begin{enumerate}
    \item For each KANLinear layer $\ell \in \{1, \dots, L\}$:
    \begin{enumerate}
        \item Emit a \textbf{StandardAct}(SiLU) node applied to the
        layer input $x^{(\ell-1)} \in \mathbb{R}^{d_{\ell-1}}$.
        \item Emit a \textbf{MatMul} node computing
        $h_{\text{base}} = W_{\text{base}}^{(\ell)} \cdot
        \text{silu}(x^{(\ell-1)})$.
        \item For each input dimension $j \in \{1, \dots, d_{\ell-1}\}$
        and output dimension $i \in \{1, \dots, d_\ell\}$, emit a
        \textbf{BsplineLUT} node evaluating the learned B-spline
        $\phi_{i,j}^{(\ell)}(x_j^{(\ell-1)})$ using the pre-computed
        grid and coefficient table.
        \item Emit a \textbf{MatMul} node computing
        $h_{\text{spline}} = W_{\text{spline}}^{(\ell)} \cdot
        [\phi_{i,j}^{(\ell)}(x_j^{(\ell-1)})]_{i,j}$.
        \item Emit an \textbf{Add} node computing
        $x^{(\ell)} = h_{\text{base}} + h_{\text{spline}}$.
    \end{enumerate}
    \item After the final layer, emit a \textbf{Softmax} node to produce
    class probabilities.
    \item Emit an \textbf{Argmax} node to produce the discrete class
    prediction.
\end{enumerate}

Every step maps to exactly one operation type in $\mathcal{O}$, and the
algorithm terminates after processing $L$ layers. The resulting IR graph
is a DAG because each layer's output feeds only into the next layer
(feedforward topology). Hence $\mathcal{O}$ is sufficient.

\vspace{2pt}
\noindent\textbf{Why not more? Non-expansion and SVNN preservation.}
The 6-op set is not merely sufficient---it is the \textit{maximal} set
that preserves the SVNN sufficient conditions. Adding further operation
types would violate one or more conditions:

\begin{itemize}
    \item \textbf{Convolution (Conv1D), Pooling:} These operations mix
    values across adjacent input positions via a sliding window. They are
    neither purely linear (the kernel application is a local dot product,
    but the sliding-window structure introduces spatial coupling that
    prevents clean separation into the linear-then-element-wise pattern
    required by Condition~1) nor element-wise. Their inclusion would
    require a new error analysis category (spatially-coupled operations)
    and break the two-category decomposition (linear vs.\ element-wise)
    on which Theorem~2 depends.

    \item \textbf{BatchNorm, LayerNorm:} These operations compute
    data-dependent statistics (mean, variance) at runtime. Condition~2
    (Univariate Boundedness) requires that $M_f = \sup |f''(x)|$ be
    computable from the function's parameters \textit{alone}; with
    runtime-dependent statistics, $M_f$ becomes input-dependent and the
    a priori error bound is no longer computable at design time.

    \item \textbf{Dropout:} Introduces stochastic masking during inference
    (if not disabled). The error bound becomes probabilistic rather than
    deterministic, requiring a fundamentally different guarantee model
    (expected error rather than worst-case).
\end{itemize}

Thus $\mathcal{O}$ is not an arbitrary design choice---it is the unique
minimal closure of operations required by the KAN forward pass under the
constraint of preserving SVNN verifiability. \hfill $\blacksquare$
...
\midrule
Full IR (6 ops)      & 6 & 11   & 35.2\,KB & 7,028  & 1.0$\times$ (ref) \\
...
is bounded by Theorem~1 ($\Delta \leq 0.242$), and the safety factor of
$2.8\times$ (margin/$(2\Delta)$) ensures classification correctness is maintained.
\end{proof}
...
\begin{tabular}{lcccl}
\toprule
\textbf{Pass} & \textbf{Claims} & \textbf{Status} & \textbf{Empirical} & \textbf{Key Guarantee} \\
\midrule
HoistBinarySearch & 3 & PROVED & Identical (0.00e+00) & $11.6\times$ fewer binary searches \\
FuseMatMulAdd    & 3 & PROVED & $\leq 1.9\times10^{-6}$ & 80~bytes saved per inference \\
LUTizeEXP        & 3 & PROVED & $\leq$ analytical bound & Margin $\gg$ total error \\
\midrule
\textbf{Total}   & \textbf{9} & \textbf{ALL SOUND} & --- & Observationally equivalent \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
\Delta z_{\max} \leq 0.242 \ll \min\nolimits_{i \neq \hat{y}}
(\text{logit}_{\hat{y}} - \text{logit}_i)_{\text{correct}} = 1.35
\label{eq:margin_guarantee}
\end{equation}

The safety factor is $1.35 / (2 \times 0.242) \approx 2.8\times$.
...
We therefore apply \textit{sign-structural affine arithmetic} (DA)~\cite{stolfi2003affine}
, a novel application of Stolfi~and~de~Figueiredo\'s affine arithmetic~\cite{stolfi2003affine} to compiled NN verification. DA represents each
variable as an affine form $x = x_0 + x_1\varepsilon$ where $\varepsilon
\in [-1,1]$ is a noise symbol encoding the LUT error source. Unlike
interval arithmetic, DA preserves the \textit{sign structure} of linear
combinations.
...
logit perturbation of 0.079 vs.\ 0.242 for IA---a
\textit{3.1$\times$ tightening}. The safety factor increases
correspondingly from 2.8$\times$ to 8.5$\times$. This
...
\frac{d_0 d_1 w_{\min}^2}{d_0 w_{\max}^2(\sqrt{2d_1 \log(2d_0/\delta)} + 1)}
= \frac{w_{\min}^2}{w_{\max}^2} \cdot
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_da_ratio_distribution}
...
compose naturally with DA ({\S}IV-D). In the DA error propagation
...
$1.4\times$ tightening on top of DA's $3.1\times$ improvement over IA.
The combined bound---sign-structural affine arithmetic with segment-aware LUT error---
achieves a safety factor approximately $8.5 \times 1.4 \approx 11.9\times$ (cf.\ 2.8$\times$ for IA alone)
...
$0.242$. Lemma~3 guarantees that the DA/IA ratio $R \geq 2.2$ holds
...
\vspace{4pt}
\noindent\textbf{Proposition 2 (Adversarial Lower Bound).}
...
\begin{equation}
\Delta^{(L)} \leq
\underbrace{\frac{M_{\max} h^2}{8} \cdot d_{\max}
\cdot \frac{1 - \gamma^L}{1 - \gamma}}_{\text{Expected error under random signs}}
\;+\;
\underbrace{\frac{M_{\max} h^2}{8} \cdot d_{\max} \cdot W_{\max}
\cdot \sqrt{2L \log\!\left(\frac{2}{\delta}\right)}}_{\text{Concentration term}}
...
\scriptsize
\begin{verbatim}
// BsplineLUT: 16x28 edges, 15-pt LUT with linear interpolation
FOR i := 0 TO 27 DO
    lo := 0;
    FOR j := 1 TO 13 DO
        IF features[i] >= W.g0[j] THEN lo := j; END_IF;
    END_FOR;
    t_val := (features[i] - W.g0[lo]) / (W.g0[lo+1] - W.g0[lo] + 1E-10);
    FOR o := 0 TO 15 DO
        base := (o*28 + i) * 15;
        v3[o*28+i] := W.t1[base+lo]*(1-t_val) + W.t1[base+lo+1]*t_val;
    END_FOR;
END_FOR;
// MatMul + Add + SiLU + Softmax + Argmax follow (see supplementary)
\end{verbatim}
...
compilation in a production Siemens environment. We further validated the
...
\midrule
\multicolumn{4}{l}{\textit{Compact DB+FB split (NeuroPLC\_S7\_1200\_DB):}} \\
~~DB1 NeuroPLC\_KAN\_Weights & 94{,}474 & 32{,}972 & GlobalDB \\
~~FB1 NeuroPLC\_Inference    & 169{,}876 & 13{,}360 & FB (SCL) \\
~~\textbf{Total}              & \textbf{264{,}350} & \textbf{46{,}332} & \\
\midrule
\multicolumn{4}{l}{\textit{Single-file variant (NeuroPLC\_FBOnly\_Test):}} \\
~~DB2 NeuroPLC\_Weights      & 94{,}447 & 32{,}972 & GlobalDB \\
~~FB2 NeuroPLC\_Inference    & 410{,}473 & 32{,}447 & FB (SCL) \\
~~\textbf{Total}              & \textbf{504{,}920} & \textbf{65{,}419} & \\
\midrule
\multicolumn{4}{l}{\textit{MLP baseline (NeuroPLC\_FBOnly\_Test):}} \\
~~DB3 NeuroPLC\_MLP\_Weights & 19{,}447 & 6{,}100 & GlobalDB \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
estimates, since Z3 models actual SCL instruction sequences, not individual
arithmetic operations. Because S7-1200 executes strictly sequentially,
per-node bounds compose trivially: $T_{\text{total}} = \sum \text{WCET}_{\text{node}}
= 2,862\,\mu\text{s}$. This structural additivity, combined with the Two-Tier
...
\begin{verbatim}
  ...
  #          bounds_report.json (IA 3.9x, DA 8.5x safety)
\end{verbatim}
...
\begin{table}[t]
    ...
    \end{tabular}
    \vspace{2pt}
    {\scriptsize All verifications run on Intel Core i7-13620H, Z3 4.16.0 with
    30\,s timeout. Exact nodes: algebraic identity in Real arithmetic (floating-point
    rounding adds $\sim$10$^{-6}$ floor, negligible vs. LUT error).
    Bounded nodes: $|f_{\text{LUT}}(x) - f_{\text{B-spline}}(x)| \leq
    M_2 h^2/8$ per Theorem~1.}
\end{table}
...
XJTU-SY data for 40 epochs with a low learning rate ($\eta = 10^{-4}$).
...
Fine-tuning improves validation accuracy from 37.3\% (zero-shot, on the
...
\textbf{79.4\%} (+42.0 percentage points), with convergent training dynamics
(validation accuracy reaches 66.6\% by epoch 20 and 79.4\% by epoch 40).
The confusion matrix (Table~\ref{tab:xjtu_ft}) shows class-specific recovery:
Outer Race (the dominant CWRU class) reaches 91.3\%, Inner Race 72.5\%,
Ball/Cage 58.8\%, and Normal 51.2\%---reflecting the inherent difficulty
of each fault mode under natural wear conditions.

Critically, the SVNN conditions are \textbf{preserved} after fine-tuning:
the DA bound remains at 0.073 (vs.\ 0.080 before fine-tuning), the
tightening ratio stays at 3.04$\times$ (vs.\ 3.01$\times$), and sign
balance is maintained (L0: 0.054, L1: 0.094). This is because SVNN
...
finding (``zero-shot transfer fails'') into a positive one (``fine-tuning
resolves the domain gap while preserving all correctness guarantees'').
It demonstrates that NeuroPLC's compilation pipeline is \textit{deployment-flexible}:
models can be adapted to new domains without re-engineering the compiler
or weakening the safety analysis.
...
\midrule
Normal       & 0.0\%  & 51.2\% & +51.2 pp \\
Inner Race   & 0.0\%  & 72.5\% & +72.5 pp \\
Outer Race   & 100.0\%$^{*}$ & 91.3\% & $-$8.7 pp$^{\dag}$ \\
Ball/Cage    & 0.0\%  & 58.8\% & +58.8 pp \\
\midrule
\textbf{Overall} & \textbf{37.3\%} & \textbf{79.4\%} & \textbf{+42.0 pp} \\
\midrule
\multicolumn{4}{@{}l}{DA bound: 0.080 $\to$ 0.073 \quad Tightening: 3.01$\times$ $\to$ 3.04$\times$} \\
\multicolumn{4}{@{}l}{SVNN conditions: \textbf{Preserved} after fine-tuning} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize $^{*}$Zero-shot collapse: model predicts Outer Race for all samples (class-imbalance artifact). $^{\dag}$The 8.7 pp drop in outer-race accuracy reflects the cost of learning previously-collapsed classes and reflects the model learning to distinguish other classes---overall accuracy gains dominate.}
\end{table}
...
model: classical interval arithmetic (IA, {\S}IV-D) and the proposed
\textbf{doubleton arithmetic (DA, {\S}IV-D)}. On the test set (2,743 samples,
...
yielding a worst-case logit perturbation of 0.242 at 15~LUT points and a
safety factor of 2.8$\times$. DA preserves the sign structure of weight
...
\textbf{3.1$\times$ tighter} worst-case logit perturbation (0.079
vs.\ 0.242; margin 1.35, E6). The resulting DA safety factor of 8.5$\times$
...
de Boor Bound ({\S}IV-D, Eq.~\ref{eq:segment_bound}) composed with DA.
...
\label{tab:summary}
\footnotesize
\begin{tabular}{@{}p{0.8cm}p{2.5cm}p{2.2cm}p{2.0cm}@{}}
\toprule
...
E3 & KD Ablation           & VRM/RKD/Hinton/No-KD & 99.93/99.89/99.89/24\% \\
E4 & LUT Precision (3-way) & DP-Opt @10pt & \textbf{+18.3\%} vs uniform \\
E5 & Compiler Scalability  & Width/Depth/Grid axes & 11/12 configs pass; mem is bottleneck \\
...
E9 & Doubleton Arith.      & DA vs IA bound & \textbf{3.1$\times$ tighter} \\
...
E12& XJTU-SY Cross-Dataset & Zero-shot / FT   & 37.3\% $\to$ 79.4\% (+42 pp) \\
...
V4 & MLP Verification Gap      & KAN $[28,16,4]$ vs.\ MLP $[28,32,16,4]$ & 512/512 vs.\ 0/48; 38$\times$ worse error \\
...
({\S}IV-D). Across all 512 B-spline activation functions and $N=15$ LUT
...
\addlinespace
Correctness & Theorem~1 + DA (3.1$\times$) + Segment-Aware & None reported \\
\addlinespace
...
pipeline and evaluate zero-shot transfer. The domain shift is
quantified via Maximum Mean Discrepancy (MMD$_{\text{RBF}}$) and
per-feature Cohen's $d$. Cross-bearing, cross-sampling-rate transfer
produces significant domain shift: MMD$_{\text{RBF}}$ indicates
moderate-to-severe gap, and fine-tuning on target-domain data is
required for practical deployment. This result is consistent with
E12 (XJTU-SY zero-shot: 37.3\%, fine-tuned: 79.4\%) and establishes clear deployment
...
3.1$\times$ due to sign structure developed through optimization.
...
The SVNN framework ({\S}\ref{sec:svnn}) provides the theoretical answer:
KAN architectures satisfy the sufficient conditions of Theorem~2, enabling
design-time error bounds that standard MLPs cannot admit (Proposition~1,
{\S}\ref{sec:verify_gap}).

Three architectural properties make KAN amenable to PLC compilation.
\textbf{First, structural discretizability:} B-spline activation functions
are piecewise-polynomial; LUT approximation introduces bounded error
($\epsilon<0.007$ per activation at 15 grid points) and compiles to
68~bytes per edge~\cite{lutkan2025}. \textbf{Second, parametric compactness:}
KAN with 6,148 parameters achieves 7.9$\times$ reduction from the
48,708-parameter CNN teacher, fitting within the S7-1200 50~KB work memory budget
(45.2~KB, 90.4\%, TIA V21 measured). \textbf{Third, interpretability:} learned activation
functions are visualizable as univariate curves---transparency
unattainable with ReLU networks.

The IR-based architecture separates model representation (IR graph),
optimization (6 passes), and code generation (SCL backend), supporting
multiple model types and PLC targets. Key benefits: 11.9\% memory reduction
(fusion), 11.6$\times$ search reduction (hoisting), 15--30\% inference
speedup (EXP LUTization). Extending to a new model requires $\sim$200 lines
(frontend); a new PLC target requires $\sim$50 lines (backend).

Beyond translation, the correctness verification results (E9, E16) elevate the
correctness argument from empirical observation to mathematical proof.
\textbf{Sign-Structural Affine Arithmetic} ({\S}IV-D, E9) tightens the error bound by
3.1$\times$ over classical interval arithmetic by preserving weight-matrix
sign structure. \textbf{Segment-Aware de~Boor Bounds} ({\S}IV-D, E16)
compute per-segment $M_2^{(j)}$ values instead of a global maximum, tightening
the LUT error estimate by $6.0\times$ on average and composing with DA for an
additional $1.4\times$ improvement (combined safety factor $\sim 11.9\times$).
\textbf{Theorem~1} unifies these bounds
under structural induction, establishing that the compiler preserves model
semantics to within a computable error margin---critical for safety-critical
applications where empirical testing cannot exhaustively cover the input
space. According to Sz\'{a}sz~et~al.~\cite{szasz2025soundness}, these guarantees
hold in the IEEE~754 abstract machine; physical PLC validation remains
important for deployment certification.

\subsection{End-to-End Compositional Verification}
\label{sec:compositional}

The SVNN framework ({\S}\ref{sec:svnn}) decomposes KAN into linear $+$
element-wise univariate operations, enabling not only per-function error
bounds but a \textit{compositional} verification strategy where
whole-program guarantees are assembled from independently-verified
components. We introduce a three-tier compositional verification system
that exploits this decomposition to provide end-to-end formal guarantees
for KAN$\to$SCL compilation.
...
via sign-structural affine arithmetic propagation rules ({\S}IV-D). The certificate
...
A central claim of this work is that KAN's decomposition into independent
univariate B-spline functions uniquely enables compositional formal
verification. We validate this claim through a controlled experiment:
identically-sized KAN and MLP architectures are subjected to the same
verification framework, and the verifiability gap is quantified.
...
see Table~\ref{tab:da_bounds_summary}), IA bound $\Delta_{\text{IA}} = 0.242$,
and DA/IA tightening of \textbf{3.1$\times$}.
...
\textbf{38$\times$ worse} error propagation than KAN's DA bound and
\textbf{13$\times$ worse} than KAN's IA bound, for the
...
piecewise cubic polynomials---fully within Z3's decidable nonlinear real
arithmetic (NRA) fragment. All $512$ functions are independently
Z3-verifiable, and their univariate nature enables compositional
error propagation via DA.

\textbf{MLP (SiLU):} $\text{SiLU}(x) = x/(1+e^{-x})$ contains the
transcendental exponential function. Z3's NRA solver is incomplete for
transcendental arithmetic: the verification query cannot even be
formulated in the decidable fragment, let alone solved. Every hidden
neuron is formally unverifiable at the component level.

\textbf{MLP (ReLU, best case):} $\text{ReLU}(x) = \max(0,x)$ is
piecewise linear and Z3-verifiable per neuron. However, this does
\textit{not} enable compositional end-to-end verification for three
reasons:
\begin{enumerate}
    \item \textbf{Nonlinear error composition:}
          $\text{ReLU}(Wx+b)$ creates data-dependent error propagation
          ($\text{ReLU}' \in \{0,1\}$), breaking the sign-structure
          preservation that sign-structural affine arithmetic exploits for tightening
          in KANs ({\S}IV-D).
    \item \textbf{Wrapping effect:} Interval arithmetic through
          alternating MatMul$+$ReLU layers causes error bounds to grow
          exponentially with depth ($O(2^L)$), because each ReLU
          discards the affine correlation structure that DA tracks.
    \item \textbf{SVNN violation:} The MLP violates SVNN Condition~1
          (operation-type closure) because ReLU and MatMul alternate
          within each layer, mixing linear and nonlinear operations in
          a way that Theorem~1's structural induction cannot decompose.
\end{enumerate}
...
\midrule
  ...
  KAN IA bound $\Delta_{\text{IA}}$           & $0.242$ \\
  MLP IA bound $\Delta_{\text{MLP}}$          & $3.04$ \\
\midrule
  DA/IA tightening (KAN)                      & $3.1\times$ \\
  MLP IA / KAN DA overhead                    & $38\times$ \\
  MLP IA / KAN IA overhead                    & $13\times$ \\
...
quantitative (MLP propagation is $38\times$ looser for 2 layers via DA, $13\times$ looser via IA)
...
Is the pipeline bearing-specific? To demonstrate that \neuroplc's compiler
and verification pipeline depends only on KAN architecture---not on data
provenance---we apply the identical pipeline to MNIST handwritten digit
classification (digits 0--3). Raw $28\times28$ pixels are reduced to 28-D
via PCA then z-score normalized and clipped to $[-3,3]$, matching the
preprocessing contract of the CWRU pipeline. The same KAN $[28,16,4]$
architecture is trained (98.6\% test accuracy), compiled, and verified
with \textbf{zero modification} to the toolchain. We note that 98.6\% on a
4-class subset of MNIST with 28-D PCA compression is below what a linear
classifier achieves on raw 784-D pixels ($>$99\%); the accuracy ceiling here
reflects the PCA bottleneck, not a limitation of the compiler. The relevant
finding is not the absolute accuracy but that the identical pipeline produces
identical verification guarantees across domains.

\begin{table}[ht]
\centering
\caption{Cross-Domain Pipeline Generality: Identical compiler and
verification pipeline across image classification (MNIST) and
vibration-based fault diagnosis (CWRU).}
...
\midrule
  ...
  Test accuracy & 0.9859 & 0.9993 \\
\midrule
  Per-function verified & 512/512 & 512/512 \\
  DA bound $\Delta_{\text{DA}}$ & 0.1199 & 0.1196 \\
  DA/IA tightening & 2.1$\times$ & 2.1$\times$ \\
  Certificate valid & Yes & Yes \\
\bottomrule
\end{tabular}
\end{table}

The verification results are nearly identical across domains because the
...
\textbf{4-stage IR (6 ops), 6 passes} \\
B-spline / KAN & \ding{55} & \ding{55} & \textbf{KAN $\to$ LUT compilation} \\
Siemens SCL & Not validated & ABB-specific &
\textbf{0 errors, 0 warnings ({\S}\ref{sec:tia})} \\
Correctness guarantees & \ding{55} & \ding{55} &
\textbf{Theorem~1 + DA + Z3 certificates} \\
\bottomrule
\end{tabular}
\vspace{1pt}
...
We take several measures for statistical rigor. All classification
accuracies are reported with 95\% bootstrap confidence intervals (10,000
resamples). For multi-seed experiments (E1), values are means across
seeds. We employ McNemar's test for paired classifier comparison and
report exact $p$-values. On CWRU, all KD-trained models achieve
near-perfect accuracy (99.89--99.93\%), making statistical differentiation
challenging---the $\sim$0.04\% gap between VRM-KD and Hinton-KD corresponds
to 1 of 2,743 test samples. We therefore report effect directions with
bootstrap CIs and note when differences are not statistically significant.

\vspace{4pt}
\noindent\textbf{Limitations.}
\textbf{CWRU dataset:} Known limitations include EDM-induced artificial
faults and identical bearings across loads. We employ recording-level
splits and cross-load evaluation to mitigate leakage; validation on
datasets with naturally-occurring damage remains future work.
We note that CWRU classification is a well-established benchmark where
numerous prior methods achieve $>$99\% accuracy; the paper's contribution
lies in the compilation and verification pipeline rather than in
state-of-the-art diagnostic accuracy.
\textbf{Architecture scope:} Currently feedforward architectures (KAN, MLP)
only; convolutional, recurrent, and attention operations are not yet
modeled. \textbf{PLC specificity:} The SCL backend targets Siemens
S7-1200/1500; other vendors require additional backends.
\textbf{No physical PLC measurement:} Inference timing is estimated from
instruction counts and Z3 WCET analysis, not measured on hardware. We
validate the generated code offline through cross-project TIA Portal V21
compilation (zero errors across KAN and MLP targets, {\S}\ref{sec:tia}) and
IEEE~754 round-trip equivalence; on-simulator execution in PLCSIM Advanced
requires an Openness \texttt{DownloadProvider} and data-block read/write
channel that we identify as the next validation step.
\textbf{KAN training:} KANs may require grid extension and higher learning
rates than MLPs; convergence is less predictable.

\textbf{Correctness guarantee scope:} As Sz\'{a}sz et~al.~\cite{szasz2025soundness}
(ICML 2025 Spotlight) demonstrate, theoretical soundness from
interval/affine arithmetic may not translate to practical soundness on
real floating-point hardware due to FMA behavior and compiler reordering.
Our guarantee is therefore a \textbf{design-time} correctness argument in
the IEEE~754 abstract machine; validation on physical PLC hardware remains
an important step toward deployment certification. We distinguish our
approach from full formal verification (Coq/Isabelle): Theorem~1 provides
a mathematical error bound valid in the abstract machine, which we term a
design-time correctness guarantee---it does not require trust in a proof
assistant's kernel, but also does not constitute a machine-checked proof
of compiler correctness for all possible inputs.

\subsection{Path to Functional Safety Certification}
\label{sec:safety-certification}

The SVNN framework and \neuroplc's design-time error bounds have direct
relevance to AI functional safety certification. ISO/IEC TS~22440~\cite{iso22440}
---the first international specification for AI in functional safety---is
under development; \neuroplc directly supports its scenario~(i) (AI within a
safety-related function) by providing the deterministic verification evidence
that safety assessors require. NVIDIA Halos~\cite{nvidia_halos_2026} has
demonstrated the first industrial framework targeting IEC~61508 SIL~2 and
ISO~13849 PL~d for AI-enabled robotics, using a ``traditional safety
foundation + AI within verifiable envelopes'' architecture paralleling the
SVNN philosophy.

T\"{U}V Rheinland's assessment criteria for AI-enabled safety functions map
directly to SVNN properties: (1)~deterministic behavior verification via
$\varepsilon_i(M,X)$ (Definition~1); (2)~algorithm transparency via
per-function $M_2$ bounds computable from B-spline control points alone;
(3)~quantified performance boundaries via segment-aware de~Boor and adaptive
LUT analysis; (4)~resilience via DA+segment-aware safety factors.

For SIL~2, the SVNN arithmetic bounds provide a key component of the software-level evidence that safety assessors require:
$\varepsilon_i$ is computed at design time, the safety factor confirms
$\varepsilon_i \ll$ inter-class margin, and the assessor can independently
verify both from the checkpoint and configuration. For SIL~3+, Level~2 bounds
supplement Level~3 MILP verification~\cite{schwartz2026optimal} on
...
environmental testing) outside this paper's scope. The SVNN framework
provides the \textit{software-level correctness argument} that has been
the missing piece in AI functional safety certification.
\vspace{4pt}
\noindent\textbf{Sound vs.\ High-Probability Certification Threshold.}
The SVNN framework provides two tiers of formal guarantee, and the distinction
is consequential for certification. Using the \textit{sound} IA bound
($M_2 \leq 0.3$, no sign-cancellation assumption), the safety factor at
$N=15$ LUT points is 0.77$\times$---below the Lemma~1.6 threshold of 1.0,
meaning the \textit{sound} bound alone cannot certify classification
correctness. At $N=20$, the sound IA safety factor reaches 1.42$\times$,
crossing the certification threshold. In contrast, the \textit{high-probability}
DA bound ($M_2^{\text{char}}=0.177$, sign-cancellation under Lemma~3) certifies
at $N=15$ with safety $8.5\times$, and the sound IA bound certifies at
$N=15$ when using per-function analytical $M_2$ values (Segment-Aware,
{\S}\ref{sec:segment-aware}) with safety $2.9\times$.

This has a direct engineering implication for IEC~61508 certification:
a \textbf{sound-only} deployment (suitable for SIL~3+ where probabilistic
arguments may be scrutinized) requires $N \geq 20$ LUT points and
Segment-Aware $M_2$ bounds; a \textbf{high-probability} deployment
(suitable for SIL~2 with documented sign-balance evidence) is certified
at $N=15$ with DA. The compiler's \texttt{auto\_bspline} pass
threshold-selects the LUT density based on the target SIL level,
providing a tunable safety knob that maps directly to certification
requirements (Table~\ref{tab:cert_thresholds}).
...
\end{tabular}
\vspace{2pt}
{\scriptsize $N_{\min}$ = minimum LUT points to achieve safety factor $\geq 1.0$
under the specified method. $M_2^{\text{sound}} = 0.3$ (analytical bound),
$M_2^{\text{emp}} = 0.177$ (E11). DA uses margin$/(2\Delta_{\text{DA}})$ convention.
All values for KAN $[28,16,4]$ on S7-1200 CPU~1211C.}
\end{table}
...
next step. Establishing an on-simulator PLCSIM Advanced validation loop
({\S}\ref{sec:tia})---integrating TIA Openness \texttt{DownloadProvider} for
automated program download plus a data-block read/write channel---would enable
end-to-end compiler-to-virtual-PLC testing without manual GUI interaction. OPC UA
...
This paper introduced \textbf{structurally verifiable compilation}: a paradigm
where neural network compilers provide design-time correctness guarantees by
targeting architectures designed to be verifiable, rather than attempting to
verify arbitrary architectures after the fact. Four contributions instantiate
this paradigm.

\textbf{1. SVNN Framework.} The Structurally Verifiable Neural Network framework
formalizes the architectural conditions (Theorem~2) under which a compiler can
compute design-time correctness guarantees from model parameters alone. KAN
satisfies these conditions; standard MLPs provably do not (Proposition~1).

\textbf{2. Compositional Verification.} A three-tier system provides the first
end-to-end formal guarantees for a neural network compiler: Tier~1 proves
compiler templates correct for \textit{all} parameters (4/6 op types, Z3 SMT,
57~ms total); Tier~2 independently verifies all 512 B-spline LUT approximations
(512/512 UNSAT); Tier~3 composes them into a machine-checkable certificate
verified by a $\sim$200-line trusted checker.

\textbf{3. KAN vs.\ MLP Verification Gap.} KAN achieves 512/512 component
verifiability with end-to-end certification; MLP achieves neither--$38\times$
worse error propagation via DA and $13\times$ worse via IA, with zero
compositionally-verifiable components. The gap is architectural, not
implementation-dependent.

\textbf{4. Domain-Agnostic Pipeline.} Identical toolchain applied to bearing
fault diagnosis (CWRU, 99.93\%) and handwritten digit classification (MNIST,
98.6\%) yields identical verification guarantees (512/512 functions verified,
certificate valid), confirming the pipeline depends on KAN architecture, not
data provenance.

The compiler is validated at 45.2~KB work memory (90.4\% of the S7-1200
CPU~1211C 50~KB budget, TIA Portal V21 measured), compiling to \textbf{0 errors,
0 warnings}. All software experiments are reproducible from the released code
and checkpoints; TIA Portal V21 measurements require the Siemens toolchain
documented in Section~\ref{sec:tia}.

The SVNN framework opens a new research direction: identifying and designing
neural architectures whose structural properties enable formal compilation
guarantees, extending beyond KANs to other verifiable-by-design architectures
for safety-critical embedded deployment. As AI enters industrial control loops
governed by IEC~61508 and ISO~13849, the question is no longer \textit{whether}
to deploy neural networks on PLCs, but \textit{which architectures} admit the
design-time guarantees that functional safety demands.
