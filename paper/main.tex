...
\begin{enumerate}
    ...
    \item \textbf{Theory (Deployment Completeness):}
         ...
         and $N_{\text{LUT}}$ LUT points on Siemens S7-1200,
          ...
          $\text{WCET} = 3.86$~ms ($4.4\times$ margin within 100~ms scan
...
binary-search the grid index $k$ ($4$ REAL comparisons, $4 \times 0.30$
...
The Z3 analysis's per-edge LUT cost of $36.7$~$\mu$s
(\texttt{wcet\_analysis.json}) counts the complete generated-SCL
instruction stream per edge---binary-search loop iterations, scalar
loads, branches, and assignments---in addition to the arithmetic
operations listed above (which alone sum to $6.4$~$\mu$s).
The MatMul per edge contributes
$7.2$~$\mu$s (per-weight multiply-accumulate plus load and loop overhead
