...
\begin{table*}[tb]
\centering
\caption{结果总结：核心实验（E1--E16）与验证实验（V1--V7）}
\label{tab:summary}
\footnotesize
...
\midrule
\multicolumn{4}{l}{\textit{核心实验（E1--E16）：}} \\
...
E5 & 编译器可扩展性  & 宽度/深度/网格 & 11/12通过；内存瓶颈 \\
E6 & 分类        & 一致性 & \textbf{100\%}，最大对数3.65 \\
E6-SMT & Z3翻译验证 & 逐节点验证 & 9/11精确，2/11有界，0失败 \\
E7 & 跨载荷            & 0/2/3~hp准确率 & 99.97/100/99.97\% \\
E8 & 优化器消融    & 全流水线 & $-$11.9\%内存，+65\%速度 \\
E9 & 双元算术      & DA vs IA界 & \textbf{2.2$\times$更紧致} \\
E10& LUT稀疏性权衡& 准确率 vs 存储 & 3--50点均达99.96\% \\
E11& EMSE校准      & $M_2^{\text{char}}$ = 0.177, $M_2^{\max}$=1.586 & 比解析界紧致98.6\% \\
E12& XJTU-SY跨数据集 & 零样本 / 微调100轮 & 29.8\% $\to$ 91.7\% (+61.9 pp) \\
E13& 特征消融      & 时域/频域/色散熵    & 频域99.93\%；全28-D 99.96\% \\
E14& 对抗鲁棒性& 噪声$\sigma$=0--0.2 & 0\%退化 \\
E15& 自适应LUT分配 & 贪心 vs 均匀  & 71.6\% $\downarrow$最差$\varepsilon$ \\
E16& 分段感知DA       & Seg-DA vs DA       & 6.0$\times$紧致，安全$\sim$11.9$\times$ \\
E54& ChebyKAN Z3验证     & 逐函数 Z3 NRA   & \textbf{496/512 (96.9\%)}，命题~\ref{prop:chebykan-svnn} \\
E55& XJTU-SY 微调 + SCL       & 100轮FT + S7-1200 & \textbf{91.7\%}, 512/512 Z3, 0e 0w SCL \\
E56& 深层KAN ($L{=}3$)       & [28,16,8,4] 120轮 & \textbf{99.60\%}, 608/608 Z3, DA 14.6$\times$线性 \\
E57& CROWN vs.\ NeuroPLC     & KAN[28,16,4] DA/IBP & \textbf{85$\times$更紧致}, CROWN=IBP对KAN \\
\midrule
\multicolumn{4}{l}{\textit{验证实验（V1--V7）：}} \\
V1 & 最差情况对抗   & 5,000输入，最差100  & 100/100保持，已验证 \\
V2 & LLM vs NeuroPLC           & SCL代码生成   & LLM: 6缺陷，无权重；NeuroPLC: 0e 0w \\
V3 & DA $\sqrt{d}$缩放律 & 105架构，7维度 & Pearson $r{=}0.987$，斜率0.896 \\
V4 & MLP验证差距      & KAN $[28,16,4]$ vs.\ MLP $[28,32,16,4]$ & 512/512 vs.\ 0/48; $14.0\times$更差（逐组件） \\
V5 & ONNX vs NeuroPLC IR    & 节点数/代码    & 导出失败；$\geq$8,393 vs.\ 11节点 \\
V6 & Z3验证WCET        & S7-1200 CPU 1211C   & 总计 $\leq$2.86\,ms，周期2.9\% \\
V7 & 验证盲点   & $N{=}4{\to}15$扫描 & 准确率99.93\%但$N{\le}7$时安全$<$1；225翻转 \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize V系列：针对具体审稿问题的验证实验（LLM对比、ONNX不兼容、缩放律、验证差距、对抗安全、WCET）。}
\end{table*}
...
\begin{table}[!t]
    ...
    \begin{tabular}{lcc}
        ...
        分解节点（基本原语）       & ---                    & $\geq$\textbf{8,393} \\
        节点爆炸因子              & 1$\times$              & $\geq$\textbf{763}$\times$ \\
        生成代码行数              & \textbf{3,818} SCL     & $>$200,000 C（估计） \\
        代码体积                  & \textbf{45.2}\,KB      & $\gg$400\,KB（估计） \\
        ONNX Runtime体积          & \textbf{0}\,KB         & \textbf{22,528}\,KB \\
        适配S7-1200（50\,KB）？   & \textbf{是}             & \textbf{否}（450$\times$超出） \\
        形式验证就绪              & \textbf{是}（Z3 SMT）  & 否 \\
        \bottomrule
    \end{tabular}
    \vspace{2pt}
    {\scriptsize 实验I：\texttt{torch.onnx.export}使用opset~14/17/20时在B样条\texttt{einsum}处失败。分解估计：512函数 $\times$ 每B样条15个ONNX原语 = 7{,}680 + 713结构节点 $\approx$ 8{,}393。ONNX Runtime最小构建 $\approx$22~MB（无量化）。SCL行数来自实际NeuroPLC输出。}
\end{table}
