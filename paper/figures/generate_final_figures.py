"""
NeuroPLC Paper - Generate Final Figures using matplotlib 直接输出 PDF/PNG
使用 matplotlib 直接生成高质量 PDF/PNG（不依赖 Inkscape/cairosvg）
"""
import matplotlib
matplotlib.use("Agg")
import numpy as np
...
# ==================== IEEE 规格 ====================
# IEEE 单栏：3.5 inches = 89mm
# IEEE 双栏：7.16 inches = 183mm
...
def fig05_da_vs_ia():
    ...
    plt.savefig(output_dir / "fig05_da_vs_ia.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig05_da_vs_ia.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
...
def fig06_adaptive_lut():
    ...
    # 保存
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig06_adaptive_lut.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig06_adaptive_lut.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
...
def fig07_da_scaling():
    ...
    # 保存
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig07_da_scaling.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig07_da_scaling.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
    print(f"  -> {output_dir / 'fig07_da_scaling.pdf'}")

# ==================== 图4: Segment Bounds (单栏) ====================
def fig08_segment_bounds():
    ...
    # 保存
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig08_segment_bounds.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig08_segment_bounds.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
    print(f"  -> {output_dir / 'fig08_segment_bounds.pdf'}")

# ==================== 图5: WCET Breakdown (单栏) ====================
def fig09_wcet_breakdown():
    ...
    ax.set_ylabel('Time Share (%)', fontsize=9)
    ax.set_ylim(0, 85)
    ...
    # 保存
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig09_wcet_breakdown.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig09_wcet_breakdown.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
    print(f"  -> {output_dir / 'fig09_wcet_breakdown.pdf'}")

# ==================== 图6: Confusion Matrices (双栏) ====================
def fig10_confusion_matrices():
    """
    Confusion Matrices
    IEEE 双栏尺寸：7.16 x 3 inches
    """
    ...
    ax1.set_title('(A) Teacher (99.93%)', fontsize=11, fontweight='bold')
    ...
    ax2.set_title('(B) Student (99.93%)', fontsize=11, fontweight='bold')
    ...
    plt.tight_layout()
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig10_confusion_matrices.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig10_confusion_matrices.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
...
def fig11_tsne_features():
    ...
    for i, (center, color, label, marker) in enumerate(zip(centers, colors, labels, markers)):
        ...
        ax1.scatter(x, y, c=color, s=40, alpha=0.7, label=label,
    ...
    ax1.set_title('(A) No-KD (24.13%)', fontsize=11, fontweight='bold')
    ...
    for i, (center, color, label, marker) in enumerate(zip(centers_vrm, colors, labels, markers)):
        ...
        ax2.scatter(x, y, c=color, s=40, alpha=0.7, label=label,
                   edgecolors='white', linewidth=0.3, marker=marker)

    ax2.set_xlabel('t-SNE Dimension 1', fontsize=10)
    ax2.set_ylabel('t-SNE Dimension 2', fontsize=10)
    ax2.set_title('(B) VRM-KD (99.93%)', fontsize=11, fontweight='bold')
    ax2.legend(fontsize=8, framealpha=0.9, edgecolor='gray')
    ax2.grid(True, alpha=0.15, linestyle=':', color='gray')

    plt.tight_layout()
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig11_tsne_features.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig11_tsne_features.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
    print(f"  -> {output_dir / 'fig11_tsne_features.pdf'}")

# ==================== 图8: Cross Validation (单栏) ====================
def fig12_cross_validation():
    ...
    # 保存
    output_dir = Path("D:/neuroplc-paper/paper/figures/final")
    plt.savefig(output_dir / "fig12_cross_validation.pdf", format='pdf', bbox_inches='tight', dpi=300)
    plt.savefig(output_dir / "fig12_cross_validation.png", format='png', bbox_inches='tight', dpi=300)
    plt.close()
...
if __name__ == "__main__":
    print("\n" + "=" * 60)
    print("NeuroPLC Paper - Generate Final Figures (IEEE Format)")
    print("=" * 60 + "\n")
    ...
    print("\n" + "=" * 60)
    print("All figures generated in IEEE format!")
    print("=" * 60)
