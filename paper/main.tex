...
% Title
% ===========================================================================
\title{A Type Theory of Certifiable Neural Architectures: Sharp LUT Bounds, Complexity Separation, and Compile-Aware Generalization, with an Industrial PLC Instantiation}

\author{Fuyue~Liu,~\IEEEmembership{Guilin~University~of~Electronic~Technology, Guilin 541004, China}}

\markboth{IEEE TRANSACTIONS ON NEURAL NETWORKS AND LEARNING SYSTEMS, VOL. XX, NO. XX, 2026}%
         {Liu: A Type Theory of Certifiable Neural Architectures}
...
% ===========================================================================
\begin{abstract}
We present a \textbf{type theory of certifiable neural architectures}:
a formal characterization of which architectures admit design-time
correctness certification under LUT-discretized compilation, and why.
Four results anchor the theory. First, the \textbf{sharp k-th order LUT
bounds} (Theorem~\ref{thm:sharp-lut}): the classical de~Boor constant
$M_2h^2/8$ generalizes to all interpolation orders with explicit sharp
constants $c_k = Q_k/k!$ and an extremal family attaining them
\emph{exactly}---upgrading the DA optimality claim from an empirical
statement on random cubics to an exact minimax-optimality theorem with a
certificate. Second, the \textbf{verifiability trichotomy}
(Theorem~\ref{thm:trichotomy}): the SVNN condition is \emph{conjecturally}
the unique regime where second-order rate, linear LUT cost, closed-form
sharp constants, and freedom from reachability computation
coexist---ReLU
architectures admit only first-order bounds, multivariate gates suffer
$N^m$ LUT cost and NP-hard tightness, and domain-dependent curvature
requires reachability. Third, \textbf{$\varepsilon$-separation}
(Theorem~\ref{thm:necessity}): the LUT-verification decision problem is
polynomial for SVNN (self-contained 3-SAT reduction showing NP-hardness
for coupled architectures), and SVNN uniquely attains the Pareto point
(polynomial-time verification, zero slack under ETH); the completeness
of this boundary is the trichotomy necessity conjecture. Fourth, the \textbf{compile-aware generalization bound}
(Theorem~\ref{thm:generalization}): the deployed (compiled) risk
decomposes into a generalization gap $O(\gamma^{L-1}/\sqrt{n})$ and a
LUT bias $O(\gamma^{L-1}c_kM_kh^k)$, yielding the
\emph{resolution-matching law} $N^{*} \asymp n^{1/(2k)}$---the deployed
LUT memory should scale as the $2k$-th root of the sample count.

The four results are not an assemblage of independent bounds: they are
the expansion of a single unifying quantity, the \textbf{verifiable
deployment capacity} $C(N,V)$ ({\S}\ref{sec:capacity})---the best
certifiable deployment error under a storage budget of $N$ cells and a
certificate-verification budget $V$. A universal packing line bounds
every scheme; three verification strata (closed-form / kinked /
structure-optimal) place every architecture; and a
\textbf{certificate proof system} $\text{CP}$---following the
proof-carrying-code and validated-numerics tradition (Gappa,
Sollya, LeanCert)---formalizes the verification budget as proof length
under perfect deterministic soundness. Within this framework the sharp
constants reappear as the capacity limit of the interpolation code, the
trichotomy reappears as three points in the $(V_B, \varepsilon)$ plane,
and the necessity direction receives a theorem-level
information-theoretic form; an architecture-as-code law decides between
table and spectral codes from the smoothness of the target.

We instantiate this theory in \neuroplc, the first
self-verifying PyTorch-to-IEC~61131-3 compiler (formally verified
templates, certificates, and composition rules; the emitter is validated
by differential self-testing), with four-tier verification: Z3
template proofs, 512/512 per-function certificates, a machine-checkable
composition certificate, and a \emph{differential self-test} that catches
code-generation regressions (it detected a real emitter bug and
validated the fix; the bug had degraded deployed accuracy to 83\%). Generated code
compiles to 0 errors/0 warnings in TIA Portal V21 with a vendor-neutral
IEC~61131-3 variant, and the runtime hot-swap protocol maintains the
certified safety state across model updates. Deployment-level
certificates close the loop: adaptive LUT allocation lifts the DA safety
factor to $3.6\times$ at unchanged storage, and soft-contractive training
to a first-principles \emph{sound} certificate of $8.37\times$ (pure
propagation, no empirical floor; IEEE-754 $\delta_{\text{fp32}} \lesssim
3\times10^{-5}$) / $493\times$ expected, with an empirically validated
$11.6\times$ float32 tier (measured floor $0.058$ vs.\ measured maxAE
$0.053$) and $99.9\%$ Box-Continuation coverage,
Z3-verified components, and depth extension to $L=3$ ($6.1\times$). Four $C^2$-BV architectures
(B-spline, Chebyshev, Fourier, Wavelet) and three datasets (CWRU, XJTU-SY,
Paderborn) validate the framework, with MNIST as a fourth dataset (E42);
RBF-KAN remains theoretical. The theory answers not ``can we compile neural networks to
embedded targets''---prior tools showed that---but ``which architectures
admit \emph{certified} compilation, with what sharp constants, at what
complexity, and with what generalization guarantee?''
\end{abstract}

\begin{IEEEkeywords}
Neural Architecture Certification, Sharp Interpolation Constants,
Verifiability Trichotomy, Complexity Separation, Compile-Aware
Generalization, Resolution Matching, Galois Connection, IEC 61131-3,
Kolmogorov-Arnold Networks, Type Theory.
\end{IEEEkeywords}
...
architectures}---a formal characterization of which architectures admit
design-time correctness certification under LUT-discretized compilation,
with what sharp constants, at what computational cost, and with what
learning-theoretic guarantee. The central insight is that the question
``can a neural network compiler provide design-time correctness
guarantees?'' is, at its
...
curvature bound). The \textbf{SVNN Sufficiency Theorem} (Theorem~2)
guarantees that well-typed architectures have computable, finite
per-output error bounds under LUT-discretized compilation---a property that
ill-typed architectures (standard MLPs with $\sigma(W\mathbf{x}+\mathbf{b})$)
provably lack (Proposition~\ref{prop:mlp_negative}). The typing is
\emph{numerical} (it certifies error bounds under discrete execution),
complementary to categorical constructions that embed logical
constraints into architecture~\cite{nye2025categorical}: there,
well-typedness rules out logical violations; here, it rules out
unbounded discretization error.
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Optimality (Corollary~\ref{cor:da-optimality}
    $+$ Theorem~\ref{thm:galois} $+$ Theorem~\ref{thm:tightness}):}
    DA is the \textbf{Galois-optimal} abstract domain for $C^2$ univariate
    functions---to our knowledge, the first Galois connection for neural
    network activations---
    and its bound $M_2 h^2 / 8$ is \textbf{semantically tight} (attained
    by $f^*(x) = M_2 x^2 / 2$ at every LUT midpoint, $1{,}000/1{,}000$
    exact), with the sharp-constant generalization to all orders
    (Theorem~\ref{thm:sharp-lut}) attained exactly by an explicit
    extremal family (E63).
...
This paper goes beyond sufficiency to establish the \textbf{exact
boundaries and quantitative laws} of certifiable compilation:

\begin{enumerate}[label=(\roman*)]
    \item \textbf{Sharp constants (Theorem~\ref{thm:sharp-lut}).}
    The de~Boor bound $M_2 h^2/8$ is the $k=2$ instance of a sharp
    all-orders family: $c_k = Q_k/k!$ with explicit extremal functions
    attaining the bound \emph{exactly} ($k=3$: $1/(9\sqrt{3})$, not the
    folklore $1/8$; $k=4$: $1/24$). DA's optimality is exact, not
    asymptotic---verified at machine precision.

    \item \textbf{The verifiability trichotomy
    (Theorem~\ref{thm:trichotomy}).}
    The SVNN condition is not one sufficient criterion among many: it is
    \emph{conjecturally} the unique regime where second-order rate,
    linear LUT cost, closed-form sharp constants, and reachability-freedom
    coexist (the necessity direction of the theorem is a conjecture with
    strong evidence).
    ReLU admits only first-order bounds; multivariate gates (products,
    softmax) suffer $N^m$ LUT cost and NP-hard tightness; domain-dependent
    curvature ($e^x$) requires reachability.

    \item \textbf{$\varepsilon$-separation
    (Theorem~\ref{thm:necessity}).}
    The LUT-verification \emph{decision problem} is polynomial for SVNN
    and NP-hard for coupled architectures (self-contained 3-SAT reduction
    via product gates); SVNN uniquely attains (polynomial time, zero
    slack under ETH) simultaneously (completeness of this boundary is the
    trichotomy necessity conjecture).

    \item \begingroup\tolerance=9999\emergencystretch=3em\textbf{Compile-aware generalization
    (Theorem~\ref{thm:generalization}).}
    The deployed risk decomposes as
    \hspace{0pt}$\hat{R}(N) + O(\gamma^{L-1}/\sqrt{n}) + O(\gamma^{L-1}c_kM_kh^k)$,
    yielding the \textbf{resolution-matching law}
    $N^{*} \asymp n^{1/(2k)}$:\hspace{0pt} deployed LUT memory should scale as the
    $2k$-th root of the sample count---a directly actionable compiler
    rule no existing PAC bound states.\par\endgroup

    \item \textbf{Verifiable deployment capacity
    (Theorem~\ref{thm:capacity-stratification}, {\S}\ref{sec:capacity}).}
    The four results above are unified as the expansion of a single
    quantity: the capacity $C(N, V)$ of certifiable deployment under a
    storage budget of $N$ cells and a certificate-verification budget
    $V$. A
    universal packing line (metric entropy) bounds every scheme; three
    verification strata---closed-form (certificate-free),
    kinked (rate-degraded), and structure-optimal
    (verification-exponential)---place every architecture class; and a
    certificate proof system (following Gappa/Sollya/LeanCert and PCC)
    formalizes $V$ as proof length under perfect deterministic
    soundness, yielding a theorem-level information-theoretic necessity
    within the calculus. An architecture-as-code law (table vs.\
    spectral codes) follows from the smoothness of the target.
\end{enumerate}
...
pre-existing safety properties under the timing premise
$\text{WCET}(P) + \text{WCET}(N) \leq$ scan cycle, directly addressing
IEC~61508 independence requirements---to our knowledge, the first such
guarantee for neural-network-on-PLC deployment. An IEC~61131-3 REAL arithmetic
...
(Theorem~\ref{thm:wcet}: $3.86$~ms, $25.9\times$ scan-cycle margin)
...
We instantiate the SVNN type system in \neuroplc,, to our knowledge, the first IR-based
...
abstract domain for $C^2$ functions---achieving $2.1\times$ tighter
error bounds than interval arithmetic ($1.0\times$ safety factor at
the $M_2^{\text{char}}$ calibration, up to $3.1\times$ vs.\ the
$\sqrt{d}$ random-weights baseline, {\S}\ref{sec:da}). KAN's B-spline decomposition is \textit{a structural
necessity}: identically-sized KAN vs.\ MLP yield 512/512 vs.\ 0/16
Z3-verifiable components (MLP~$[28,16,4]$, E41).
...
violating the SVNN conditions (Theorem~5). \neuroplc instantiates the
theory as, to our knowledge, the first self-verifying PyTorch-to-IEC~61131-3 compiler,
demonstrating that the certification transfers across vendors (a
vendor-neutral IEC~61131-3 backend with identical semantics) and across
...
\begin{enumerate}
    \item \textbf{Theory (Type-Theoretic Foundation):} The SVNN framework
         ...
         up to $d=128$ (MATLAB). \textbf{Sharp k-th Order LUT Bounds}
          (Theorem~\ref{thm:sharp-lut}) generalize the de~Boor constant to
         all orders with exact sharp constants $c_k = Q_k/k!$ and an
         extremal family attaining them at machine precision (superseding
         the earlier cubic-restricted, Monte-Carlo-verified DA optimality
         claim). \textbf{SVNN Compositionality}
    ...
    \item \textbf{Theory (Abstract Interpretation Foundation):}
         ...
         interpretation framework with a defined Galois connection
          $(\alpha, \gamma)$, making NeuroPLC, to our knowledge, the first neural network
    ...
    \item \textbf{Theory (Deployment Safety):} The
         ...
         all pre-existing safety properties under the timing premise
         $\text{WCET}(P) + \text{WCET}(N) \leq$ scan cycle---to our
         knowledge, the first such guarantee for
         neural-network-on-PLC deployment, directly addressing
         IEC~61508 independence requirements. The \textbf{WCET Theorem} (Theorem~11)
         provides parameterized worst-case execution time bounds
          ($3.86$~ms for KAN $[28,16,4]$, $25.9\times$ scan-cycle margin).
    ...
    \item \textbf{Algorithm (Verification \& Optimization):} Three
         algorithms: (i)~\textbf{Sign-structural affine arithmetic (DA)},
         achieving a $2.1\times$ tighter error bound than interval arithmetic
         ...
         for substantial per-segment tightening (composing with DA for a
         $3.6\times$/$6.6\times$ safety-factor improvement at
         $N{=}15/20$ under the worst-function calibration, recomputed
         2026-08-04); and
    ...
    \item \textbf{Theory (Completeness):} Beyond sufficiency, we establish
          \textbf{$\varepsilon$-separation} (Theorem~5,
          {\S}\ref{sec:svnn-necessity}): the LUT-verification \emph{decision
         problem} is polynomial for SVNN (with sharp constants,
         Theorem~\ref{thm:sharp-lut}) and NP-hard for coupled architectures
         (self-contained 3-SAT reduction via product gates), and SVNN
         uniquely attains the Pareto point (polynomial time, zero slack
         under ETH; completeness of the boundary is the trichotomy necessity
         conjecture).
         Condition~3 yields a depth-improving
         ...
         and \textbf{E57} (NeuroPLC DA substantially tighter than
         CROWN-IBP, which reduces to IBP for KAN)
    ...
    \item \textbf{Theory (Algebraic Completeness):} \textbf{Theorem~8}
         ...
         compose their guarantees without re-analysis.
          \textbf{Theorem~9} ({\S}\ref{sec:da-optimality}) proves the
         sharp $k$-th order LUT error bounds: the de~Boor constant
         $M_2h^2/8$ is the $k=2$ instance of an exact all-orders family
         $c_k = Q_k/k!$ ($c_3 = 1/(9\sqrt{3})$, $c_4 = 1/24$), attained
         by an explicit extremal family at machine precision---DA is
         minimax-optimal among affine design-time schemes
         (Theorem~\ref{thm:sharp-lut}, E63).
    ...
    \item \textbf{Theory (Deployment Completeness):}
          \textbf{Theorem~11} ({\S}\ref{sec:wcet}) provides the worst-case
         ...
         and $N_{\text{LUT}}$ LUT points on Siemens S7-1200,
          ...
          $\text{WCET} = 3.86$~ms ($25.9\times$ margin within 100~ms scan
         cycle, $3.9\%$ utilization).
    ...
    \item \textbf{System (Multi-Architecture Validation):}
         ...
         and WaveletKAN architectures: FourierKAN $[28,16,4]$ (6 harmonics,
          ...
          512/512 edges satisfying $M_2 \cdot h^2 / 8 \leq 0.063$
          ($\mathbf{100\%}$ Z3-equivalent);
         WaveletKAN $[28,16,4]$ (8 Mexican hat scales, M2-regularized,
          $\sim$4,600 params)
         achieves 100.0\% CWRU accuracy with 512/512 edges verified
          ($\mathbf{100\%}$ Z3-equivalent), confirming the $C^2$-BV
...
74 experiments (E1--E58 + E60--E68
+ V1--V7) plus eleven capacity-theory verification experiments
(E-T1--E-T11, {\S}\ref{sec:capacity}) spanning compression,
generalization, correctness bounds,
...
CNN-1D-KAN~\cite{cnn1dkan2025}, and polynomial-basis KAN variants
...
that KAN's univariate B-spline structure is, to our knowledge, uniquely amenable to LUT-based
...
LUT-KAN~\cite{lutkan2024} replaces Cox-de~Boor recursion with precomputed
...
Proposition~\ref{prop:mlp_negative}). Corr\^ea demonstrates KAN-on-PLC feasibility but provides no
native SCL compilation, no correctness guarantee, and no compiler toolchain.
\neuroplc is, to our knowledge, the first to provide all three.

\textbf{KAN verification.} Despite two decades of NN verification
benchmarks, the KAN verification landscape remains nascent: none of the
25 benchmarks of VNN-COMP 2025 (16 regular, 9 extended) involves KAN
activations~\cite{vnncomp2025}, and the existing KAN-specific efforts
are instance-level. Schwartz et~al.~\cite{schwartz2026optimal} proposed
PWA abstraction with MILP encoding for KAN verification---operating at SVNN
Level~3 (exact certification within PWA abstraction error). In this paper we
certify the trained model under Conditions~1--2
({\S}\ref{sec:svnn-conditions})---the trained student is \emph{not}
contractive (measured per-layer amplification $\gamma = [15.4, 5.3]$,
E68), and we identify contractive training as the recipe for the
depth-uniform certificate---noting that
polynomial-basis KANs admit structural Lipschitz bounds via coefficient
magnitudes. KAN4CBC~\cite{zhang2025kan4cbc} demonstrated
...
pre-certification for CI/CD integration.
...
contribute, to our knowledge, the first VRM-KD to a KAN student: the distillation is the
...
% ===========================================================================
\input{section_characterization}
% ===========================================================================

\input{section_galois}
% ===========================================================================

\input{section_tightness}
% ===========================================================================

\input{section_ir_semantics}
...
engineering-toolchain compilation evidence (0e 0w). The compiler operates
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig1_overview}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig2_compiler_arch}
...
The Siemens-specific surface is minimal---\texttt{S7\_Optimized\_Access},
\texttt{VERSION}, \texttt{NON\_RETAIN} attributes and the quoted
DB/FB identifiers---so the same IR and the same verification chain
(Tiers~1--4) support a \emph{vendor-neutral} IEC 61131-3 backend
(\texttt{backend\_iec.py}): standard \texttt{VAR\_GLOBAL}/\texttt{VAR}/
\texttt{FUNCTION\_BLOCK} syntax with zero Siemens-specific tokens,
compilable by CODESYS/TwinCAT/any IEC-compliant toolchain. The generated
vendor-neutral ST has identical semantics to the Siemens SCL
(Tier~4 differential test: 100\% agreement, max logit error 0.47, E67),
confirming that the certification is target-independent---the
IEC~61131-3:2025 REAL semantics (Lemma~\ref{lem:iec_universal}) transfer
to any compliant controller. This is the multi-vendor deployment path
required by the trichotomy's Regime-1 design guidance.
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig06_adaptive_lut}
...
where $\Delta = 10/(N-1)$ is the grid spacing, $M_2^{\text{SiLU}} = 0.5$,
...
For SiLU on $[-5,5]$: $|\text{SiLU}''(x)| = 0.5$ (analytically,
the maximum occurs at $x=0$). With $N=64$,
$\Delta = 10/63 \approx 0.1587$, giving $\varepsilon_{\text{SiLU}} \leq 0.00157$.
...
is bounded by Theorem~1: the IA envelope is $\Delta \leq 1.38$ (sound, but
exceeding the half-margin $0.675$), and the DA certificate $\Delta \leq 0.66$
({\S}\ref{sec:da}, recomputed 2026-08-03) ensures classification
correctness is maintained with a safety factor of $1.0\times$.
\end{proof}
...
Eq.~\eqref{eq:ia_l1} accounts for Lipschitz amplification ($L_B = 2.21$
layer-0 / $2.05$ layer-1, measured over the trained student's 512
B-spline activations, E68) at the inter-layer transition before layer 1's
linear combination.
With measured $\|W^{(0)}\|_{1,\max} = 0.32$ and
...
\begin{equation}
\Delta z_{\max} \leq 1.38, \quad \min\nolimits_{i \neq \hat{y}}
(\text{logit}_{\hat{y}} - \text{logit}_i)_{\text{correct}} = 1.35
\label{eq:margin_guarantee}
\end{equation}

The interval-arithmetic envelope ($1.38$, recomputed from the released
checkpoint 2026-08-03) exceeds the half-margin $1.35/2 = 0.675$: the IA
bound is sound but is \emph{not} a classification certificate for this
checkpoint. The DA bound ($0.66$, {\S}\ref{sec:da}) provides an
\emph{expected-tier} (high-probability, random-sign) certificate with a
safety factor of $1.0\times$ at the $M_2^{\text{char}}$ calibration
(per-logit-element, in-domain; the worst-function $M_2^{\max}$
calibration pushes the bound above the half-margin,
{\S}\ref{sec:limitations}): classification correctness holds with
high probability under the LUT error distribution \emph{within the
Box-Continuation domain}, but no \emph{sound} certificate exists for
this checkpoint (the full-$\varphi$ float64 recalibration gives $1.70$,
safety $0.4\times$). The margin is restorable to $3.6\times$ via
adaptive allocation, {\S}\ref{sec:adaptive-lut}, or to $8.37\times$
(sound) / $11.6\times$ (validated) / $493\times$ (expected) via
soft-contractive training, E-T9.
...
logit perturbation of 0.66 vs.\ 1.38 for IA---a
\textit{2.1$\times$ tightening} (recomputed from the released checkpoint,
2026-08-03). The IA envelope exceeds the half-margin $0.675$ and is not
a certificate; the DA bound certifies classification with a safety
factor of $1.0\times$ at the $M_2^{\text{char}}$ calibration; with
adaptive LUT allocation (E15, unchanged storage budget) the bound drops
to $0.19$---a $3.6\times$ safety factor---and the IA envelope to $0.39$
(now a sound certificate, $1.7\times$), Table~\ref{tab:da_bounds_summary}.
This applies affine arithmetic to
compiled neural network verification on industrial controllers.
...
\label{tab:da_sign}
\footnotesize
\begin{tabular}{@{}p{1.9cm}p{1.5cm}p{1.5cm}p{1.5cm}p{1.3cm}@{}}
\toprule
...
\setcounter{lemma}{2}
\begin{lemma}[Moment-Robust DA/IA Tightening Ratio]
\label{lem:da_moment}
Let $\mathbf{w} = (w_1, \dots, w_{d_1}) \in \mathbb{R}^{d_1}$ be
i.i.d.\ weights with symmetric, mean-zero distribution, bounded
$|w_j| \leq w_{\max}$ a.s., with $\mu := \mathbb{E}|w| > 0$,
$\nu^2 := \mathbb{E} w^2$, and finite moment ratio
$\kappa := \nu/\mu$. Let $\boldsymbol{\delta} = (\delta_1, \dots,
\delta_{d_1})$ be the per-neuron LUT errors, with $|\delta_j| \leq e$
and $\boldsymbol{\delta}$ \emph{independent of} $\mathbf{w}$ (LUT error
signs are determined by B-spline geometry, not by weight values). Define
the DA error term $S = \sum_j w_j \delta_j$ and the IA error term
$E_{\text{IA}} = e \sum_j |w_j|$. Then for every $p \in (0,1)$:

\begin{enumerate}
    \item \textbf{Upper tail (Bennett).} With probability at least
    $1-p$ over $\mathbf{w}$,
    \begin{equation}
    |S| \;\leq\; C \cdot e \cdot \nu \cdot \sqrt{d_1 \log(2/p)}
    \label{eq:bennett_upper}
    \end{equation}
    for an absolute constant $C$ (e.g., $C = 4$ suffices when
    $w_{\max} \leq 3\nu$, which holds for Gaussian, uniform, Laplace,
    and all sub-Gaussian families; the Hoeffding-form
    $t = C e w_{\max}\sqrt{d_1 \log(2/p)}$ is a uniform fallback).

    \item \textbf{Lower tail.} With probability at least
    $1 - \exp(-d_1 \mu^2 / (2 w_{\max}^2))$,
    \begin{equation}
    E_{\text{IA}} \;\geq\; \tfrac{1}{2}\, d_1 e \mu.
    \label{eq:lower_tail}
    \end{equation}

    \item \textbf{Ratio separation.} With probability at least
    $1 - p - \exp(-d_1\mu^2/(2w_{\max}^2))$,
    \begin{equation}
    R \;\equiv\; \frac{E_{\text{IA}}}{|S|} \;\geq\;
    \frac{\mu}{2\sqrt{2}\, C\, \kappa} \cdot
    \sqrt{\frac{d_1}{\log(2/p)}}
    \;=\; \Omega\!\left( \frac{\sqrt{d_1}}{\sqrt{\log(1/p)}} \right)
    \label{eq:moment_ratio}
    \end{equation}
    whenever $\kappa = O(1)$. Union-bounding over $d_0$ output
    coordinates at failure level $\delta_0$:
    $R \geq \Omega(\sqrt{d_1}/\sqrt{\log(d_0/\delta_0)})$ with
    probability $\geq 1 - \delta_0$.
\end{enumerate}

\noindent\textbf{Sharpness (necessity of the moment condition).}
The condition $\kappa = O(1)$ is \emph{necessary} up to constants: for
the sparse-outlier family $w_j \in \{0, \pm d_1 \mu\}$ with
$\mathbb{P}(\pm d_1\mu) = 1/(2d_1)$, $\mathbb{P}(0) = 1 - 1/d_1$
(so $\kappa = \sqrt{d_1}$, bounded weights, finite variance), the
adversarial $\delta$ aligned with the outlier gives
$R = O(1/\sqrt{\log(1/p)})$ with probability $\approx 1 - e^{-1}$
---no $\sqrt{d_1}$ gain is possible. With only a finite second moment
(no $\kappa$ bound), the best guaranteed ratio is
$R \geq \Omega(1/\sqrt{\log(1/p)})$: the width benefit evaporates
($\mathbb{E}|w| \geq \mathbb{E}w^2/w_{\max} \geq \nu/\sqrt{d_1}$
by Cauchy--Schwarz in the worst case).
\end{lemma}

\vspace{2pt}
\noindent\textit{Proof sketch.}
\textbf{(1)} Apply Bennett's inequality to $X_j = w_j \delta_j$: for
fixed $\boldsymbol{\delta}$ with $|\delta_j| \leq e$,
$\mathbb{E}[X_j] = 0$ (symmetry), $|X_j| \leq w_{\max} e =: b$, and
variance $V = \sum_j \delta_j^2 \nu^2 \leq d_1 e^2 \nu^2$. Bennett
gives $\mathbb{P}(|S| \geq t) \leq 2\exp(-t^2 / (2(V + bt/3)))$;
solving for $t$ yields Eq.~\eqref{eq:bennett_upper} with the stated
$C$. \textbf{(2)} One-sided Hoeffding on the non-negative bounded
variables $|w_j| \leq w_{\max}$ (no magnitude uniformity required).
\textbf{(3)} Combine: $|S| \leq C e \nu \sqrt{d_1 \log(2/p)}$ and
$E_{\text{IA}} \geq \tfrac{1}{2} d_1 e \mu$ simultaneously, giving
$R \geq (d_1 \mu / 2) / (C \nu \sqrt{d_1 \log(2/p)})
= \tfrac{\mu}{2C\nu} \sqrt{d_1/\log(2/p)}$; with $\kappa = \nu/\mu$
this is Eq.~\eqref{eq:moment_ratio}. \textbf{(4)} For the outlier
family, $\mathbb{P}(\exists j: |w_j| = d_1\mu) = 1 - (1-1/d_1)^{d_1}
\to 1 - e^{-1}$; conditioned on the outlier, the adversary's
$\delta$ gives $|S| = e d_1 \mu = E_{\text{IA}}$, so $R = 1$.
Unconditionally $R = O(1)$ with constant probability, defeating any
high-probability $\sqrt{d_1}$ bound. $\square$

\vspace{2pt}
\noindent\textbf{Comparison with the earlier statement.} The previous
version of this lemma required uniform magnitudes
($w_{\min}^2 \leq |W_{k,j}^{(1)}W_{j,i}^{(0)}| \leq w_{\max}^2$) and
claimed $R \geq 2.2$ at $d_1=16, \delta=0.05$; its core inequality
$|\sum_j m_j s_j| \leq w_{\max}^2 |\sum_j s_j|$ fails for non-uniform
magnitudes (counterexample: $d_1=2$, magnitudes $(w_{\max}^2,
w_{\min}^2)$, signs $(1,-1)$), and its own formula evaluates to
$R \geq 0.60$, not $2.2$. Lemma~\ref{lem:da_moment} replaces the
uniform-magnitude restriction with the moment ratio
$\kappa = \nu/\mu$---computable from the weight distribution---and
supplies the sharpness counterexample proving the condition tight.
Empirically (E64), $R \approx 4.6$ at $d=16$, $9.5$ at $d=64$,
$18.8$ at $d=256$, $37.7$ at $d=1024$ for Gaussian weights,
matching $\sqrt{d}/\kappa$; the trained KAN's measured $R = 3.1\times$
($d_1 = 16$) is consistent with its weight statistics.

\vspace{4pt}
\noindent\textit{Implication for Theorem~1.}
Lemma~\ref{lem:da_moment} upgrades the DA bound from an empirical observation to a
...
(using IA as the conservative baseline), Lemma~
\ref{lem:da_moment} establishes that the
...
and the typical case (Lemma~
\ref{lem:da_moment}, DA) are relevant, the dual bounds
provide a complete picture: Theorem~1 guarantees correctness under
any weight configuration, while Lemma~
\ref{lem:da_moment} quantifies how much tighter
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig07_da_scaling}
...
sign coherence from training---exactly as Lemma~
\ref{lem:da_moment} predicts. This establishes
...
confirms the scaling law predicted by Lemma~
\ref{lem:da_moment}.}
...
Figure~\ref{fig:da_vs_ia} compares the DA and IA bounds across LUT
resolutions, and Figure~\ref{fig:wcet_breakdown} ({\S}WCET) breaks
down the worst-case execution time.

\vspace{2pt}
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/final/fig05_da_vs_ia}
    \caption{\textbf{DA vs.\ IA Bound Comparison with LUT Resolution.}
    Left: DA achieves $2.1\times$ tighter bounds than IA across all LUT resolutions
    ($N=8$--$20$), decreasing $\propto 1/N^2$ (Theorem~\ref{thm:tightness}).
    At $N{=}15$: DA $=0.66$, IA $=1.38$ (recomputed from the released
    checkpoint, 2026-08-03). Right: bar comparison.
...
further tightening on top of DA's $2.1\times$ improvement over IA
...
(Seg-DA, E16; recomputed on the released checkpoint, 2026-08-04) addresses
the heavy-tail regime directly: under the worst-function calibration
($M_2^{\max}$, global bound $5.9$), per-input segment errors reduce the
bound to $1.65$ ($N{=}15$) and $0.89$ ($N{=}20$)---a
$3.6\times$/$6.6\times$ tightening precisely where the
plain-$M_2^{\text{char}}$ bound under-delivers. (The earlier
$11.9\times$ figure mixed $M_2$ calibrations and is superseded by this
sound recomputation, \texttt{verify\_seg\_da.py}.) Combined with adaptive
allocation (E15), plain DA itself reaches a $3.6\times$ safety factor at
unchanged storage ({\S}\ref{sec:adaptive-lut}).

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig08_segment_bounds}
...
benefit among active functions.\par\endgroup
...
Under LUT interpolation error $\delta_{\text{in}}$ in the input, the mean value theorem gives $|\phi(x+\delta_{\text{in}}) - \phi(x)| = |\phi'(\xi)| \cdot |\delta_{\text{in}}| \leq L_B \cdot |\delta_{\text{in}}|$ for some $\xi \in [x, x+\delta_{\text{in}}]$.
...
($X = [-3,3]^{28}$ for the CWRU case study). Assume the
Box-Continuation condition (Lemma~\ref{lem:box_continuation}): the
reachable intermediate signals remain within the LUT domains, verified
in $O(L \cdot d_{\max}^2 \cdot N)$ time. For all $x \in X$ satisfying this condition, two complementary
...
$L_{\text{net}}^{\text{IA}} = 171.1$,
$\Delta_{\text{logit}}^{\text{IA}} \leq \mathbf{1.38}$,
which exceeds the half-margin $0.675$: the interval-arithmetic bound is
sound but not a classification certificate for this checkpoint
(recomputed from the released checkpoint, 2026-08-03;
\texttt{verify\_da\_bounds\_recomputed.py}).
...
of Lemma~
\ref{lem:da_moment} (and empirically for the trained model):
\begin{equation}
...
\label{eq:l_net}
\end{equation}
For the trained KAN: $L_{\text{net}}^{\text{DA}} \approx 162.2$,
$\Delta_{\text{logit}}^{\text{DA}} \leq \mathbf{0.66}$,
\textbf{DA safety factor $1.0\times$} (margin $1.35 / 2{\times}0.66$). The DA
bound certifies classification correctness ($0.66 <
\text{min-margin}/2 = 0.675$) at the $M_2^{\text{char}}$ calibration,
but the margin is thin: under the measured per-function LUT-error
distribution ($M_2 h^2$ proxy: mean $0.017$, max $0.041$, E58; the
de~Boor $\varepsilon$ bound used in Eq.~\ref{eq:theorem1} is
$M_2^{\text{char}} h^2/8 = 0.00406$) the bound
exceeds the half-margin. Two deployment recipes restore a comfortable
margin at unchanged storage: adaptive LUT allocation (E15,
{\S}\ref{sec:adaptive-lut}) lifts the DA safety factor to $3.6\times$,
and soft-contractive training (E-T9) to $8.37\times$ (first-principles
sound) / $11.6\times$ (validated) with full
Box-Continuation coverage---both implemented and verified
(2026-08-04).

The DA bound is the effective correctness certificate;
the IA bound is the conservative sound upper envelope.
...
By Lemma~\ref{lem:per_op} (MatMul), the SCL MatMul is exact up to IEEE~754 rounding.
...
input $p$. By Lemma~\ref{lem:per_op} (BsplineLUT), the LUT introduces $\varepsilon_{\text{LUT}}$
fresh error per activation. By Lemma~\ref{lem:bspline_lipschitz}, the input error $\Delta_p$ is
...
$[-3,3]$. By Lemma~\ref{lem:per_op} (StandardAct), the SCL implementation is exact. Hence
...
$\text{base}_j + \text{spline}_j$ per output neuron. By Lemma~\ref{lem:per_op} (Add),
...
By Lemma~\ref{lem:per_op} (Softmax), the softmax Lipschitz constant is $\frac{1}{2}$ in $\ell_\infty$.
Hence $\Delta_n \leq \frac{1}{2} \cdot \Delta_p$. The softmax does not alter
the argmax ordering: by Lemma~\ref{lem:per_op} (Argmax), correctness is preserved as long as
...
is less than half the minimum inter-class margin (Lemma~\ref{lem:per_op} (Argmax)). This is
the \textit{output correctness criterion}: Theorem~1 guarantees semantic
preservation at the logit level; Lemma~\ref{lem:per_op}(vi) guarantees that this
...
162.2$ (DA bound), yielding $\Delta_{\text{logit}} \leq 0.00406 \times
162.2 = 0.66$ at 15 LUT points. The minimum inter-class margin among
correctly classified test samples is $1.35 > 2 \cdot 0.66 = 1.32$ (E6),
satisfying Lemma~\ref{lem:per_op}(vi) and guaranteeing classification correctness.
\end{theorem}
...
Lemma~
\ref{lem:da_moment}, this bound can be tightened probabilistically: with
...
0.66$ (DA form), while the interval-arithmetic form would yield
$1.38$. Lemma~\ref{lem:da_moment} guarantees that the DA/IA ratio
$R \geq \Omega(\sqrt{d_1})$ holds with high confidence across random
KAN initializations (with a moment-ratio constant computable from the
weight statistics), meaning the deployed safety margin is
$\sqrt{d_1}$-times larger than a naive interval analysis would report.
The empirical ratio $R = 3.1\times$ (Table~\ref{tab:da_sign}) and the
30-seed distribution (Fig.~\ref{fig:da_ratio_dist}) confirm that
trained KANs consistently exceed this conservative probabilistic
lower bound.
\end{remark}
...
per-element DA bound (0.66) and the empirical worst-case logit perturbation
(MaxAE 3.65, E6).
...
\vspace{2pt}
\noindent\textit{Why the per-element DA bound (0.66) is smaller than MaxAE (3.65).}
The DA bound of 0.66 is a per-logit-element, \emph{in-domain} guarantee:
for any single output dimension $k$ and any input whose intermediate
signals remain inside the LUT grid, $|\Delta z_k| \leq 0.66$. The empirical
MaxAE of 3.65, by contrast, is the maximum absolute perturbation across all
logit dimensions and all test samples---including samples whose layer-1
activations leave the $[-3,3]$ LUT grid (47\% of layer-1 signals at
deployment, E68), where the LUT extrapolation diverges from the learned
B-spline. The gap is therefore explained by input \emph{domain}, not by
the aggregation method: the MaxAE samples lie outside the certificate's
scope. Within the certified in-domain regime, three secondary effects
...
The combined effect of these secondary factors is dwarfed by the domain
effect: the empirical maximum is dominated by out-of-domain samples,
where the LUT extrapolation diverges (47\% of layer-1 signals leave
$[-3,3]$, E68) and the certificate no longer applies. Critically, this
analysis confirms that the DA bound is not
\textit{incorrect}---it correctly bounds the per-element error for typical
in-domain inputs---but it is incomplete as an aggregate guarantee. We therefore
...
correlation-aware aggregate bound evaluates to $2.95$ at $N=15$ LUT points
(recomputed with the audit constants $L_{\text{net}}^{\text{IA}} = t_1 =
160.4$; the DA amplification constant $L_B^{(1)} m_{\text{row}} + s_1 =
162.2$ is a distinct quantity, 2026-08-04),
covering the in-domain empirical worst case ($0.80$, E53) with a safety
factor of ${\sim}3.7\times$---a realistic margin that reflects the genuine
conservatism of the arithmetic bound approach without an implausible
theory--data gap.
Classification correctness is preserved because the relevant guarantee is
the \emph{per-element} DA bound ($\Delta_{\text{logit}} \leq 0.66$ per output
dimension), which remains below the minimum inter-class margin (1.35);
the aggregate bound ($2.95$) is a sum-of-absolute-errors quantity that
over-counts correlated cancellations and is reported only to bracket the
in-domain empirical worst case; the E6 MaxAE (3.65) is dominated by
out-of-domain activations ({\S}\ref{sec:limitations}) and lies outside the
certificate's validity domain.
\end{remark}

\setcounter{lemma}{5}
\begin{lemma}[Adversarial Lower Bound]
...
via E21). This lower bound is covered by the recomputed Theorem~1 DA
bound ($\Delta \leq 0.66$ at the $M_2^{\text{char}}$ calibration),
confirming that the per-function $M_2$ heavy tail is captured by the
certificate only when the worst-function $M_2^{\max} = 1.586$ is used
(bound $0.66 \to 5.9$); a single global $M_2 = 0.177$ applied to all
512 functions---sound for the \textit{average} B-spline
function---underestimates the worst case by design.

\vspace{2pt}
\noindent\textit{Root cause.} The per-function $M_2$ distribution
across 512 B-spline activations is heavy-tailed: mean $0.611$, maximum
$1.592$ (2.6$\times$ the mean, E58), with 11\% of functions exceeding
...
$\sim$0.13--5.9 depending on input alignment, exceeding the
...
$\Delta \leq 0.66$ for typical inputs at the $M_2^{\text{char}}$
calibration while the adversarial worst-case is bounded by
$\Delta \leq 5.9$ using $M_2^{\max}$. The factor of 2.6$\times$
...
$14.17$---outside the Box-Continuation domain: the search drives
layer-1 activations past the $[-3,3]$ LUT grid (47\% out-of-domain at
deployment, E68), where the LUT's extrapolation diverges from the
exact B-spline. In-domain cases are bounded by the certificates above;
out-of-domain cases are the deployment requirement addressed by
per-layer widened LUTs (E68) and the safety monitor (Algorithm~3).
\end{lemma}
...
model of Lemma~
\ref{lem:da_moment}: each weight product $\mathbf{W}^{(\ell)}_{k,j} \cdot
...
concentration term grows only as $O(\sqrt{L \log(1/\delta)})$.
For the trained $[28,16,4]$ KAN, however, the honest checkpoint
measurements are $L_B = 2.21$ (layer-0) / $2.05$ (layer-1) and
$\|W_{\text{eff}}\|_{1,\infty} = 12.36$ (layer-0) / $6.52$ (layer-1),
giving per-layer Lipschitz bounds of $25.1$ / $11.9$ and a measured
per-activation-layer amplification $\gamma = [15.4, 5.3]$ (E68)---well
above $1$, so the trained checkpoint falls in the $\gamma \geq 1$ branch
below: the honest instantiation (Eq.~\ref{eq:expected_error},
$\varepsilon_{\text{char}} = 0.00406$) evaluates to
$\mathbb{E}[\Delta^{(2)}] \approx 1.8$---the expected error itself
exceeds the half-margin $0.675$, quantifying why the $\gamma \geq 1$
regime needs the deployment recipes of {\S}\ref{sec:adaptive-lut} and
E-T9 (the soft-contractive checkpoint evaluates to $\approx 0.22$, an
$8\times$ reduction).
...
same inputs). Under the random-sign model of Lemma~
\ref{lem:da_moment}, the expected
...
$[28,16,4]$ KAN the measured per-activation-layer amplification is
$\gamma = [15.4, 5.3] > 1$ (E68), placing it in the $\gamma \geq 1$
branch: the expected-error term grows with depth rather than converging,
and the earlier numeric instantiation ($\gamma \approx 0.18$,
$\Delta \leq 0.71$, total $\Delta \leq 0.86$) used stale constants;
the honest $\gamma \geq 1$ instantiation gives
$\mathbb{E}[\Delta^{(2)}] \approx 1.8$ (Eq.~\ref{eq:expected_error}).

\vspace{4pt}
\noindent\textit{Practical significance.}
Theorem~4 resolves the depth-scalability concern for industrial KAN
deployment. When contractive training keeps $\gamma = L_B \cdot W_{\max}
< 1$, the expected error converges to a depth-independent constant
$\varepsilon_{\text{LUT}} \cdot d_{\max} / (1-\gamma)$ as $L$ increases.
For the current checkpoint this regime is \emph{not} reached (measured
per-activation-layer amplification $\gamma = [15.4, 5.3] > 1$, E68);
the $\gamma \geq 1$ branch of Theorem~4 still rules out the adversarial
exponential blowup (the random-sign concentration analysis applies
regardless of $\gamma$). This means that \textit{deeper KANs
...
provided per-layer amplification is controlled during training.

\end{theorem}

\noindent\textit{Depth Feasibility Analysis.}
Theorem~4 provides a practical test for depth feasibility---provided the
per-layer amplification factor $\gamma = L_B \cdot W_{\max}$ is known.
For the trained $[28,16,4]$ student the honest checkpoint measurements
($L_B = 2.21$ layer-0 / $2.05$ layer-1;
$\|W_{\text{eff}}\|_{1,\infty} = 12.36$ / $6.52$; measured
per-activation-layer amplification $\gamma = [15.4, 5.3]$, E68) put the
network in the $\gamma \geq 1$ branch: the expected-error term grows
with depth, so the earlier depth-independent feasibility estimates
($\Delta^{(L)} \leq 0.14$--$0.21$) computed with the stale
$\gamma \approx 0.20$ are obsolete; the honest expected-error values are
$1.8$ (released) and $0.22$ (soft-contractive).

\textit{Practical limit.} For a contractively trained KAN
($\gamma = L_B \cdot W_{\max} < 1$), Theorem~4
...
and the concentration term grows only as $O(\sqrt{L})$: even at $L = 12$,
the 95\% confidence bound increases by at most $0.02$ relative to $L = 2$.
In that regime the practical depth limit is determined by the PLC's memory budget
...
32.4~KB parameter arrays) and \texttt{NeuroPLC\_\allowbreak{}Inference} (FB2, 168~lines
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig16_scl_code}
    \caption{Generated SCL excerpt (\texttt{FB\_Inference}): B-spline LUT
    evaluation with binary-search grid lookup. The 15-point adaptive LUT
    compresses each B-spline edge into DB constants; the full listing contains
    168~lines (FB) + 212~lines (DB) and compiles to 45.2~KB work memory on
    S7-1200 (TIA V21 measured, 0 errors).}
    \label{fig:scl_snippet}
\end{figure}

This is, to our knowledge, the first PyTorch-trained model achieving 0-error
...
per-inference latency for KAN\hspace{0pt} $[28,16,4]$ and MLP $[28,32,16,4]$ on both targets.\par\endgroup
...
~~KAN (Table~\ref{tab:instr_timing}) & \textbf{3.2~ms} & --- \\
~~MLP (est.\ $2.7\times$ dynamic ops) & --- & $\sim$8.6~ms \\
\textbf{Dynamic timing (S7-1500)} & & \\
~~Estimated ($\sim$100$\times$ faster FPU) & $\sim$0.03~ms & $\sim$0.09~ms \\
\bottomrule
\end{tabular}
\end{table}

KAN $[28,16,4]$ achieves 3.2~ms on S7-1200---comfortably within the
default 100~ms scan cycle (one inference every $\sim$31 scan cycles), leaving
$>$96\% of cycles for control logic. On S7-1500 ($\sim$100$\times$ faster FPU),
inference completes in $\sim$0.03~ms within a single scan cycle. The dominant
S7-1200 cost is MatMul accumulation (REAL ADD: 42.8\%, REAL MUL: 29.1\%),
followed by DB array access (16.8\%). All estimates use the Siemens
instruction timings for S7-1200 CPU~1211C V4.7~\cite{siemens_s7_1200_2022};
physical hardware may differ.


\vspace{2pt}
\noindent\textit{Scope note.} Table~\ref{tab:instr_timing} (3.2~ms) estimates the full forward pass including DB memory-access overhead (16.8\% of estimated time); the Z3 kernel WCET of Table~\ref{tab:wcet} (2.86~ms) formally certifies the arithmetic core; and the conservative parameterized bound of Theorem~\ref{thm:wcet} (3.86~ms) covers the full pipeline for scan-cycle feasibility. All instruction timings use the same Siemens base (S7-1200 System Manual, 05/2024, Appendix A; \texttt{wcet.py}).
...
The Z3-verified kernel WCET (2,862\,$\mu$s) is roughly an order of
magnitude below the conservative full-pipeline bound of
Theorem~\ref{thm:wcet} (3.86~ms), which additionally accounts for
binary-search loop bodies, branch and DB-access costs, and FB call
overhead that the kernel model omits; the Theorem~\ref{thm:wcet} bound
is the deployment-relevant certificate for scan-cycle feasibility
(Table~\ref{tab:wcet} summarizes the kernel breakdown). Because the S7-1200 executes
...
additivity, combined with the four-tier
...
\textit{within the scan cycle}. We provide, to our knowledge, the first parameterized WCET
...
\end{equation}
where $C_{\text{LUT}}(N_{\text{LUT}}) = 6.2$~$\mu$s per edge for
$N_{\text{LUT}}=15$ (binary search + linear interpolation on REAL array),
$C_{\text{MAC}} = 1.2$~$\mu$s per edge (REAL multiply-accumulate),
$C_{\text{softmax}} \approx 66$~$\mu$s (4-class softmax with soft-float EXP),
and $C_{\text{overhead}} \approx 0.4$~$\mu$s (residual FB overhead).
\end{theorem}

\vspace{2pt}
\noindent\textit{Proof.}
Each B-spline activation in SCL evaluates a piecewise-linear interpolant:
binary-search the grid index $k$ ($4$ REAL comparisons), compute
$t = (x - \text{grid}[k]) / (\text{grid}[k+1] - \text{grid}[k])$
($3$ REAL SUB, $1$ REAL DIV), and interpolate
$y = \text{LUT}[k] + t \cdot (\text{LUT}[k+1] - \text{LUT}[k])$
($2$ REAL SUB, $1$ REAL MUL, $1$ REAL ADD, $4$ array accesses).
S7-1200 instruction timings (Siemens S7-1200 System Manual, 05/2024,
Appendix A; the values used by the Z3-WCET analysis, \texttt{wcet.py}):
REAL ADD 0.50~$\mu$s, REAL MUL 0.60~$\mu$s, REAL DIV 1.20~$\mu$s,
REAL CMP 0.30~$\mu$s, array index 0.10~$\mu$s.
The Z3 analysis's per-edge LUT cost of $6.2$~$\mu$s
(\texttt{wcet\_analysis.json}, regenerated 2026-08-03 on this timing
base) counts the complete generated-SCL instruction stream per
edge---binary-search loop iterations, scalar loads, branches, and
assignments---in addition to the arithmetic operations listed above.
The MatMul per edge contributes
$1.2$~$\mu$s (per-weight multiply-accumulate plus load and loop overhead
in the same Z3 accounting). All branches are
...
\text{WCET} &= 512 \cdot (6.2 + 1.2) + 66 + 0.4 \\
            &= 3{,}788.8 + 66.4
             = 3{,}855.2~\mu\text{s} \approx 3.86~\text{ms}.
\end{align*}

The S7-1200 default scan cycle is 100~ms. The WCET of 3.86~ms
consumes only 3.9\% of the scan cycle, leaving 96.1~ms (96.1\%)
for application logic—a $\mathbf{25.9\times}$ safety margin.
...
serving a distinct role in the verification chain. All instruction
timings use the same Siemens base (S7-1200 System Manual, 05/2024,
Appendix A; \texttt{wcet.py}):
...
\begin{enumerate}[label=(\roman*)]
   ...
   interpolation + matrix multiply), modeled instruction-by-instruction
   on the Siemens timing base. It provides the strongest
   formal evidence but excludes DB variable access and FB call overhead.

    \item \textbf{Instruction-Level Estimate: $3.2$~ms}
    (Table~\ref{tab:instr_timing}). This estimate counts SCL instructions
   for the full forward pass on S7-1200, including DB reads/writes, on
   the same Siemens timing base. It is a practical, data-sheet-based
   estimate, not a formal guarantee; it corroborates the Z3 kernel WCET.

    \item \textbf{Conservative WCET Upper Bound (Theorem~11): $3.86$~ms.}
   ...
   bound of $3.86$~ms provides a safe overestimate with $25.9\times$
...
Theorem~\ref{thm:wcet} provides, to our knowledge, the first formal guarantee that a
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.55\linewidth]{figures/final/fig09_wcet_breakdown}
    \caption{\textbf{WCET Breakdown: KAN $[28,16,4]$ on S7-1200 CPU~1211C.}
    LUT dominates at $82.3\%$ (448+64 edges). MatMul $15.9\%$, Softmax $0.5\%$,
    overhead $0.3\%$. Total: $3.86$~ms ($3.9\%$ of 100~ms scan cycle).}
...
\begin{enumerate}[label=(\roman*)]
   ...
   Execution time $\approx 66$~$\mu$s ($< 1.8\%$ of 3.86~ms WCET),
...
\vspace{4pt}
\noindent\textbf{Runtime Model Updates with Downgrade Guarantees (Algorithm~4).}
Industrial deployment must support model updates---retraining, fine-tuning
(E55), or domain adaptation---without stopping the controller. We extend
the monitor architecture with a \emph{double-buffered hot-swap protocol}
that maintains the certified safety state throughout the update:

\begin{algorithm}[t]
\caption{Runtime LUT Hot-Swap with Downgrade Guarantee (Algorithm~4)}
\label{alg:hot_swap}
\begin{algorithmic}[1]
\Require Active LUT table $T_{\text{old}}$ with certified bound
$\varepsilon_{\text{old}}$ (Tiers~1--3); new LUT table $T_{\text{new}}$;
monitor FB (Algorithm~3); rollback window $W$ (scan cycles)
\Ensure Safety invariant maintained across the update
\State \textbf{Pre-check (offline, compile-side):} run Tier~4 differential
      test on $T_{\text{new}}$ (in-distribution + adversarial); abort
      update if it fails
\State Download $T_{\text{new}}$ to the \emph{standby} DB block
      (no activation; the active DB is untouched)
\State \textbf{Shadow validation ($W$ cycles):} execute inference on
      $T_{\text{new}}$ \emph{in shadow mode}, feeding the monitor's
      existing domain/confidence checks; keep using $T_{\text{old}}$ for
      control decisions
\State \textbf{Atomic switch:} swap the DB pointer to $T_{\text{new}}$ in
      a single scan cycle (no intermediate state)
\State \textbf{Rollback window:} for $W$ cycles, retain $T_{\text{old}}$;
      if the monitor flags any violation \emph{not} attributable to
      out-of-domain input, revert to $T_{\text{old}}$ atomically and set
      \texttt{update\_failed := TRUE}
\State \textbf{Certify:} run the full Tier~2--3 verification on
      $T_{\text{new}}$; on success, discard $T_{\text{old}}$ and clear the
      rollback window
\end{algorithmic}
\end{algorithm}

\begin{theorem}[Downgrade Guarantee for Runtime Updates]
\label{thm:hot_swap_safety}
Under Algorithm~4, the safety invariant of the deployed system---every
control decision is covered by a sound error bound or by the safety
monitor's safe-state fallback---is preserved at every scan cycle across
the update. Specifically:
\begin{enumerate}[label=(\roman*)]
    \item \textbf{Before the switch:} control decisions use $T_{\text{old}}$,
    whose bound $\varepsilon_{\text{old}}$ is certified (Tiers~1--3).
    \item \textbf{During shadow validation:} control decisions still use
    $T_{\text{old}}$; $T_{\text{new}}$ is exercised only for monitoring
    data, and any anomaly raises \texttt{safe\_state\_request} under the
    existing completeness property of the monitor.
    \item \textbf{After the switch, during the rollback window:} either
    (a)~$T_{\text{new}}$ behaves within the monitor's checks---then each
    decision is either within an \emph{uncertified-but-monitored}
    envelope enforced by the monitor's domain/confidence/range checks, or
    (b)~a violation occurs and the system reverts to $T_{\text{old}}$ in
    one cycle, restoring the certified bound $\varepsilon_{\text{old}}$.
    \item \textbf{After certification:} $T_{\text{new}}$ inherits the full
    certificate (Theorem~\ref{thm:svnn-closure}: SVNN composition is
    closed, so re-certification is per-module).
\end{enumerate}
\end{theorem}

\vspace{2pt}
\noindent\textit{Proof sketch.}
The invariant is ``control decisions are covered by a certified bound or
by the monitor fallback''. (i)~holds by the certificate of
$T_{\text{old}}$. (ii)~holds because decisions never use $T_{\text{new}}$
in shadow mode, and the monitor's completeness (Algorithm~3 property (i))
fires on any anomalous $T_{\text{new}}$ output. (iii)~the rollback window
is the safety net: case (a) is enforced by the monitor's soundness
(Algorithm~3 property (ii)) within the validated domain, and its
completeness outside it; case (b) restores $T_{\text{old}}$ atomically,
so at most one scan cycle runs on the new table, and the monitor flags
the violation before any \emph{certified} decision is required of
$T_{\text{new}}$. (iv)~follows from the SVNN closure theorem
(Theorem~\ref{thm:svnn-closure}): re-certification composes module-wise.
$\square$

\vspace{2pt}
\noindent\textbf{Discussion.} Algorithm~4 aligns the deployment pipeline
with ISO/IEC TS~22440's runtime-verification requirements: the monitor
provides continuous runtime checks, the shadow phase provides empirical
validation before exposure, and the rollback window provides the
fail-safe path. The cost is $2\times$ DB memory for the standby table
(45.2~KB $\to$ 90.4~KB, still within the 1.5~MB S7-1500 budget; the
S7-1200's 50~KB budget fits the single-table case with the standby
block in load memory). This is, to our knowledge, the first runtime-update protocol for
certified NN-on-PLC deployment.

% ===========================================================================
% V. EXPERIMENTS
% ===========================================================================
\input{section_iec_universal}

\input{section_noninterference}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig10_confusion_matrices}
...
\label{tab:models}
\footnotesize
\begin{tabular}{@{}p{1.55cm}p{3.0cm}rrr@{}}
\toprule
...
FourierKAN    & Fourier [28,16,4], $K{=}6$, \newline $\omega{=}0.4$ & 6,676 & 8,188 & \textbf{100\%} \\
WaveletKAN    & Wavelet [28,16,4], \newline $S{=}8$ & 4,628 & 5,644 & \textbf{100\%} \\
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig11_tsne_features}
...
type is counted and multiplied by the Siemens instruction timings used
throughout the WCET analysis (S7-1200 System Manual, 05/2024, Appendix A;
\texttt{wcet.py}). The dominant cost is MatMul accumulation
(REAL ADD: 42.8\%, REAL MUL: 29.1\%), followed by DB
array access (16.8\%, 0.10~$\mu$s/index) and REAL division (5.2\%); library
EXP evaluation contributes 3.0\% (2.0~$\mu$s/call---the S7-1200 has no
hardware FPU). The total estimated inference time is \textbf{3.2~ms} per
forward pass, corroborating the Z3-verified kernel WCET of 2.86~ms
(Table~\ref{tab:wcet}); the conservative full-pipeline bound of
Theorem~\ref{thm:wcet} (3.86~ms) remains the deployment-relevant
certificate for scan-cycle feasibility. On the S7-1500 ($\sim$100$\times$ faster
for floating-point operations), the same inference completes in
$\sim$0.03~ms---well within a single scan cycle. The EXP$\to$LUT strength
reduction pass ($\S$\ref{sec:method}) would eliminate the 3.0\% EXP cost,
reducing inference to $\sim$3.1~ms.
...
\midrule
\textbf{Total}       & ---   & \textbf{3,173} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize Timing per Siemens S7-1200 System Manual (05/2024, Appendix A),
the same base used by the Z3-WCET analysis (\texttt{wcet.py}): REAL ADD
0.50~$\mu$s, REAL MUL 0.60~$\mu$s, REAL DIV 1.20~$\mu$s, REAL CMP
0.30~$\mu$s, array index 0.10~$\mu$s, INT arithmetic 0.15~$\mu$s, EXP
2.0~$\mu$s, loop overhead 0.20~$\mu$s/iter.}
\end{table}
...
($\leq 0.66$ per output dimension), which remains below the observed minimum
...
\begin{table}[t]
\centering
\caption{Scalability: Grid Resolution (KAN $[28,16,4]$, S7-1200; DA bounds at
the E11 $M_2^{\text{char}}$ network-level recalibration, anchored at $G{=}15$)}
...
\midrule
4  & 11\,KB & 4,518  & 2,948\,$\mu$s & 14.4  & $<1$ \\
8  & 19\,KB & 4,562  & 3,050\,$\mu$s & 2.64  & $<1$ \\
12 & 27\,KB & 4,606  & 3,151\,$\mu$s & 1.07  & $<1$ \\
15 & 33\,KB & 4,606  & 3,151\,$\mu$s & 0.66  & 1.0$\times$ \\
20 & 43\,KB & 4,650  & 3,252\,$\mu$s & 0.36  & 1.9$\times$ \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize DA bound scales as $\Delta(G) = \Delta(15)\,(h_G/h_{15})^2$ with
$h_G = 6/(G-1)$ (Theorem~1, $\Delta \propto h^2/8$), anchored at the
recalibrated $\Delta(15) = 0.66$ (Table~\ref{tab:compositional}); WCET
scales as $O(\log G)$ (binary search). Only $G \geq 15$ yields a
classification certificate for the released checkpoint---raising $G$ is
the cheap axis of the precision--certificate trade-off ($+10\%$ WCET
from $G{=}15$ to $G{=}20$; soft-contractive training is the orthogonal
axis, E-T9).}
\end{table}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/final/fig12_cross_validation}
...
the DA bound \textit{tightens} after fine-tuning (E68-constant
recomputation, 2026-08-04: DA $0.411 \to 0.165$, safety factor
$4.1\times$; IA $0.664 \to 0.269$, safety $2.5\times$---now a sound
certificate where it was not before), the Z3 per-function
verification remains at 512/512 (100\%, E55), and sign balance is
maintained (L0: 0.080, L1: 0.031, recomputed 2026-08-04). This is because SVNN
...
Let $\mathcal{M}_0$ be a KAN satisfying the SVNN conditions
(Conditions~1--2, Theorem~2) with safety factor
$\mathrm{SF}_0 = \min_k \bigl(\text{margin}_k / \text{bound}_k\bigr) > 1$,
where $\text{margin}_k$ is the inter-class margin of output $k$ and
$\text{bound}_k$ the compiler's DA logit-perturbation bound for that
output ({\S}\ref{sec:da}); we write $\mathrm{SF}$ rather than $\gamma$
to avoid confusion with the Condition~3 contractivity constant. Let
$\mathcal{M}_1$ be obtained from $\mathcal{M}_0$ by fine-tuning with
weight perturbation $\|\Delta W\|_\infty \leq \delta$ per layer. Then
$\mathcal{M}_1$ satisfies the SVNN conditions with safety factor
$\mathrm{SF}_1 \geq \mathrm{SF}_0 - C_{\text{FT}} \cdot \delta$, where
$C_{\text{FT}} = \frac{L \cdot d_{\max} \cdot L_B}{(1-\gamma)^2}$
depends only on the architecture and $\gamma = L_B \cdot
\|W\|_{1,\infty} < 1$ is the per-layer amplification of the
depth-uniform (Condition~3) regime. Consequently, there exists
$\delta_{\max} = (\mathrm{SF}_0 - 1) / C_{\text{FT}}$ such that
fine-tuning with $\|\Delta W\|_\infty \leq \delta_{\max}$ preserves SVNN
certification.
\end{proposition}
...
safety factor $\mathrm{SF} = \min_k (\text{margin}_k / \text{bound}_k)$
therefore changes by at most $C_{\text{FT}} \cdot \delta$ for
architecture-dependent constant $C_{\text{FT}}$.

The trained student $[28,16,4]$ is \emph{not} in the contractive
regime (measured per-layer amplification $\gamma = [15.4, 5.3] > 1$,
E68; $L_B = 2.21$ layer-0 / $2.05$ layer-1,
$\|W_{\text{eff}}\|_{1,\infty} = 12.36$ / $6.52$), so the
$(1-\gamma)^2$ form of $C_{\text{FT}}$ does not apply to this
checkpoint and the earlier instantiation ($C_{\text{FT}} \approx 27.2$
from the stale $\gamma = 0.182$, $L_B = 0.65$) is withdrawn.
What remains directly measured is the fine-tuning story itself: the
observed perturbation $\|\Delta W\|_\infty \approx 0.003$ left the
certificate intact---the DA bound \textit{tightened} after fine-tuning
(DA $0.411 \to 0.165$, safety $4.1\times$, recomputed 2026-08-04)
(i.e., $\mathrm{SF}$ \emph{increased}), consistent with the DA bound
being Lipschitz in the weights with a small architecture constant, and
Z3 verification stayed at 512/512 (E55). $\square$
...
\end{tabular}
\vspace{2pt}
{\scriptsize $^{*}$Per-class zero-shot shows Inner Race retains partial transfer (76.7\%), while Normal collapses completely (0\%). The 29.8\% overall zero-shot accuracy (on the validation split) differs from the 62.5\% full-dataset figure (Table~\ref{tab:xjtu_sy}) due to class-imbalance normalization in stratified splitting. Fine-tuning at 100 epochs recovers all classes to $\geq$80\%, with Normal and Outer Race exceeding 95\%. DA bound: $0.411 \to 0.165$ after FT ($4.1\times$, 2026-08-04); Z3 verified: \textbf{512/512} (post-FT). SCL compilation: 2,188 lines, \textbf{0 errors 0 warnings} (S7-1200). SVNN Condition~3: \textbf{not satisfied} by this checkpoint (measured per-layer amplification $\gamma = [15.4, 5.3] > 1$, E68); Conditions~1--2 \textbf{preserved} (the certificate carried post-fine-tuning is the depth-dependent bound).}
\end{table}
...
yielding a worst-case logit perturbation of 1.38 at 15~LUT points
(recomputed from the released checkpoint, 2026-08-03), which exceeds the
half-margin $0.675$: the IA envelope is sound but is not a classification
certificate for this checkpoint. DA preserves the sign structure of weight
...
\textbf{2.1$\times$ tighter} worst-case logit perturbation (0.66
vs.\ 1.38; margin 1.35, E6). The resulting DA safety factor of $1.0\times$
at 15~points certifies classification with a thin margin; adaptive
allocation (E15, {\S}\ref{sec:adaptive-lut}) lifts this to $3.6\times$,
and soft-contractive training (E-T9) to $8.37\times$ (sound) /
$11.6\times$ (validated).
...
Thm~1 DA (recomputed)     & 15 & DA, $M_2^{\text{char}}$ calibration & 0.66  & $1.0\times$ \\
Thm~1 DA, $M_2^{\max}$    & 15 & DA, worst-function $M_2$           & 5.9   & $<1$ \\
E16 Seg-DA$^{\dag}$       & 15 & DA $+$ Segment-Aware, $M_2^{\max}$ & 1.65  & $0.4\times$ \\
E16 Seg-DA$^{\dag}$       & 20 & DA $+$ Segment-Aware, $M_2^{\max}$ & 0.89  & $0.8\times$ \\
E40 compositional & 15 & DA network-level (Tier~3)                 & 0.66   & $1.0\times$ \\
Adaptive-DA (E15) & 15$^{*}$ & DA, greedy adaptive $\varepsilon$    & 0.19   & $3.6\times$ \\
Adaptive-IA (E15) & 15$^{*}$ & IA, greedy adaptive $\varepsilon$    & 0.39   & $1.7\times$ \\
Soft-contractive expected (E-T9) & 15 & DA, $M_2^{\text{char}}$ & 0.00137 & $493\times^{\ddag}$ \\
Soft-contractive worst (E-T9) & 15 & DA, $M_2^{\max}$ & 0.039 & $17.5\times^{\natural}$ \\
Soft-contractive sound (E-T9) & 15 & first-principles propag. & 0.081 & $8.37\times$ \\
...
\vspace{2pt}
{\scriptsize
\textbf{IA bound} (1.38, recomputed): sound upper envelope for in-domain
$x$, uses $\sum|W_{k,j}W_{j,i}|$; exceeds the half-margin $0.675$, so it
is not a classification certificate for this checkpoint
(\texttt{verify\_da\_bounds\_recomputed.py}, 2026-08-03).
All released-student rows use the E11 $M_2^{\text{char}} = 0.177$
calibration (500-pt float32); the full-$\varphi$ float64 recalibration
(2026-08-04, \texttt{verify\_sound\_chain.py}) gives expected/worst
envelopes 1.70/2.29 (safety $0.4\times$/$0.3\times$)---no \emph{sound}
certificate exists for this checkpoint, and its measured maxAE ($0.52$)
is a lucky $4.4\times$ below the theoretical envelope.
\textbf{DA bound} (0.66): \emph{expected-tier} (high-probability,
random-sign, Lemma~\ref{lem:da_moment}), uses $|\sum W_{k,j}W_{j,i}|$;
certifies classification in the expected sense at the
$M_2^{\text{char}}$ calibration with thin margin ($1.0\times$; see the
adaptive and soft-contractive rows below). \textbf{Validated rows}
(0.058/0.026/0.138): measured-floor bounds ($\times 1.1$ margin over the
full-set Tier-4 maxAE on 13{,}714 inputs); they certify those inputs
empirically---the first-principles envelopes are the \emph{sound}
numbers (0.081/8.37$\times$; WaveletKAN 0.237 first-principles covers
its measured 0.157, so no floor is needed there).
\textbf{DA, $M_2^{\max}$} (5.9): worst-function $M_2=1.586$ replaces the
characteristic value---conservative, exceeds the half-margin.
$^{\natural}$\textbf{Soft-contractive worst} (0.039/$17.5\times$):
worst-function ($M_2^{\max}$) envelope, per-function independent---the
float32 validated bound is $\max(0.039, 0.053\times1.1) = 0.058$.
$^{\dag}$\textbf{Seg-DA} (1.65/0.89): per-input segment errors under the
worst-function $M_2^{\max}$ calibration (global DA $5.9$): a
$3.6\times$/$6.6\times$ tightening (recomputed 2026-08-04,
\texttt{verify\_seg\_da.py}); the earlier $0.056/11.9\times$ mixed
$M_2$ calibrations and is superseded. \textbf{Compositional DA} matches
the recomputed network-level DA ($0.66$, 2026-08-04; cf.\
Table~\ref{tab:compositional}).
\textbf{MaxAE} (3.65): empirical maximum absolute logit error across
2,743 test samples---includes out-of-domain activations (E68), not a
certified bound; the in-domain Tier-4 full-set maxAE is $0.52$
(E52, 2026-08-04). In-domain certified values are the recomputed rows
above; the empirical MaxAE reflects worst-sample behavior without the
aggregation-safe assumption of independent error signs.
\textbf{Adaptive rows} (0.19/0.39): greedy allocation (E15,
Algorithm~\ref{alg:adaptive_lut}) at the $N{=}15$ storage budget,
$\varepsilon_{\max} = 0.00115$; recomputed 2026-08-04
(\texttt{verify\_adaptive\_da.py}).
\textbf{Soft-contractive rows}: E-T9 checkpoint ($\gamma =
[1.03, 1.38]$, 98.5\% acc), full-$\varphi$ float64 methodology,
2026-08-04 (\texttt{verify\_sound\_chain.py}).
$^{\ddag}$Expected tier (high-probability, Lemma~\ref{lem:da_moment}),
$M_2^{\text{char}} = 0.051$ on this checkpoint. The \textbf{worst row}
(0.026) is the float64 deployment certificate (per-function
no-cancellation propagation floored by the float64 simulator maxAE
$0.023$; theoretical envelope $0.0078$); the \textbf{sound row}
(0.058) is the float32 deployment certificate (measured floor: maxAE
$0.053$, 13{,}714 inputs, Tier-4 simulator). Released student:
per-function envelope $2.29$ (safety $0.3\times$, not a certificate),
expected $1.70$ ($0.4\times$).}
\end{table}
...
$\varepsilon_i$ values, the DA bound tightens under the heavy-tail
regime (recomputed 2026-08-04): with the worst-function $M_2^{\max}$
calibration (global bound $5.9$), the segment-aware DA bound is $1.65$
at $N=15$ and $0.89$ at $N=20$ ($3.6\times$/$6.6\times$). This provides
...
show a DA safety factor of $1.0\times$ at 15 LUT points at the
$M_2^{\text{char}}$ calibration (empirical refinement;
analytical bound: ${\sim}72\times$ using the B-spline derivative bound
$M_2^{\text{analytic}} \leq 12.8$ vs.\ the empirical
$M_2^{\text{char}} = 0.177$ (Eq.~\ref{eq:bspline_curve_bounds}).
Extending to
...
This empirically validates Theorem~1's core claim: the recomputed DA bound
($\Delta_{\text{logit}} \leq 0.66$ at $N{=}15$) keeps the certified logit
perturbation below the minimum inter-class margin ($2 \times 0.66 = 1.32 <
1.35$), and the resulting safety factor of $\sim$1.0$\times$ at the
$M_2^{\text{char}}$ calibration is not a theoretical artifact---it is an
...
\end{tabular}
\vspace{2pt}
{\scriptsize Theorem~1 bound: $\Delta_{\text{logit}} \leq 0.66$ (DA, $N{=}15$,
$M_2^{\text{char}}$ calibration).
Safety factor: $1.0\times$. The 10 full-set mismatches occur only at near-tied
...
\midrule
4  & 99.93\% & 14.36 & 0.05$\times$ & \textbf{YES} & 225/20,000 \\
5  & 99.93\% & 8.08  & 0.08$\times$ & \textbf{YES} & 122/20,000 \\
6  & 99.93\% & 5.17  & 0.13$\times$ & \textbf{YES} & 80/20,000 \\
7  & 99.93\% & 3.59  & 0.19$\times$ & \textbf{YES} & 75/20,000 \\
8  & 99.93\% & 2.64  & 0.26$\times$ & \textbf{YES} & 64/20,000 \\
10 & 99.93\% & 1.60  & 0.42$\times$ & \textbf{YES} & 54/20,000 \\
12 & 99.93\% & 1.07  & 0.63$\times$ & \textbf{YES} & 43/20,000 \\
15 & 99.93\% & 0.66  & 1.0$\times$ & No$^{\dag}$ & 37/20,000 \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize Safety factor = $(m/2)/\Delta_{\text{DA}}$ with $m=1.35$ (E6)
and the DA amplification recomputed from the released checkpoint
($L_{\text{net}}^{\text{DA}} = 162.2$, 2026-08-03;
\texttt{verify\_da\_bounds\_recomputed.py}).
Safety $<1$ means the certified error exceeds half the inter-class margin---the
Lemma~\ref{lem:per_op}(vi) threshold for guaranteed classification preservation.
Blind spot: the certified safety factor remains below $1$ through
$N = 12$; $^{\dag}$at $N = 15$ (the deployment default) it closes only
marginally ($1.02\times$). The adversarial-flip counts (measured, not
certified) are unchanged; all flips occur strictly inside $[-3,3]^{28}$.}
\end{table}
...
M_2^{\max}$). At $N=4$, the per-activation LUT error bound
($M_2^{\text{char}}$-calibrated) propagates through the recomputed KAN
amplification to a worst-case logit perturbation of 14.4, far above the
certification threshold of 0.675. An adversarial search confirms this is not
merely a loose bound: 225 of 20,000 random in-domain inputs cause actual
misclassification. Critically, the certified safety factor remains below $1$
through $N = 12$ and closes only marginally at $N=15$ ($1.02\times$): with the
released checkpoint, the certification margin of the deployment default is
thin---empirical accuracy alone (99.93\%) is a severe under-estimator of the
certified blind spot. Adaptive LUT allocation (E15,
{\S}\ref{sec:adaptive-lut}) restores a comfortable margin at unchanged
storage: the worst-case per-function $\varepsilon$ drops to $0.00115$, the
DA bound to $0.19$ (safety $3.6\times$), eliminating the blind spot at the
deployment budget.
...
factor of $2.0\times$ (twice the Lemma~\ref{lem:per_op}(vi) threshold) as a hard deployment
...
mean $0.607$, maximum $1.586$ (Lemma~6, {\S}\ref{sec:compiler-correctness}),
...
adversarial worst-case uses $M_2^{\max}=1.586$ (Lemma~6). The analytical
...
\begin{table*}[tb]
\centering
\caption{Results Summary: Core Experiments (E1--E20, E52--E68), Capacity-Theory Verification (E-T1--E-T9), and Validation Experiments (V1--V7)}
\label{tab:summary}
\scriptsize
\setlength{\tabcolsep}{4pt}
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{@{}p{0.7cm}p{4.6cm}p{3.6cm}p{4.9cm}@{}}
\toprule
...
E9 & Doubleton Arith.      & DA vs IA bound & \textbf{2.1$\times$ tighter} \\
...
E16& Segment-Aware DA       & Seg-DA vs DA       & \textbf{3.6$\times$} vs.\ $M_2^{\max}$ (recomp.\ 2026-08-04) \\
...
E56& Deep KAN ($L{=}3$)       & [28,16,8,4] 120ep & \textbf{99.60\%}, 608/608 Z3, DA 5.66, sub-exp. depth; E-T9-3L: \textbf{98.6\%}, sound $2.4\times$ (Box-in 81\%) + validated $6.1\times$ \\
E57& CROWN vs.\ NeuroPLC     & KAN[28,16,4] DA/IBP & \textbf{DA substantially tighter}, CROWN=IBP for KAN \\
E58& Per-Func.\ M$_2$ / Z3    & M$_2$ proxy, Z3 NRA & mean 0.017, max 0.041; Z3 512/512 \\
E60& FourierKAN SVNN           & [28,16,4] $K{=}6$, CWRU & \textbf{100\% acc}, 512/512 Z3, 2184-line SCL; E-T10: validated $4.9\times$ + sound $5.2\times$ (L0-direct) \\
E61& WaveletKAN SVNN           & [28,16,4] $S{=}8$, M2-reg & \textbf{100\% acc}, 512/512 Z3, 2184-line SCL \\
E62& XJTU-SY Cross-Arch.       & FT Fourier/WaveletKAN & \textbf{100\% acc}, 512/512 Z3 preserved \\
E53& Sound In-Domain Worst-Case & Real LUT, in-domain search & IA-sound certifies $N{\ge}20$; high-prob.\ DA at all $N$ \\
E63& Sharp k-th Order LUT Bounds & $k{=}2..5$, extremal family & \textbf{extremal ratio $=$ 1.000000}; soundness $\leq 1$ \\
E64& Moment-Robust DA Ratio    & Bennett, 4 weight families & $R$: 4.6/9.5/18.8/37.7 at $d{=}16..1024$; outlier $\kappa{=}\sqrt{d}$: $R{=}1$ \\
E65& $\varepsilon$-Separation   & 3-SAT product-gate encoding & 200/200 both directions; spline exact $5{\times}10^{-8}$ \\
E66& Verifiability Trichotomy  & Rates: tanh/sin/B-spline/ReLU & slopes $-2.04/-2.06/-1.77/-1.03$; tensor LUT $N^m$ \\
E67& Tier-4 Differential Test  & PyTorch vs SCL-sim (emitter) & 3{,}357 samples 100\% agree; maxAE 0.47; \textbf{caught scale regression} \\
E68& Compile-Aware PAC         & Decomp.\ + resolution matching & bias slope 0.99 vs $h^2$; per-act.\ ratio 0.996--1.000; knee $N^{*}$ exp.\ 0.14 \\
\midrule
\multicolumn{4}{l}{\textit{Capacity-Theory Verification (E-T1--E-T11):}} \\
E-T1 & Curvature-Aware LUT Optimality & Worst-$\varepsilon$ vs uniform & \textbf{2.75$\times$} cross-func. \\
E-T2 & Compile-Aware Minimax Rate & Deployment risk vs width & Matches $W^2_\infty$ minimax \\
E-T3 & Besov Deployment PAC & Deployment risk & PAC rate ($s$-smooth) \\
E-T4 & Packing-Line Constant Matching & KT extremal ratio & \textbf{1.000000} ($N{=}8..64$) \\
E-T5 & Three-Layer Separation & Slope: kink vs $C^2$ & $-1.11$ vs $-2.05$ \\
E-T6 & Architecture Selection Law & Spectral-vs-LUT crossover & $\rho^*$ Lambert-W closed form \\
E-T7 & Packing Constant $c_{\text{ent}}$ & Constructive lower bound & $c_{\text{ent}} \approx 0.079$ \\
E-T8 & Width Constant $c_*$ & Numerical estimate & $c_* \lesssim 0.082$; gap $\geq 1.53\times$ \\
E-T9 & Soft-Contractive Training & $\gamma$ post-training & $[1.03,1.38]$; 98.5\%; sound cert.\ $11.6\times$ \\
E-T10 & FourierKAN Contr.\ Cert. & Bounded-amp.\ basis & sound $0.138$ ($4.9\times$); \textbf{99.96\%} \\
E-T11 & WaveletKAN Contr.\ Cert. & Bounded-amp.\ basis & sound $0.237$ ($2.8\times$); \textbf{99.67\%} \\
\midrule
...
V4 & MLP Verification Gap      & KAN $[28,16,4]$ vs.\ MLP $[28,32,16,4]$ & 512/512 vs.\ 0/48; $4.6\times$ worse (E2E) \\
...
Crucially, the adaptive worst-case $\varepsilon$ propagates linearly
through the DA/IA bounds (Eq.~\ref{eq:da_bound}): at the $N{=}15$ budget
the DA bound drops from $0.66$ to $0.19$ (safety $1.0\times \to
3.6\times$) and the IA bound from $1.38$ to $0.39$---the first sound IA
certificate at $N{=}15$ (Table~\ref{tab:da_bounds_summary}, recomputed
2026-08-04, \texttt{verify\_adaptive\_da.py}).
...
$\varepsilon_i$ values (recomputed 2026-08-04) tightens the bound under the
heavy-tail regime: with the worst-function $M_2^{\max}$ calibration
(global bound $5.9$), segment-aware DA reduces it to $1.65$ at $N=15$ and
$0.89$ at $N=20$---$3.6\times$/$6.6\times$ tightening. (The earlier
$\sim 11.9\times$ figure mixed $M_2$ calibrations and is superseded;
sound recomputation in \texttt{verify\_seg\_da.py}.)
Fig.~\ref{fig:segment_bounds} already reported the per-resolution breakdown;
the $6.0\times$ per-segment tightening and the DA compositional
...
provides the strongest correctness bound for compiled neural
...
The per-feature analysis reveals an actionable insight for deployment
...
2.1$\times$ due to sign structure developed through optimization.
...
and 0/48 for an identically-sized MLP. This $4.6\times$ end-to-end gap
in error propagation (recomputed from $\Delta_{\text{DA}} = 0.66$,
2026-08-03) is not an implementation
artifact---it is a structural consequence of the SVNN conditions
(Theorem~2, Proposition~\ref{prop:mlp_negative}, Theorem~5).
...
cancellation to achieve a $2.1\times$ tighter bound than interval arithmetic,
while empirical calibration ($M_2^{\text{char}} = 0.177$) tightens the de~Boor
LUT bound by a further $98.6\%$ vs.\ the analytical $M_2$ (E11).
Segment-aware per-segment evaluation composes with DA to contain the
heavy-tail regime (safety $3.6\times$/$6.6\times$ at $N{=}15/20$ vs.\ the
worst-function calibration, E16, recomputed 2026-08-04).
...
error growth; absent Condition~3, the bound remains finite but grows
with depth. We validate the depth-scaling behavior experimentally with a
3-layer KAN[28,16,8,4]
trained from scratch on CWRU (120 epochs, 99.60\% test accuracy, 7,302 parameters).
The result is consistent with the depth-dependent (non-convergent) form:
\begin{itemize}
    ...
    \item \textbf{DA error bound:} the 3-layer DA chain measures
          $5.66$ (E68-constant recomputation, 2026-08-04; the earlier
          $1.90$ used the pre-audit $L_B = 0.65$)---below the full
          per-layer amplification envelope $\gamma^{L-1} = 15.4^2 = 237$
          that would apply at $L = 3$ under worst-case amplification,
          consistent with \emph{sub-exponential} depth scaling, not the
          exponential blowup expected for non-SVNN architectures (where
          $O(2^L)$ growth arises from wrapping effects).
          \textbf{Soft-contractive depth extension (E-T9-3L, 2026-08-04):}
          training the 3-layer KAN with the same $\gamma$-projection recipe
          yields $\gamma = [0.99, 1.03, 1.02]$, $98.6\%$ test accuracy, and a
          \emph{sound} certificate of $0.284$ (safety $2.4\times$;
          signal-domain propagation, Box-in $81.4\%$; the output-layer
          per-edge Lipschitz is $0.29$ on its actual signal domain vs.\
          $4.69$ on the full design domain; 2026-08-05), plus the
          validated $0.110$ ($6.1\times$, full-set measured floor) and
          per-function float64 bound $0.019$ (safety $36\times$)---depth scaling
          \emph{with} a certificate, the $\gamma \to 1$ branch of
          Theorem~\ref{thm:generalization} instantiated at $L = 3$
          (per-layer bound growth $1.9\times$ vs.\ $8.6\times$ for the
          untrained 3-layer).
    \item \textbf{Condition~3 not certified:} the earlier $\gamma_0 = 0.404$ /
         $\gamma_1 = 0.318$ figures used a crossed-index max-abs-weight
         proxy, which is \emph{not} a valid contraction test; the honest
         per-layer amplification of the 2-layer student is
         $\gamma = [15.4, 5.3] > 1$ (E68), and contractivity was never
         established for the 3-layer network either.
...
\end{itemize}
The empirical depth-scaling ratio ($8.6\times$ for the single $2\to 3$
depth step, from the E68-constant bounds $5.66/0.66$) is substantially
below the $O(2^L)$
worst-case for MLP-style wrapping effects, directly supporting
Theorem~4's depth-scaling analysis (its $\gamma \geq 1$ branch applies
to the trained networks, which are not contractive). The 3-layer
KAN's 99.60\% accuracy (vs.\ 2-layer's 99.93\%) confirms that depth
scaling is architecturally viable for industrial accuracy requirements.
(Note: the compile-aware generalization bound
(Theorem~\ref{thm:generalization}) does not claim depth-monotonicity of
the gap---its depth threshold is quantified in
Corollary~\ref{cor:depth_threshold}.)
...
\textbf{NeuroPLC DA ($\Delta = 0.66$, safety factor $1.0\times$) is
substantially tighter than CROWN-IBP}: the CROWN-IBP bound evaluates to
$9.97$ on the released checkpoint (recomputed 2026-08-04)---a $15.1\times$
gap. (The earlier $8.3\times$ figure used a pre-recomputation baseline
and is withdrawn.)
...
characterization). The $15.1\times$ gap between DA and CROWN-IBP quantifies the
...
(cannot certify classification correctness) versus DA's $1.0\times$ safety
factor (certifies: margin $1.35 > 2\Delta_{\text{DA}} = 1.32$) makes this distinction operationally
...
what the four-tier verification system exploits.
(Table~\ref{tab:compositional} summarizes the full compositional results.)

\vspace{4pt}
\noindent\textbf{Four-Tier Architecture.}
The key insight is that whole-program verification can be decomposed into
four independent tiers, each with a distinct role and verification scope:
\begin{enumerate}
    ...
    \item \textbf{Tier~4---Differential Self-Test (Every Compilation):}
         Before deployment, the compiler runs a differential test comparing
         PyTorch FP32 logits against a faithful simulation of the generated
         SCL semantics (topological-order DAG execution with the actual
         emitted LUT tables and folded weights) on in-distribution features
         and adversarial worst-case inputs. The test asserts 100\%
         classification agreement and per-logit error below
         $\frac{1}{2} \times$ the certified margin (E6: 1.35). Tier~4
         catches code-generation regressions that the three proof tiers
         cannot: it validates the \emph{emitter}, not just the templates.
         In the 2026-08-01 audit, Tier~4-style testing caught a real
         regression---the base/spline scale factors were stored in the IR
         but dropped by the SCL emitter, causing 83\% classification
         agreement on generated code---which no Tier~1--3 proof detected
         (the templates and bounds were individually sound; the emitter
         was not). After the fix, Tier~4 passes: 3{,}000 in-distribution
         samples, 100\% agreement, max logit error 0.47 (E67).

         \textit{Chronology of the fix.} The scale-factor regression
         predates the July experiments and was fixed on 2026-08-03; the
         SCL bundle shipped with this paper was regenerated after the
         fix (results/scl\_output, 2026-08-03), and all reported
         quantities that exercise emitted-SCL arithmetic reflect the
         post-fix emitter: E67 was rerun post-fix (2026-08-03: 3{,}357
         samples, 100\% agreement, maxAE 0.47) and E68 was validated on
         the post-fix bundle; the certificate bundle corresponds to the
         released checkpoint. The worst-case LUT sweeps (E21, E53) and
         per-function Z3 verifiability (E58) measure the LUT tables and
         template proofs rather than the emitter's arithmetic and are
         unaffected by the regression; where an SCL artifact is
         involved, the 2026-08-03 regeneration is authoritative.

        \textit{Formal verification boundary.} To be precise about what
        ``verified compiler'' means: the \emph{formal} proofs cover
        (i)~the IR-to-SCL code \emph{templates} (Tier~1, symbolic Z3
        over all inputs), (ii)~the per-function LUT \emph{certificates}
        (Tier~2) and (iii)~their \emph{composition} (Tier~3,
        machine-checkable). The \emph{emitter}---the code generator that
        instantiates templates with concrete weights, tables, and
        addressing---is \emph{not} covered by a formal proof; it is
        validated by the differential self-test (Tier~4), which is an
        empirical guarantee over the emitted artifact (3{,}357 samples,
        E67), not a proof. This boundary is standard in verified
        compilation practice (CompCert-style certified generators are
        the stronger alternative~\cite{leroy2009compcert}); we state it
        explicitly so that ``the first verified PyTorch-to-IEC~61131-3
        compiler'' is read as: templates, certificates, and composition
        formally verified; the emitter validated by differential
        self-testing.
\end{enumerate}
...
confirming the per-function error bound $\varepsilon = 0.0036$ (the E40
verification query; the $M_2^{\text{char}}$ audit value is $\varepsilon =
0.00406$, Table~\ref{tab:compositional}).
Average verification time is $285$\,ms per function (total 146\,s).
This is, to our knowledge, the first formal, machine-checkable proof that \textit{every}
...
\midrule
 Per-function LUT error $\varepsilon$        & $0.00406$ \\
 IA network bound $\Delta_{\text{IA}}$       & $1.38$ \\
 DA network bound $\Delta_{\text{DA}}$       & $0.66$ \\
...
The four-tier architecture achieves whole-program formal verification
...
Z3-verifiability, with DA error bound $\Delta_{\text{DA}} = 0.66$ (model-specific,
see Table~\ref{tab:da_bounds_summary}), IA bound $\Delta_{\text{IA}} = 1.38$,
and DA/IA tightening of \textbf{2.1$\times$} (end-to-end).
...
\textbf{4.6$\times$ worse} error propagation than KAN's DA bound
(end-to-end, using KAN's network-level $\Delta_{\text{DA}}{=}0.66$;
recomputed 2026-08-03) and
\textbf{2.2$\times$ worse} than KAN's IA bound ($\Delta_{\text{IA}} = 1.38$),
for the same per-component error magnitude $\varepsilon = 0.0041$.
...
tightening. The MLP error bound is $4.6\times$ worse than KAN's DA bound
(end-to-end, using the network-level DA; recomputed 2026-08-03) for identical architecture dimensions.
\vspace{4pt}
\noindent\textbf{Quantitative Error Propagation.}
Table~\ref{tab:verify_gap_propagation} compares the error propagation
bounds for the same per-component error $\varepsilon = 0.0041$.
...
\midrule
 ...
 KAN DA bound $\Delta_{\text{DA}}$           & $0.66$ \\
 KAN IA bound $\Delta_{\text{IA}}$           & $1.38$ \\
...
quantitative (MLP propagation is $4.6\times$ looser via DA end-to-end; $2.2\times$ looser via IA)
...
\toprule
\textbf{Metric} & \textbf{MNIST} & \textbf{CWRU} & \textbf{XJTU-SY} & \textbf{ChebyKAN} & \textbf{FourierKAN} & \textbf{WaveletKAN} \\
 & \textbf{(Image)} & \textbf{(Vib.)} & \textbf{(Nat.Wear)} & \textbf{(Poly.)} & \textbf{(E-T10)} & \textbf{(E-T11)} \\
\midrule
 Accuracy & 0.9859$^{\sharp}$ & 0.9993 & 0.9172$^{*}$ & 1.0000 & \textbf{0.9996} & \textbf{0.9967} \\
 Training & Scratch & KD (VRM) & FT 100ep & Scratch & C2-BV contr.\ FT$^{\S}$ & C2-BV contr.\ FT$^{\S}$ \\
\midrule
 Per-func.\ verified & 512/512 & 512/512 & 512/512 & 496/512 & \textbf{512/512} & \textbf{512/512} \\
 M$_2$ max & 0.83$^{\P}$ & 1.586 & 27.1$^{\P}$ & 5.4$\times$ M$_2$ & 1.324 & 4.312 \\
 Safety margin & 5.6$\times^{\P\P}$ & 1.0$\times^{**}$ & 13.7$\times^{\P\P\P}$ & 1.1$\times$ & \textbf{9.6$\times^{\S\S}$} & \textbf{4.0$\times^{\S\S}$} \\
 SVNN Cond.~3 & No & No & No & No & \textbf{Yes} & \textbf{Yes} \\
 SCL compiled & 0e 0w & 0e 0w & 0e 0w & 0e 0w$^{\ddag}$ & \textbf{0e 0w} & \textbf{0e 0w} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize
$^{\sharp}$MNIST digits 0--3, PCA(784$\to$28) + z-score, scratch training, 512/512 verified, DA~0.1199 (E42).
$^{*}$Fine-tuned 100 epochs from CWRU checkpoint (E55).
$^{\S}$Bounded-amplitude basis with soft-contractive projection (E-T10/E-T11).
$^{**}$CWRU column is the released checkpoint: \emph{expected-tier} 1.0$\times$ (E11 $M_2^{\text{char}}$ calibration); no sound certificate ($\gamma=[15.4,5.3]>1$, E68).
$^{\S\S}$E-T10/E-T11 certificates (2026-08-05 v4/v2): FourierKAN $\gamma=[0.82, 0.95]$ (post-hoc projection, Cond.~3 satisfied, 99.96\% acc preserved) L0-direct sound 0.070 (safety $9.6\times$, N=16 L1 LUT, Box 100\%, covers measured 0.049); full-LUT N=15 validated floor 0.138 (4.9$\times$) retained; WaveletKAN $\gamma=[0.92, 0.95]$ (Cond.~3 satisfied, 99.75\%) sound 0.168 (safety $4.0\times$, covers measured 0.157 family).
$^{\ddag}$ChebyKAN uses Markov-based global $M_2$ bounds, 5.4$\times$ looser than empirical (E54).
$^{\P}$MNIST/XJTU $M_2$ max measured full-$\varphi$ float64 (40{,}001 points, 2026-08-05): MNIST 0.83 (scratch-trained, E42); XJTU-SY 27.1 after 100-epoch fine-tuning (E55), consistent with its $\gamma>1$ and expected-tier-only status; the CWRU column retains the existing folded-weight convention (1.586, E11).
$^{\P\P}$MNIST safety = $0.675/\Delta_{\text{DA}} = 5.6\times$ (expected-tier, E42 DA~0.1199, 512/512 Z3; no sound certificate---$\gamma=[6.3,3.2]>1$).
$^{\P\P\P}$XJTU-SY safety = $0.675/0.0493 = 13.7\times$ (sign-balance DA after 100-epoch FT, E55 script convention; expected-tier, no sound certificate---$\gamma>1$ under the E68 measurement; the DA convention here differs from the E11 $M_2^{\text{char}}$ convention used for the CWRU column).
...
SVNN Cond.~3 (per-layer $\gamma<1$) is a \emph{training} property, not a pipeline property: unsatisfied by the released checkpoint and by E-T10/E-T11 (second-layer $\gamma>1$); measured on MNIST (E42, $\gamma=[6.3,3.2]$) and ChebyKAN (E54, $\gamma=[1.2,2.1]$) on 2026-08-05 (cond3\_complement.json). This table documents \emph{pipeline} generality (compiler/verification domain-independence); contraction is established by the soft-contractive recipe (E-T9--E-T11).}
\end{table*}
...
\textbf{Type theory + 17 Thm + DA + Z3} \\
WCET / Safety & \ding{55} & \ding{55} &
\textbf{WCET 3.86ms + Non-Interference (Thm~E)} \\
IEC~61508 Aligned & \ding{55} & \ding{55} &
\textbf{\ding{51} (Non-Interference + WCET)} \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize NeuroPLC provides the only design-time correctness guarantee (17 theorems),
...
Formal cert. & \xmark & \xmark & \xmark & \xmark & \textbf{17 Thm+Z3} \\
...
\end{tabular}
\vspace{2pt}
{\scriptsize NeuroPLC is the only platform providing: formal certification (17 theorems),
...
(1)~\textbf{type-theoretic foundation}---17 theorems providing, to our knowledge, the first formal
...
(4)~\textbf{deployment safety}---WCET guarantee (Theorem~11) and
...
process, producing 2,188 lines of SCL that compiles with 0 errors and 0
...
As Sz\'{a}sz et~al.~\cite{szasz2025floating} (ICML 2025 Spotlight)
...
correctness for all possible hardware inputs. The composition
...
with sign-balance evidence (SIL~2, $N{=}15$, safety $1.0\times$), and a
\textbf{sound} tier using global IA with no sign-cancellation assumption
(SIL~2 at $N{\geq}22$, SIL~3 at $N{\geq}25$; segment-aware IA composes
further per the recomputed Seg-DA analysis, {\S}\ref{sec:segment-aware}).
For SIL~3+, Level~2 arithmetic
...
\footnotesize
\begin{tabular}{@{}lcccc>{\raggedright\arraybackslash}p{2.1in}@{}}
\toprule
\textbf{Guarantee} & \textbf{Method} & \textbf{$N_{\min}$} &
\textbf{Safety} & \textbf{SIL Target} & \textbf{Evidence} \\
\midrule
High-probability & DA & 15 & $1.0\times$ & SIL~2 & Sign-balance (Lemma~
\ref{lem:da_moment}); recomputed $\Delta_{\text{DA}} = 0.66$ \\
Sound            & IA (global) & 22 & $1.1\times$ & SIL~2 & No sign
assumptions; $\Delta_{\text{IA}}(N) = 1.38\,(14/(N{-}1))^2$ (recomputed
2026-08-03: $N{=}15$ is \emph{not} a certificate) \\
Sound (adaptive) & IA (greedy) & 15$^{*}$ & $1.7\times$ & SIL~2 & Adaptive
$\varepsilon_{\max}{=}0.00115$ (E15); recomputed 2026-08-04: $N{=}15$
\emph{is} a certificate under adaptive allocation \\
Soft-contractive & Validated IA & 15 & $11.6\times$ & SIL~3+ & E-T9 checkpoint
(98.5\%; $\gamma=[0.95,0.95]$ pervasive, per-edge-$L$ recipe
2026-08-06); Box-Continuation coverage 99.9\%; Z3 512/512;
expected tier $493\times$; sound tier $8.37\times$ \\
Soft-contractive 3L & Signal-domain IA & 15 & $2.4\times$ & SIL~2 & E-T9-3L
(98.6\%); output-layer per-edge Lipschitz $0.29$ on its signal domain
vs.\ $4.69$ full-domain (2026-08-05); Box-in $81.4\%$; validated $6.1\times$ \\
Sound + margin   & IA (global) & 25 & $1.4\times$ & SIL~3 &
safety factor $0.675/0.470 = 1.44\times$ \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize $N_{\min}$ = minimum LUT points for safety factor $\geq 1.0$.
$^{*}$Adaptive allocation (E15) at the $N{=}15$ storage budget.
$M_2^{\text{char}} = 0.177$ (E11); half-margin $= 0.675$; all values
recomputed from the released checkpoint (2026-08-03) for KAN
$[28,16,4]$ on S7-1200 CPU~1211C. Segment-aware rows follow the
recomputed Seg-DA analysis ({\S}\ref{sec:segment-aware}, 2026-08-04).}
\end{table}
...
architectures}---a formal characterization of which architectures admit
design-time correctness certification under LUT-discretized compilation, with
what sharp constants, at what computational cost, and with what
learning-theoretic guarantee---and instantiated it as the first
self-verifying PyTorch-to-IEC~61131-3 compiler (formally verified
templates, certificates, and composition rules; emitter validated by
differential self-testing). The central finding is that compiler
correctness certification is not an engineering problem---it is an
\textit{architectural} problem, governed by a precise mathematical
frontier.
...
\begin{enumerate}[label=\arabic*.]
    \item \textbf{Verifiable Deployment Capacity
    (Theorem~\ref{thm:capacity-stratification}).} The unifying
    framework: a universal packing line bounds every certified
    deployment scheme; three verification strata place every
    architecture; a certificate proof system formalizes the
    verification budget under perfect deterministic soundness; we prove
    a theorem-level information-theoretic necessity within the calculus
    and derive an architecture-as-code selection law. The sharp
    constants, minimax rates, separation, and trichotomy of the items
    below are its expansion ({\S}\ref{sec:capacity}).

    \item \textbf{Sharp k-th Order LUT Bounds (Theorem~\ref{thm:sharp-lut}).}
   The de~Boor constant $M_2h^2/8$ is the $k=2$ instance of an exact
   all-orders family $c_k = Q_k/k!$, attained by explicit extremal
   functions at machine precision---DA optimality is a theorem with a
   certificate, not an empirical observation.

    \item \textbf{Verifiability Trichotomy (Theorem~\ref{thm:trichotomy}).}
   The SVNN condition is \emph{conjecturally} the unique regime where
   second-order rate, linear LUT cost, closed-form sharp constants, and
   reachability-freedom coexist; ReLU admits only first-order bounds,
   multivariate gates suffer $N^m$ cost and NP-hard tightness, and
   domain-dependent curvature requires reachability. The necessity
   direction---that no other regime attains all four properties---is
   stated as a conjecture with strong evidence; within the
   affine-certificate class it \emph{is} a theorem
   (Theorem~\ref{thm:necessity_first}, {\S}\ref{sec:necessity-first}:
   gates are essentially $C^2$ and univariate, and product coupling
   makes $\varepsilon$-verification NP-hard), and extending it beyond
   affine certificates is the open problem that the regime exclusions
   delimit.

    \item \textbf{$\varepsilon$-Separation (Theorem~\ref{thm:necessity}).}
   The LUT-verification decision problem is polynomial for SVNN and
   NP-hard for coupled architectures (self-contained 3-SAT reduction);
   SVNN uniquely attains (polynomial time, zero slack under ETH)
   (completeness of this boundary is the trichotomy necessity conjecture).

    \item \textbf{Compile-Aware Generalization
   (Theorem~\ref{thm:generalization}).} The deployed risk decomposes as
   $\hat{R}(N) + O(\gamma^{L-1}/\sqrt{n}) + O(\gamma^{L-1}c_kM_kh^k)$
   with the resolution-matching law $N^{*} \asymp n^{1/(2k)}$---a
   directly actionable compiler rule for embedded deployment.
   ...
   Non-SVNN architectures (MLPs) degrade by $\Theta(d^{1/2})$ per layer
   ($\Theta(d^{L/2})$ over $L$ layers) due to cross-term coupling in
   ...
   connection $(\alpha, \gamma)$, making the framework, to our knowledge, the first neural
   network compilation theory formally grounded in abstract interpretation.
   ...
   properties under the timing premise $\text{WCET}(P) + \text{WCET}(N)
   \leq$ scan cycle---to our knowledge, the first such guarantee for
   NN-on-PLC deployment.

    \item \textbf{WCET (Theorem~11).} Parameterized worst-case execution time
   bound: $3.86$~ms for KAN $[28,16,4]$ on S7-1200 ($25.9\times$ scan-cycle
   margin).
...
\vspace{4pt}
\noindent\textbf{Empirical validation (74 experiments + 11 capacity
experiments E-T1--E-T11, 4 architectures).}
...
Soft-contractive training (E-T9) converts the certificate from thin
($1.0\times$, expected) to sound ($8.37\times$, first-principles) with a
validated float32 tier of $11.6\times$ (measured-floor bound $0.058$
vs.\ measured maxAE $0.053$), $99.9\%$ Box-Continuation coverage, and
depth extension to $L{=}3$ (sound $2.4\times$ via signal-domain
propagation, 2026-08-05). The bounded-amplitude $C^2$-BV family reaches
\emph{pervasive contractivity} ($\gamma < 1$ at every layer, output-layer
projection being accuracy-neutral under logit scaling) with sound
certificates of $9.6\times$ (FourierKAN, L0-direct configuration) and
$4.0\times$ (WaveletKAN)---a full-chain, wide-coverage guarantee;
adaptive allocation (E15) achieves $3.6\times$ at unchanged storage.

\vspace{4pt}
\noindent\textbf{Open directions.} We mechanized the core SVNN type system,
the repaired Galois connection, and the abstract type-soundness statement
in Coq~9.1 ({\S}\ref{app:coq}, \texttt{code/coq/}, compiled and
verified 2026-08-06); extending the mechanization to the full
type-soundness theorem and completing the three admitted
algebraic sub-lemmas of the balanced-allocation proof remain open.
Physical PLC WCET measurement (all current timing
evidence is simulation-based via PLCSIM Advanced and Z3 SMT), and online fine-tuning
...
His research interests include neural network compilation, the
information-theoretic and type-theoretic foundations of certified
deployment, industrial automation, formal verification of PLC software,
and knowledge distillation for edge deployment.
\end{IEEEbiography}
