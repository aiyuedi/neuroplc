...
for coupled architectures), and SVNN uniquely attains (polynomial time,
zero slack under ETH; the completeness of this boundary is the trichotomy
necessity conjecture). Fourth, the \textbf{compile-aware generalization bound}
...
deployment capacity} $C(B,V)$ ({\S}\ref{sec:capacity})---the best
certifiable deployment error under a storage budget $B$ and a
...
PyTorch-to-IEC~61131-3 compiler, with four-tier verification: Z3
...
code-generation regressions (it detected and validated the fix of a real
emitter bug that degraded deployed accuracy to 83\%). Generated code
...
Paderborn) validate the framework; RBF-KAN and MNIST remain
theoretical/planned. The theory answers not ``can we compile neural networks to
...
\begin{enumerate}[label=(\roman*)]
    ...
    quantity: the capacity $C(B, V)$ of certifiable deployment under a
    storage budget $B$ and a certificate-verification budget $V$. A
...
(Theorem~\ref{thm:wcet}: $3.86$~ms, $4.4\times$ scan-cycle margin)
...
$\sqrt{d}$ random-weights baseline, {\S}\ref{sec:da}). KAN's B-spline decomposition is \textit{structural
...
theory as the first verified PyTorch-to-IEC~61131-3 compiler,
...
\begin{enumerate}
    ...
    \item \textbf{Theory (Deployment Safety):} The
         ...
         provides parameterized worst-case execution time bounds
          ($3.86$~ms for KAN $[28,16,4]$, $4.4\times$ scan-cycle margin).
    ...
    \item \textbf{Theory (Completeness):} Beyond sufficiency, we establish
         ...
         and \textbf{E57} (NeuroPLC DA 8.3$\times$ tighter than CROWN-IBP)
...
$M_2^{\text{char}}$ calibration: classification correctness is
mathematically guaranteed under the LUT error distribution, albeit with
a thin margin.
...
\begin{enumerate}[label=(\roman*)]
   ...
   bound of $3.86$~ms provides a safe overestimate with $4.4\times$
...
certificate intact---the DA bound \textit{tightened} from 0.064 to 0.049
...
E57& CROWN vs.\ NeuroPLC     & KAN[28,16,4] DA/IBP & \textbf{8.3$\times$ tighter}, CROWN=IBP for KAN \\
...
\begin{itemize}
    ...
    \item \textbf{DA error bound:} $\Delta_{\text{DA}}^{(L=3)} = 0.98$, a
          $15.3\times$ increase from $\Delta_{\text{DA}}^{(L=2)} = 0.064$ —
...
$8.3\times$ tighter than CROWN-IBP ($\Delta = 5.45$, safety factor $0.1\times$)}.
...
learning-theoretic guarantee---and instantiated it as the first verified
PyTorch-to-IEC~61131-3 compiler. The central finding is that compiler
...
\begin{enumerate}[label=\arabic*.]
    ...
    verification budget under perfect deterministic soundness;
    theorem-level information-theoretic necessity within the calculus;
    an architecture-as-code selection law. The sharp constants, minimax
    rates, separation, and trichotomy of the items below are its
    expansion ({\S}\ref{sec:capacity}).
   ...
   bound: $3.86$~ms for KAN $[28,16,4]$ on S7-1200 ($4.4\times$ scan-cycle
   margin).
...
\vspace{4pt}
\noindent\textbf{Empirical validation (74 experiments, 4 architectures).}
