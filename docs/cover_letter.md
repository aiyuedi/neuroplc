...
**Deployment certificates (closing the loop)**: adaptive LUT allocation lifts the DA safety factor to 3.6× at unchanged storage; soft-contractive training (E-T9, 98.5% acc) reaches a sound certificate of 11.6× (no-cancellation, measured-floor bound 0.058 vs measured maxAE 0.053), expected tier 763×, Box-Continuation coverage 99.9%, Z3 512/512, full SCL compile chain, and depth extension to L=3 (6.1×). The released checkpoint remains non-contractive (γ = [15.4, 5.3]) with a thin 1.0× expected-tier margin — the empirical motivation for, and contrast against, the contractive recipe.
