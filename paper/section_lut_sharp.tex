...
\qquad
e^{*}_{\text{cross}} \;=\; \frac{(b-a)^2}{8 \bigl(\sum_i
(M_2^{(i)})^{-1/2}\bigr)^{2}},
\label{eq:optimal_cross}
\end{equation}
which improves on the uniform allocation by the factor
$\max_i M_2^{(i)} \cdot \bigl(\sum_j (M_2^{(j)})^{-1/2}\bigr)^{2}/N^{2}$
(for the released checkpoint: $2.99\times$, E-T1).
...
(\texttt{verify\_optimal\_lut.py}, E-T1: cross-function $2.99\times$,
