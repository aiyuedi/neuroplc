...
For a linear layer $\ell$ followed by an element-wise layer:
\begin{equation}
...
d_{\ell-1})}{1 - \gamma} = O(L \cdot M_{\max} \cdot h^2 \cdot d_{\max})
...
geometric series, yielding a factor of $M_{\max} h^2$ per layer
multiplied by the $d_{\ell-1}$ parallel activations. This form is
\emph{depth-uniform}: it grows only linearly in $L$ and admits deployment on
...
characterize a genuine structural boundary? We argue the latter through
three observations.
...
$^{\ddag}$Polynomial and wavelet activations are piecewise-polynomial and closable under Z3's polynomial theory; full verifiability follows by the same structural induction as B-spline KAN (Theorem~2, Step~3).\\
...
certificate of {\S}IV-F---yields no proof for any SiLU segment. Empirically:
