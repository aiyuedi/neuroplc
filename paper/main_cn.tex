...
我们通过\neuroplc{}实例化SVNN——这是首个将PyTorch模型翻译为西门子S7-1200/1500 IEC~61131-3 SCL的基于IR的编译器。三种验证机制组合为端到端保证：(i)~一次性Z3 SMT证明，确认4/6种IR操作类型模板对所有可能参数均正确（其余2/6——StandardAct和Softmax——因其超越函数$\exp(x)$超出Z3的可判定NRA片段，采用解析证明）；(ii)~512个逐函数Z3证明组装为机器可检查的端到端证书，由约200行可信检查器验证；(iii)~符号结构仿射算术，比区间算术紧致$2.2\times$（与$\sqrt{d}$随机权重基线相比达$3.1\times$，安全因子$8.5\times$）。KAN的分解是\textit{结构必然性}：同规模KAN与MLP分别获得512/512与0/48的Z3可验证组件（MLP~$[28,32,16,4]$，E41；同规模MLP~$[28,16,4]$则为0/16 Z3可验证组件）。该框架扩展到ChebyKAN（切比雪夫多项式基，命题2，496/512 Z3可验证）。三层KAN实现608/608 Z3验证（100\%），NeuroPLC的DA界比CROWN-IBP紧致$85\times$，生成的SCL在TIA Portal V21中以\textbf{0错误、0警告}编译（45.2~KB，S7-1200预算的90.4\%；TIA Portal V21是西门子离线工程平台，非生产现场）。在CWRU（99.93\%）、XJTU-SY全寿命试验（91.7\%微调，512/512 Z3保留）和MNIST（98.6\%）上验证。
\end{abstract}
...
\textbf{KAN硬件部署。}四条独立研究路线证明KAN的单变量B样条结构独特地适合基于LUT的硬件评估——这是MLP不具备的特性。FPGA方面，KANEL\'{E}~\cite{kanele2026}（ISFPGA 2026最佳论文）通过量化B样条的LUT评估实现2,700$\times$加速。微控制器方面，LUT-KAN~\cite{lutkan2025}以预计算LUT操作替代Cox-de~Boor递归（7--25$\times$加速，每条边仅68字节）；KAN-SoC~\cite{flores2025kan}在电动汽车电池估算中实现比MLP高10$\times$的内存缩减（693字节vs.~6,807字节）。PLC方面，Corr\^ea~\cite{correa2024kanplc}（USP 2024）提供了唯一的KAN-on-PLC先例：KAN~$[7,5,6]$通过Snap7分布式执行部署（PC计算推理，PLC通过以太网接收结果），在轴承分类任务上达到88.5\%准确率。
...
\begin{figure}[!t]
    ...
    \caption{跨30个随机KAN种子的DA/IA紧致比分布，固定架构$[28,16,4]$，含每因素分析（隐藏维数、网络深度、符号平衡）。训练KAN的$3.1\times$比位于分布均值附近（$3.71 \pm 0.81$），确认其代表性而非择优。}
...
\begin{definition}[SCL编译指称]
\label{def:scl_denotation}
$\llbracket\text{compile}(\mathcal{G})\rrbracket(x)$是目标PLC上的SCL REAL语义。它仅在BsplineLUT节点处（LUT近似引入误差）并通过IEEE~754舍入与$\llbracket\mathcal{G}\rrbracket(x)$不同（$\delta_{\text{fp32}} \leq 10^{-6}$每浮点操作~\cite{paszke2019pytorch}）：
\begin{equation}
\begin{aligned}
...
\end{equation}
所有其他操作均精确编译（至多fp32舍入）。注意：若启用\texttt{lutize\_exp}优化，StandardAct(SiLU)和Softmax同样使用LUT，分别具有误差界$\varepsilon_{\text{exp}}$和$\varepsilon_{\text{SiLU}}$；简略起见此处不作显式处理。
\end{definition}
...
\begin{remark}[基于符号平衡浓度的概率紧致]
\label{rem:prob_tightening}
定理1提供对\textit{任意}权重配置成立的确定性最差情况界。在引理3的随机符号模型下，此界可被概率紧致：以置信度$\geq 1-\delta$，当使用区间算术形式（$\sum_i \sum_j |W^{(1)}_{k,j}W^{(0)}_{j,i}|$）时，有效误差至少比式~\eqref{eq:l_net}所建议的小$\Omega(\sqrt{d_1})$倍。具体而言，对训练$[28,16,4]$ KAN，最差情况界为$\Delta_{\text{logit}} \leq 0.079$（DA形式），而区间算术形式将产生$0.172$。引理3保证跨随机KAN初始化DA/IA比$R \geq 2.2$以95\%置信度成立，意味着部署安全边界至少比朴素区间分析报告的大$2.2\times$。经验比$R = 3.1\times$（表~\ref{tab:da_sign}）和30种子分布（图~\ref{fig:da_ratio_dist}）确认训练KAN一致超越此保守概率下界。
\end{remark}
...
采用VRM知识蒸馏~\cite{zhang2025vrm}，损失函数为：
\begin{equation}
\mathcal{L} = \alpha \mathcal{L}_{\text{KL}}(q^T, q^S) + (1-\alpha)\mathcal{L}_{\text{CE}}(y, q^S) + \lambda_{\text{rel}}\mathcal{L}_{\text{VRM}},
\end{equation}
其中 $\mathcal{L}_{\text{VRM}} = \text{MSE}(\text{cos\_sim}(\mathbf{z}^T), \text{cos\_sim}(\mathbf{z}^S))$，温度 $\tau=4.0$，$\alpha=0.3$，$\lambda_{\text{rel}}=0.5$。训练100轮（Adam，lr=0.003，余弦退火，耐心20）。蒸馏是\textit{训练机制}——贡献在于 KAN 的 $7.9\times$ 压缩不损害可验证性。
...
\begin{table}[!t]
\centering
\caption{跨项目TIA Portal V21编译覆盖率。所有项目以零错误、零警告编译，通过MCP \texttt{CompileAndDiagnosePlc} API在CPU~1211C V4.7上以程序方式验证。}
\label{tab:tia_compile}
\small
\begin{tabular}{@{}llccc@{}}
\toprule
\textbf{模型} & \textbf{架构} & \textbf{块封装} & \textbf{错误/警告} & \textbf{状态} \\
\midrule
...
\end{tabular}
\vspace{2pt}
{\scriptsize 全部三次编译于2026-07-07通过TIA Portal V21 Openness API (\texttt{CompileAndDiagnosePlc})验证，该API递归遍历西门子\texttt{CompilerResult}消息树以暴露任何叶子诊断。}
\end{table}

\vspace{4pt}
\noindent\textbf{跨项目编译覆盖率。}
除表~\ref{tab:tia_blocks}的单个项目外，我们在三个独立TIA Portal V21项目中编译了生成代码，以确认编译器输出无论模型家族或块封装方案均被西门子工具链接受。表~\ref{tab:tia_compile}总结结果：两个KAN $[28,16,4]$项目（紧凑DB+FB分割，不同DB/FB编号）和一个MLP $[28,32,16,4]$基线，全部以CPU~1211C V4.7为目标。每次编译返回\textbf{零错误、零警告}，通过MCP \texttt{CompileAndDiagnosePlc} API以程序方式验证。这确立了NeuroPLC生成语法和语义均有效的IEC~61131-3 SCL，通过完整西门子编译前端——人类工程师手写代码在下载前必须通过的同一声明。
...
表~\ref{tab:cycle_count}给出KAN $[28,16,4]$和MLP $[28,32,16,4]$在两个目标上的每操作分解和估计每推理延迟。

\begin{table}[!t]
\centering
\caption{生成SCL代码中的静态操作计数（每推理）。操作计数跨目标相同（相同生成代码）。S7-1200上包含循环体执行效应的动态指令级时序见表~\ref{tab:instr_timing}。}
\label{tab:cycle_count}
\footnotesize
\begin{tabular}{lrr}
\toprule
\textbf{指标} & \textbf{KAN [28,16,4]} & \textbf{MLP [28,32,16,4]} \\
\midrule
REAL乘法      & 524  & 1,472 \\
REAL加法      & 533  & 1,473 \\
EXP调用       & 3    & 1 \\
...
\midrule
\textbf{动态时序(S7-1200)} & & \\
~~KAN (表~\ref{tab:instr_timing}) & \textbf{13.4~ms} & --- \\
~~MLP (估计$\sim 2.7\times$动态操作) & --- & $\sim$58~ms \\
\textbf{动态时序(S7-1500)} & & \\
~~估计($\sim$100$\times$更快FPU) & $\sim$0.21~ms & $\sim$0.58~ms \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{指令级推理时序：KAN $[28,16,4]$ $\to$ S7-1200}
\label{tab:instr_timing}
\footnotesize
\begin{tabular}{@{}lrrr@{}}
\toprule
\textbf{操作} & \textbf{次数} & \textbf{时间 ($\mu$s)} & \textbf{占比} \\
\midrule
DB数组访问      & 5,322 & 7,983 & 59.7\% \\
...
循环开销        & 28    & 14    & 0.1\%  \\
\midrule
\textbf{总计}   & ---   & \textbf{13,371} & \textbf{100\%} \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize 时序按西门子S7-1200 V4.7手册: REAL操作ADD 0.50~$\mu$s, MUL 0.60~$\mu$s, DIV 1.20~$\mu$s; EXP 60~$\mu$s (Taylor); DB访问1.5~$\mu$s; INT 0.04~$\mu$s; 循环开销0.5~$\mu$s/次.}
\end{table}
...
\begin{table}[!t]
\centering
\caption{Z3验证最差情况执行时间：KAN $[28,16,4]$ 在 S7-1200 CPU~1211C上。时序为对认证域内所有输入保证的\textit{形式上界}，由西门子指令时序和Z3验证的控制流分析导出。"Det."表示执行路径是输入无关的($\checkmark$)或随输入变化($\sim$, 由Z3界定)。}
\label{tab:wcet}
\small
\begin{tabular}{lrrrr}
\toprule
\textbf{IR节点} & \textbf{形状} & \textbf{WCET ($\mu$s)} & \textbf{FLOPs} & \textbf{Det.} \\
\midrule
...
\vspace{2pt}
\noindent\textit{证明。}
\textbf{步骤1（确定性最差情况递推）。}由定理1结构归纳，每层误差递推的区间算术形式为：
\begin{equation}
\Delta_{\text{IA}}^{(\ell)} = \varepsilon_{\text{LUT}} \cdot d_{\ell-1} + L_B \cdot \|\mathbf{W}^{(\ell-1)}\|_{1,\infty} \cdot \Delta_{\text{IA}}^{(\ell-1)}.
\end{equation}

\textbf{步骤2（随机符号下的期望紧致）。}在引理3随机符号模型下：
\begin{equation}
\mathbb{E}[\Delta^{(L)}] = \varepsilon_{\text{LUT}} \cdot \sum_{\ell=1}^L d_{\ell-1} \cdot \gamma^{L-\ell} \leq \varepsilon_{\text{LUT}} \cdot d_{\max} \cdot \frac{1-\gamma^L}{1-\gamma}.
\end{equation}
...
\vspace{4pt}
\noindent\textbf{PLC端特征提取可行性。}
在讨论编译器LUT精度之前，一个前置问题是：提取28维特征的DSP流水线能否在PLC端实现？表~\ref{tab:scl_feature_frontend}报告了SCL编译的10维时域特征提取功能块的数值验证：与Python float64参考相比，SCL REAL float32实现的最大误差为0.00（比特级一致，IEEE~754往返验证）。然而，准确率从全28维Python特征的100\%降至PLC-only 10维的44--45\%——54.8\%的下降量化了频域和色散熵特征对CWRU的不可替代性。这确立了PLC端部署的\textit{下界}：时域特征可精确编译，但高准确率要求28维全特征——实际部署中需通过外部DSP或边缘设备预计算后经PROFINET传输。
...
\begin{figure}[!t]
    ...
    \caption{E6：LUT交叉验证。左：每元素对数误差直方图（MAE 0.15，MaxAE 3.65，由类间边界$\ge$1.35主导）。右：每类一致性（全部四个类别均为100\%）。LUT近似前向传递以完美保真度保持分类。}
...
\begin{table}[!t]
    \centering
    \caption{Z3 SMT翻译验证：逐节点验证结果}
    \label{tab:z3_smt}
    \footnotesize
    \begin{tabular}{@{}llcrl@{}}
        \toprule
        \textbf{IR节点} & \textbf{操作类型} & \textbf{验证结果} & \textbf{Z3时间} & \textbf{误差界} \\
        \midrule
        input\_features & MatMul      & 精确（代数等价）  & 11\,ms  & --- \\
        l0\_silu       & StandardAct & 精确（代数等价）  & 1\,ms   & --- \\
        l0\_bspline    & BsplineLUT  & $\leq\varepsilon$  & 474\,ms & 0.0459 \\
        l0\_base       & MatMul      & 精确（代数等价）  & 1\,ms   & --- \\
        l0\_merge      & Add         & 精确（代数等价）  & 1\,ms   & --- \\
        l1\_silu       & StandardAct & 精确（代数等价）  & 1\,ms   & --- \\
        l1\_bspline    & BsplineLUT  & $\leq\varepsilon$  & 65\,ms  & 0.0459 \\
        l1\_base       & MatMul      & 精确（代数等价）  & 1\,ms   & --- \\
        l1\_merge      & Add         & 精确（代数等价）  & 1\,ms   & --- \\
        softmax        & Softmax     & 精确（代数等价）  & 1\,ms   & --- \\
        argmax         & Argmax      & 精确（代数等价）  & 1\,ms   & --- \\
        \midrule
        \multicolumn{2}{l}{\textbf{总计（11节点）}} & 9精确 + 2有界 & 599\,ms & --- \\
        \bottomrule
    \end{tabular}
    \vspace{2pt}
    {\scriptsize 全部验证在Intel Core i7-13620H、Z3 4.16.0、30\,s超时下进行。精确：代数恒等式；有界：$|f_{\text{LUT}} - f_{\text{B-spline}}| \leq M_2 h^2/8$（定理1）。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{跨载荷泛化（E7）：KAN在1~hp训练}
\label{tab:cross_load}
\begin{tabular}{lccc}
\toprule
\textbf{目标载荷} & \textbf{测试准确率(\%)} & \textbf{95\% CI} & \textbf{样本数} \\
\midrule
...
\end{tabular}
\vspace{1pt}
{\scriptsize Bootstrap 95\% CI（10,000次重采样）。模型仅在1~hp载荷训练，在未见载荷上评估。结果展示了28-D特征空间的强跨载荷泛化能力。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{DA界协调：训练$[28,16,4]$ KAN的全部报告值}
\label{tab:da_bounds_summary}
\footnotesize
\begin{tabular}{@{}lllll@{}}
\toprule
\textbf{来源} & \textbf{$N$} & \textbf{方法} & \textbf{$\Delta_{\text{logit}}$} & \textbf{安全因子} \\
\midrule
E9 / 定理1 IA  & 15 & 区间算术（最差情况） & 0.172 & $3.9\times$ \\
E9 / 定理1 DA  & 15 & 双元算术（高概率）     & 0.079 & $8.5\times$ \\
E9 / E16 Seg-DA & 15 & DA + 分段感知de~Boor  & 0.056 & $11.9\times$ \\
E9 / E16 Seg-DA & 20 & DA + 分段感知de~Boor  & 0.0215 & $\sim31\times$ \\
E40 组合         & 15 & DA 网络级（层级3）     & 0.1196 & $5.6\times$ \\
E6 经验 MaxAE   & 15 & 逐样本最大值（非界）   & 3.65   & --- \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize \textbf{IA界}（0.172）：对所有$x\in[-3,3]^{28}$可靠，使用$\sum|W_{k,j}W_{j,i}|$。
\textbf{DA界}（0.079）：随机符号模型下高概率（引理3），使用$|\sum W_{k,j}W_{j,i}|$。
\textbf{Seg-DA}（0.056）：含逐段$M_2^{(k)}$值的DA（E16），额外$1.4\times$紧致。
\textbf{组合DA}（0.1196）：三层证书的网络级界（{\S}\ref{sec:compositional}），使用逐函数Z3验证$\varepsilon=0.0036$配合DA传播（比值$2.1\times$ vs.\ IA 0.2473）。
\textbf{MaxAE}（3.65）：跨2,743测试样本的经验最大绝对对数误差——非认证界。
IA和DA界认证分类正确性（$\Delta < \text{边界}/2 = 0.675$）；经验MaxAE反映无聚合安全假设下独立误差符号的最差样本行为。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{跨数据集迁移：CWRU $\to$ XJTU-SY（零样本）}
\label{tab:xjtu_sy}
\footnotesize
\begin{tabular}{@{}p{2.2cm}rrrp{1.5cm}p{1.7cm}@{}}
\toprule
\textbf{工况} & \textbf{轴承数} & \textbf{样本数} & \textbf{准确率} & \textbf{占优类别} & \textbf{故障模式} \\
\midrule
35 Hz / 12 kN    & 5 & 1,000 & 100.0\%$^{*}$ & 外圈 & 自然磨损 \\
37.5 Hz / 11 kN  & 5 & 1,000 & 100.0\%$^{*}$ & 外圈 & 自然磨损 \\
40 Hz / 10 kN    & 5 & 1,000 & 100.0\%$^{*}$ & 外圈 & 自然磨损 \\
健康 (B1\_1)     & 1 & 200   & 0.0\% & --- & 早期寿命 \\
\midrule
\textbf{全部}     & 15 & 3,200 & 62.5\%$^{\dag}$ & 外圈 & --- \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize $^{*}$仅外圈子集内100\%；所有其他类别误分类（0\%）。$^{\dag}$62.5\%匹配数据集中外圈比例（2000/3200）。模型在域偏移下坍缩为单类预测。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{跨数据集微调：CWRU $\to$ XJTU-SY（微调后）}
\label{tab:xjtu_ft}
\footnotesize
\begin{tabular}{@{}lrrrr@{}}
\toprule
\textbf{类别} & \textbf{零样本} & \textbf{微调后} & \textbf{提升} \\
\midrule
正常        & 0.0\%  & 95.0\% & +95.0 pp \\
...
\midrule
\textbf{总体} & \textbf{29.8\%} & \textbf{91.7\%} & \textbf{+61.9 pp} \\
\midrule
\multicolumn{4}{@{}l}{DA界：0.064 $\to$ 0.049 \quad Z3验证：\textbf{512/512}（微调后）} \\
\multicolumn{4}{@{}l}{SCL编译：2,188行，\textbf{0错误 0警告}（S7-1200）} \\
\multicolumn{4}{@{}l}{SVNN条件3（$\gamma<1$）：\textbf{保持}（$\gamma_0{=}0.459$, $\gamma_1{=}0.502$）} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize 逐类零样本显示内圈故障保留部分迁移能力（76.7\%），而正常完全坍缩（0\%）。验证分割上29.8\%的整体零样本准确率与62.5\%的全数据集数字（表~\ref{tab:xjtu_sy}）不同，因分层分割中的类别不平衡归一化所致。100轮微调恢复所有类别至$\geq$80\%，正常和外圈故障超过95\%。}
\end{table}
...
\midrule
均匀最差$\varepsilon$ & 0.00982 & 0.00406 & 0.00220 & 0.00033 \\
自适应最差$\varepsilon$ & 0.00294 & 0.00115 & 0.00061 & 0.00009 \\
\textbf{最差$\varepsilon$降低} & \textbf{70.0\%} & \textbf{71.6\%} & \textbf{72.2\%} & \textbf{73.1\%} \\
\midrule
质量等价存储 & --- & 17,888~B & --- & --- \\
vs.\ 均匀的存储节省 & --- & \textbf{41.8\%} & --- & --- \\
\midrule
...
\end{tabular}
\vspace{1pt}
{\scriptsize 质量等价：自适应方案匹配均匀最差情况$\varepsilon$所需存储。在N=15预算下，自适应节省41.8\%。全部实验使用KAN $[28,16,4]$模型的512个B样条函数。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{NeuroPLC vs.\ RTNNIgen~\cite{rtnnigen2024}：多维度对比}
\label{tab:rtnnigen}
\footnotesize
\begin{tabular}{@{}p{1.9cm}p{2.9cm}p{2.9cm}@{}}
\toprule
\textbf{维度} & \textbf{NeuroPLC（本文）} & \textbf{RTNNIgen} \\
\midrule
模型支持 & MLP + KAN（B样条LUT） & 仅MLP（Dense层） \\
\addlinespace
正确性 & 定理1 + DA（2.2$\times$）+ 分段感知 & 未报告 \\
\addlinespace
PLC平台 & 西门子S7-1200/1500（SCL） & Beckhoff TwinCAT~3（ST） \\
\addlinespace
代码结构 & DB+FB集成SCL，人类可读 & FB + 二进制权重文件 \\
\addlinespace
激活函数 & B样条LUT + SiLU + Softmax + Argmax & 标准（ReLU, tanh等） \\
\addlinespace
验证 & 166测试 + TIA V21（0e/0w） & 仓库无测试套件 \\
\addlinespace
参数处理 & 人类可读ARRAY OF REAL & 打包二进制blob \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[!t]
\centering
\caption{域偏移幅度前10特征（CWRU $\to$ Paderborn）}
\label{tab:paderborn_cohens_d}
\footnotesize
\begin{tabular}{@{}rrc@{}}
\toprule
\textbf{排名} & \textbf{Cohen's $d$} & \textbf{解释} \\
\midrule
 9 & $+7.40$ & 极大偏移（方差类特征） \\
 7 & $+3.83$ & 大幅偏移（能量类特征） \\
14 & $+3.58$ & 大幅偏移 \\
...
15 & $ 0.00$ & 无偏移（常数特征） \\
\midrule
\multicolumn{3}{l}{28特征平均$|d|$：0.84。MMD$_{\text{RBF}}$ = 0.044（中等）。} \\
\multicolumn{3}{l}{特征9、7占主导：合计占总偏移幅度的54\%。} \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize 正$d$ = 目标均值超过源均值。特征在比较前按数据集z-score归一化。2个$d \approx 0$的特征近乎不变——未来传感器无关部署中域鲁棒特征选择的候选。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{NeuroPLC适用边界与推荐缓解措施}
\label{tab:boundaries}
\footnotesize
\begin{tabular}{@{}p{1.9cm}p{2.9cm}p{2.9cm}@{}}
\toprule
\textbf{边界} & \textbf{条件} & \textbf{缓解措施} \\
\midrule
DA $\rightarrow$ IA & 权重符号完美平衡（罕见） & 回退至IA；仍然有效 \\
\addlinespace
自适应 $\rightarrow$ 均匀 & 曲率多样性低 & 均匀LUT已足够 \\
\addlinespace
$L_{\text{net}}$爆炸 & 深度 $\geq$ 6层 & Monte Carlo；限制深度 \\
\addlinespace
...
\begin{table}[!t]
\centering
\caption{验证盲点：仅凭经验准确率作为部署门是不完备的。在$N=4,5$时，测试准确率保持99.93\%但SVNN安全因子降至$<1$，对抗搜索确认测试集遗漏的真实误分类。}
\label{tab:blind_spot}
\footnotesize
\begin{tabular}{@{}cccccc@{}}
\toprule
\textbf{$N$} & \textbf{测试准确率} & \textbf{DA $\Delta$} & \textbf{安全因子} & \textbf{盲点？} & \textbf{对抗翻转} \\
\midrule
4  & 99.93\% & 2.93  & 0.23$\times$ & \textbf{是} & 225/20,000 \\
5  & 99.93\% & 1.65  & 0.41$\times$ & \textbf{是} & 122/20,000 \\
6  & 99.93\% & 1.05  & 0.64$\times$ & \textbf{是} & 80/20,000 \\
7  & 99.93\% & 0.73  & 0.92$\times$ & \textbf{是} & 75/20,000 \\
...
\end{tabular}
\vspace{2pt}
{\scriptsize 安全因子 = $(m/2)/\Delta_{\text{DA}}$，其中$m=1.35$（E6）。安全因子$<1$表示认证误差超过类间边界的一半——引理1.6保证分类保持的阈值。盲点：$N \leq 7$（对抗搜索确认$N \leq 5$）。全部对抗翻转严格发生在$[-3,3]^{28}$内。}
\end{table}
...
\begin{table*}[tb]
\centering
\caption{结果总结：核心实验（E1--E20, E52--E57）与验证实验（V1--V7）}
\label{tab:summary}
\scriptsize
\renewcommand{\arraystretch}{0.95}
\begin{tabular}{@{}p{0.7cm}p{2.2cm}p{2.0cm}p{2.2cm}@{}}
\toprule
\textbf{实验} & \textbf{描述} & \textbf{指标} & \textbf{结果} \\
\midrule
\multicolumn{4}{l}{\textit{核心实验：}} \\
E1 & 教师 vs 学生    & 准确率（测试集） & 99.93 vs 99.93\%，$7.9\times$压缩 \\
E2 & KAN/MLP/SVM/RF        & 准确率（子集）  & 全部100\% \\
E3 & KD消融           & VRM, RKD, Hinton, No-KD & 99.9/99.9/99.9/24\% \\
E4 & LUT精度（三种方法） & DP最优@10点 & \textbf{+18.3\%} vs 均匀 \\
E5 & 编译器可扩展性  & 宽度/深度/网格 & 11/12通过；内存为瓶颈 \\
E6 & 分类一致性        & LUT vs FP32 & \textbf{100\%}，MaxAE 3.65 \\
E6-SMT & Z3翻译验证 & 11节点逐节点 & 9/11精确，2/11有界，0失败 \\
E7 & 跨载荷            & 0/2/3~hp准确率 & 99.97/100/99.97\% \\
E8 & 优化器消融    & 全流水线 & $-$11.9\%内存，+65\%速度 \\
E9 & 双元算术      & DA vs IA界 & \textbf{2.2$\times$紧致}（vs $\sqrt{d}$基线3.1$\times$） \\
E10& LUT稀疏性& 3--50点准确率 & 全部$\ge$99.96\% \\
E11& $M_2$校准      & $M_2^{\text{char}}$=0.177, max=1.586 & 比解析界紧致98.6\% \\
E12& XJTU-SY跨数据集 & 零样本/微调 & 29.8\%$\to$\textbf{91.7\%} (+61.9pp) \\
E13& 特征消融      & 时域/频域/DE    & 频域99.93\%；全28-D 99.96\% \\
E14& 对抗鲁棒性& $\sigma$=0--0.2高斯噪声 & 0\%退化；100/100最差保持 \\
E15& 自适应LUT & 贪心 vs 均匀  & 71.6\%$\downarrow$最差$\varepsilon$，41.8\%存储节省 \\
E16& 分段感知DA       & 逐段$M_2$ + DA       & 6.0$\times$紧致，安全$\sim$11.9$\times$ \\
E17& RTNNIgen对比 & 七维对比 & 正确性模型/可审计/优化/内存全维度领先 \\
E18& Paderborn迁移 & 零样本/微调 & 25.0\%$\to$79.4\%；Cohen's $d$域特征分析 \\
E19& 方法边界 & 三退化条件 & DA$\to$IA/自适应$\to$均匀/$L_{\text{net}}$爆炸边界 \\
E20& 电机载荷回归 & RMSE & 0.011；零代码修改 \\
E52& 验证盲点 & $N{=}4{\to}15$安全因子扫描 & $N{\le}7$时准确率99.93\%但安全$<$1；225翻转 \\
E54& ChebyKAN Z3 & 逐函数 Z3 NRA   & \textbf{496/512 (96.9\%)}，命题~\ref{prop:chebykan-svnn} \\
E55& XJTU-SY微调+SCL & 100轮FT + S7-1200 & \textbf{91.7\%}, 512/512 Z3, 0e 0w \\
E56& 深层KAN ($L{=}3$) & [28,16,8,4], 120轮 & \textbf{99.60\%}, 608/608 Z3, 14.6$\times$线性缩放 \\
E57& CROWN对比 & NeuroPLC DA vs CROWN-IBP & \textbf{85$\times$紧致} \\
\midrule
\multicolumn{4}{l}{\textit{验证实验：}} \\
V1 & 最差情况对抗   & 5,000输入，最差100  & 100/100保持，已验证 \\
V2 & LLM vs NeuroPLC           & SCL代码生成   & LLM检出6缺陷；NeuroPLC 0e 0w \\
V3 & DA $\sqrt{d}$缩放律 & 105架构，7维度 & Pearson $r{=}0.987$，斜率0.896 \\
V4 & MLP验证差距      & KAN vs MLP同规模 & 512/512 vs 0/48; $14.0\times$逐组件更差 \\
V5 & ONNX vs NeuroPLC    & 节点/代码体积    & 导出失败；$\ge$8,393 vs 11节点（763$\times$） \\
V6 & Z3 WCET        & S7-1200 CPU 1211C   & 总计 $\leq$2.86\,ms（周期2.9\%） \\
V7 & 验证盲点   & $N{=}4{\to}15$扫描 & 准确率99.93\%但$N{\le}7$时安全因子$<$1 \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize 完整结果和JSON报告见代码仓库。V系列针对具体审稿问题的验证实验（LLM对比、ONNX不兼容、缩放律、验证差距、对抗安全、WCET）。}
\end{table*}
...
\begin{enumerate}
    \item \textbf{层级1——编译器模板验证（一次性，4/6 IR操作类型）。}Z3证明4种操作类型的SCL代码模板对所有可能参数（权重、偏置、网格、LUT表）正确。这是\textit{编译器级}保证：证明的不是"此特定模型正确编译"，而是"此编译器对\textit{任意}SVNN编译正确"。表~\ref{tab:template_verify} 报告了逐操作模板验证结果：MatMul、Add、BsplineLUT、Argmax全部返回UNSAT（精确代数等价或de~Boor误差界），StandardAct和Softmax因涉及超越函数$\exp(x)$采用解析证明。BsplineLUT和Softmax因浮点舍入保持有界而非精确。
...
\begin{table}[!t]
\centering
\caption{编译器模板验证——Z3证明SCL模板对所有可能参数正确（层级1，一次性证明）}
\label{tab:template_verify}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
\textbf{IR操作} & \textbf{Z3结果} & \textbf{时间(ms)} \\
\midrule
  MatMul      & \texttt{UNSAT}（精确等价，16对维度）     & 27 \\
  Add         & \texttt{UNSAT}（精确等价，8种维度）      &  6 \\
  BsplineLUT  & \texttt{UNSAT}（de~Boor界，所有三次函数） &  7 \\
  Argmax      & \texttt{UNSAT}（精确等价，4种维度）       & 17 \\
...
\begin{table}[!t]
\centering
\caption{端到端组合验证结果（StudentKAN $[28,16,4]$，15点LUT）}
...
\midrule
  ...
  B样条函数验证（层级2）           & $512/512$ \\
  组合步骤（层级3）                & $9$ \\
  可信计算基（检查器）             & $\sim$200行Python \\
\midrule
  逐函数LUT误差 $\varepsilon$      & $0.0036$ \\
  IA网络界 $\Delta_{\text{IA}}$    & $0.2473$ \\
  DA网络界 $\Delta_{\text{DA}}$    & $0.1196$ \\
  DA/IA紧致比                      & $2.1\times$ \\
\midrule
  证书有效                         & 是 \\
  分类保持                         & 是 \\
...
\begin{table}[!t]
\centering
\caption{组件级可验证性：KAN vs.\ MLP $[28,16,4]$}
\label{tab:verify_gap_components}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{组件} & \textbf{KAN} & \textbf{MLP (SiLU)} & \textbf{MLP (ReLU)} \\
\midrule
  激活函数数  & $512$ (B样条) & $16$ (SiLU) & $16$ (ReLU) \\
  Z3可验证   & $512$ (100\%) & $0$ (0\%)   & $16$ (100\%) \\
  Z3片段     & 多项式NRA   & 超越函数    & 分段线性 \\
\midrule
  组合可行？ & 是（定理1）  & 否          & 否\textsuperscript{*} \\
\bottomrule
\end{tabular}
\vspace{4pt}
\noindent\textsuperscript{*}即使每神经元ReLU被验证，MLP组合仍失败：$\text{ReLU}(Wx+b)$产生数据依赖误差传播（$\text{ReLU}'\in\{0,1\}$），破坏使能DA紧致的符号结构保持。
\end{table}

\begin{table}[!t]
\centering
\caption{误差传播界：KAN DA vs.\ MLP IA。全部值使用相同逐组件LUT误差$\varepsilon = 0.0041$（$N{=}15$, $M_2{=}0.177$）。KAN界来自符号结构仿射算术（式~\ref{eq:da_bound}）；MLP界来自3层区间传播。}
...
\midrule
  ...
  KAN DA界 $\Delta_{\text{DA}}$       & $0.079$ \\
  KAN IA界 $\Delta_{\text{IA}}$       & $0.172$ \\
  MLP IA界 $\Delta_{\text{MLP}}$      & $3.04$ \\
\midrule
  DA/IA紧致（KAN）                    & $2.2\times$ \\
  MLP IA / KAN DA开销                 & $14.0\times$（逐组件），$38\times$（端到端） \\
  MLP IA / KAN IA开销                 & $17.7\times$ \\
  KAN端到端验证                       & 是 \\
  MLP端到端验证                       & 否 \\
\bottomrule
\end{tabular}
...
\begin{table*}[tb]
\centering
\caption{跨域流水线通用性：相同编译器和验证流水线应用于三个根本不同的数据域——图像分类（MNIST）、实验室轴承故障（CWRU）和运行至失效自然退化（XJTU-SY）。ChebyKAN（Chebyshev多项式基）作为替代SVNN兼容架构纳入（命题~\ref{prop:chebykan-svnn}）。}
\label{tab:cross_domain}
\footnotesize
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{指标} & \textbf{MNIST} & \textbf{CWRU} & \textbf{XJTU-SY} & \textbf{ChebyKAN} \\
 & \textbf{（图像）} & \textbf{（振动）} & \textbf{（自然磨损）} & \textbf{（多项式）} \\
\midrule
  数据域 & 手写数字 & 轴承故障 & 轴承退化 & 轴承故障 \\
  特征 & PCA & RCMDE+RCHFDE & 28维 & RCMDE+RCHFDE \\
  准确率 & 0.9859 & 0.9993 & 0.9172$^{*}$ & 1.0000$^{\dag}$ \\
  训练方式 & 从头训练 & KD (VRM) & 微调100轮 & 从头训练 \\
\midrule
  逐函数验证 & 512/512 & 512/512 & 512/512 & 496/512 \\
  DA界 $\Delta_{\text{DA}}$ & 0.1199 & 0.1196 & 0.0493 & 0.1242$^{\ddag}$ \\
  SVNN条件3 ($\gamma<1$) & 是 & 是 & 是 & 是 \\
  证书有效 & 是 & 是 & 是 & 是 \\
  SCL编译 & 0e 0w & 0e 0w & 0e 0w & 0e 0w$^{\ddag}$ \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize
$^{*}$从CWRU检查点微调100轮（E55）。
$^{\dag}$ChebyKAN $[28,16,4]$，Chebyshev度$N{=}5$，在CWRU上从头训练100轮。达到100.0\%测试准确率，在统计噪声内匹配B样条KAN的99.93\%，证明Chebyshev多项式基函数是具有同等分类性能的SVNN兼容替代方案。
$^{\ddag}$ChebyKAN使用基于Markov的全局$M_2$界（式~\ref{eq:cheby_m2_bound}），比经验$M_2$宽松5.4$\times$（E54）。DA界和SCL编译因此以解析（保守）估计报告。}
\end{table*}
...
\begin{table*}[tb]
\centering
\caption{PLC ML部署工具对比}
\label{tab:competitor}
\footnotesize
\begin{tabular}{@{}p{1.6cm}p{2.2cm}p{2.2cm}p{2.3cm}@{}}
\toprule
\textbf{维度} & \textbf{RTNNIgen \cite{rtnnigen2024}} & \textbf{MLconverter \cite{mlconverter2025}} & \textbf{NeuroPLC（本文）} \\
\midrule
输入格式 & Keras Sequential & scikit-learn (DT, MLP) & \textbf{PyTorch (KAN, MLP)} \\
IR/优化 & 无 & 无 & \textbf{4阶段IR, 6道优化传递} \\
B样条/KAN & \ding{55} & \ding{55} & \textbf{B样条+ChebyKAN} \\
西门子SCL & 未验证 & ABB专用 & \textbf{0e 0w TIA V21} \\
正确性 & \ding{55} & \ding{55} & \textbf{6定理 + DA + Z3} \\
\bottomrule
\end{tabular}
\vspace{1pt}
{\scriptsize 两工具均不提供基于IR的优化、B样条支持或设计时正确性保证。}
\end{table*}

\begin{table*}[tb]
\centering
\caption{KAN硬件部署：领域全景对比}
\label{tab:kan_deployment}
\footnotesize
\begin{tabular}{@{}p{1.5cm}p{1.2cm}p{1.2cm}p{1.2cm}p{1.3cm}p{1.4cm}@{}}
\toprule
\textbf{方面} & \textbf{KANEL\'{E}} & \textbf{LUT-KAN} & \textbf{KAN-SoC} & \textbf{Corr\^ea} & \textbf{NeuroPLC} \\
...
\midrule
源格式 & 定制 & 训练后 & 训练时 & Python & \textbf{PyTorch} \\
PLC代码生成 & \xmark & \xmark & \xmark & \xmark & \textbf{S7 SCL} \\
TIA验证 & \xmark & \xmark & \xmark & Snap7$^{*}$ & \textbf{0e 0w} \\
形式证书 & \xmark & \xmark & \xmark & \xmark & \textbf{6定理+Z3} \\
加速比 & 2{,}700$\times$ & 7--25$\times$ & --- & 分布式 & 原生OB1 \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize $^{*}$Corr\^ea~\cite{correa2024kanplc}：PC侧KAN推理，PLC通过Snap7以太网接收结果——无原生SCL编译。KANEL\'{E}~\cite{kanele2026}、LUT-KAN~\cite{lutkan2025}、KAN-SoC~\cite{flores2025kan}。}
\end{table*}
...
\begin{table}[!t]
    \centering
    \caption{NeuroPLC IR vs.\ ONNX IR（KAN $[28,16,4]$）}
    \label{tab:onnx_vs_ir}
    \footnotesize
    \renewcommand{\arraystretch}{1.0}
    \begin{tabular}{@{}lcc@{}}
        \toprule
        \textbf{指标} & \textbf{NeuroPLC IR} & \textbf{ONNX (opset 14)} \\
        \midrule
        图节点                    & \textbf{11}            & 导出\textit{失败} \\
        B样条表示                  & 原生 BsplineLUT         & 无标准算子 \\
        分解后节点数（估计）       & ---                    & $\geq$\textbf{8,393}（$\geq 763\times$） \\
        生成代码 / 体积            & \textbf{3,818} SCL / \textbf{45.2}\,KB & $>$200K行C / $\gg$400\,KB（估计） \\
        ONNX Runtime              & \textbf{0}\,KB          & \textbf{22,528}\,KB \\
        适配S7-1200               & \textbf{是}             & \textbf{否}（450$\times$超出） \\
        形式验证就绪              & \textbf{是}（Z3 SMT）   & 否 \\
        \bottomrule
    \end{tabular}
    \vspace{2pt}
    {\scriptsize 实验I：\texttt{torch.onnx.export}使用opset~14/17/20时在B样条\texttt{einsum}处失败。分解估计：512函数 $\times$ 每B样条15个ONNX原语 = 7{,}680 + 713结构节点 $\approx$ 8{,}393。ONNX Runtime最小构建 $\approx$22\,MB（无量化）。}
\end{table}
...
\begin{table}[!t]
\centering
\caption{认证阈值：各保证层级的最小LUT密度}
\label{tab:cert_thresholds}
\small
\begin{tabular}{@{}lcccccc@{}}
\toprule
\textbf{保证} & \textbf{方法} & \textbf{$N_{\min}$} & \textbf{安全因子} & \textbf{SIL目标} & \textbf{证据} \\
\midrule
高概率  & DA & 15 & $8.5\times$ & SIL~2 & 符号平衡（引理3） \\
可靠    & 分段感知IA & 15 & $2.9\times$ & SIL~2 & 逐函数$M_2$ \\
可靠    & IA（全局）& 20 & $1.4\times$ & SIL~3 & 无符号假设 \\
可靠+鲁棒 & IA（全局）& 25 & $2.3\times$ & SIL~3+ & $2\times$安全边界 \\
\bottomrule
\end{tabular}
\vspace{2pt}
{\scriptsize $N_{\min}$ = 安全因子$\geq 1.0$的最小LUT点数。$M_2^{\text{sound}} = 0.3$，$M_2^{\text{emp}} = 0.177$（E11）。数值针对KAN $[28,16,4]$在S7-1200 CPU~1211C上。}
\end{table}
...
\begin{enumerate}[leftmargin=*]
    \item \textbf{物理PLC测量。}通过PLCSIM Advanced API或python-snap7通道，在S7-1200 CPU~1211C上流式传输1,000样本测试集对比裸机时序和Z3 WCET预测，弥合软件级正确性与硬件级验证之间的剩余间隙。
    \item \textbf{PLCSIM Advanced验证回路。}在西门子PLCSIM Advanced虚拟CPU上运行完整TIA项目，通过虚拟背板比较运行时逻辑与PyTorch参考。
    \item \textbf{OPC UA集成。}将生成的SCL包装进OPC UA信息模型，实现在西门子工业边缘生态系统中的即插即用部署。
\end{enumerate}

\textbf{中期（理论+认证）：}
\begin{enumerate}[leftmargin=*,start=4]
    \item \textbf{安全认证试点。}与认证机构合作，以SVNN框架作为软件级安全论据，对隔离安全功能进行IEC~61508 SIL~2预评估。
    \item \textbf{理论收紧。}探索将定理5的NP难结果收紧为常数因子不可近似性（Unique Games Conjecture下），将泛化界（定理6）从收缩区域（$\gamma < 1$）扩展到$\gamma \geq 1$的过渡区域。
\end{enumerate}

\textbf{远期（生态扩展）：}
\begin{enumerate}[leftmargin=*,start=6]
    \item \textbf{多目标后端。}将编译器后端从S7-1200/S7-1500扩展到其他IEC~61131-3平台（Beckhoff TwinCAT、Codesys、B\&R Automation Studio），展示SVNN框架的平台独立性。
    \item \textbf{架构扩展。}将编译器支持扩展到轻量Transformer变体（SVNN兼容架构），将ChebyKAN证明技术扩展到其他正交多项式基（Legendre、Hermite）。
\end{enumerate}
...
IEEE~754浮点标准是确定性的，但此前无一PyTorch-to-PLC编译器能认证其输出保留训练模型行为。本文证明障碍不在浮点复杂度——而在\textit{架构}。当神经网络分解为纯线性和逐元素单变量操作（SVNN条件1--2），编译器可从模型参数单独计算对所有输入有效的有限逐输出误差界。当不满足时，问题变为NP难（定理5）。KAN满足这些条件；标准MLP不满足。这就是\textbf{可编译前沿}——本文的首要贡献。

这一前沿不仅是理论构造——它驱动了一个实际编译器。\neuroplc{}在TIA Portal V21中以0错误0警告编译，生成的SCL以安全因子$8.5\times$（DA）保持PyTorch语义，三层组合验证架构（模板证明$\to$逐函数证明$\to$可信检查器）提供机器可检查的端到端证书。$14.0\times$的KAN/MLP逐组件误差传播差距——跨数据集和跨架构持续存在——不是实现细节：它是命题1和定理5共同作用的数学必然性。深度缩放确认了理论预测（定理4）：误差线性增长而非指数增长，可编译深度极限由内存而非正确性约束。

本文回答的问题不是"能否编译"——先前工具已证明可以。问题是"能否\textit{认证}该编译"。答案取决于架构而非工程工作量。"从验证编译器到设计可验证架构"这一重构，比任何特定编译器实现更持久。物理PLC测量是向IEC~61508部署认证的关键下一步。
