...
\begin{figure}[t]
    ...
    \caption{$C^2$-BV Architecture Family: representative activation basis
            functions with $N{=}15$ LUT grid points (vertical stems).
            Panel (a): B-spline KAN (cubic local basis, $M_2 {\approx} 0.68$);
            (b): FourierKAN ($\omega{=}0.4$, $K{=}6$, $M_2 {\approx} 2.30$);
            (c): WaveletKAN (Mexican hat, $M_2 {\approx} 2.60$);
            (d): ChebyKAN (degree 5, $M_2 {\approx} 3.12$);
            (e): RBF-KAN ($\sigma{=}0.6$, $M_2 {\approx} 3.09$);
            (f): all five overlaid with $N{=}15$ LUT grid.
            The filled area under each curve illustrates that all $C^2$-BV
            activations are positive on compact domains, enabling the universal
            discretization strategy of Proposition~\ref{prop:c2bv-family}.}
