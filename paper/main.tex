...
$763\times$ expected certificate with $99.9\%$ Box-Continuation coverage,
...
$11.6\times$ (sound) / $763\times$ (expected) via soft-contractive
training, E-T9).
...
Soft-contractive DA (E-T9) & 15 & DA, $\gamma{=}[1.03,1.38]$ model & 0.0009 & $763\times^{\ddag}$ \\
Soft-contractive IA (E-T9) & 15 & IA, $\gamma{=}[1.03,1.38]$ model & 0.0013 & $537\times^{\ddag}$ \\
Soft-contractive sound (E-T9) & 15 & IA-form, no cancell., floored & 0.058 & $11.6\times$ \\
...
[1.03, 1.38]$, 98.5\% acc), constants measured on that checkpoint,
2026-08-04 (\texttt{verify\_contractive\_bounds.py}).
$^{\ddag}$Expected tier (high-probability, Lemma~\ref{lem:da_moment}),
$M_2^{\text{char}} = 0.040$ on this checkpoint. At the $M_2^{\max}$
calibration the DA/IA envelopes read 0.030/0.043 (safety
$22.6\times$/$15.9\times$). The \textbf{sound row} (0.058) uses the
no-cancellation IA form floored by the full-set measured maxAE
($0.053$, 13{,}714 inputs, Tier-4 simulator).}
\end{table}
...
\begin{itemize}
    ...
    \item \textbf{DA error bound:} the 3-layer DA chain measures
          ...
          no-cancellation IA form floored by the full-set measured maxAE
          $0.10$)---depth scaling \emph{with} a certificate, the
          $\gamma \to 1$ branch of Theorem~\ref{thm:generalization}
          instantiated at $L = 3$ (per-layer bound growth $1.9\times$ vs.\
          $8.6\times$ for the untrained 3-layer).
