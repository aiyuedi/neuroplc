...
\subsection{The Verifiability Trichotomy: Why SVNN Is the Unique Optimum}
...
the SVNN condition is exactly the regime where all four properties coexist.
...
\begin{enumerate}
    ...
    univariate, element-wise, $C^2$ on all of $\mathbb{R}$ with
    design-time-computable global $M_k$, then (P1)+(P2)+(P3) hold with
    the sharp constants of Theorem~\ref{thm:sharp-lut}. This is exactly
    the SVNN condition (Conditions~1--2 of
    Theorem~\ref{thm:svnn_conditions}).
    ...
    softmax, normalization): P2/P3 fail and P1 degrades.}
    ...
    E66). Computing \emph{tight} propagation constants requires solving
    ...
    $x \sin x$): P3 fails.} $M_k$ is finite on the reachable set but not
    globally computable; closed-form tight certification would require
    the reachable set itself---reachability, NP-hard. The polynomial-time
    alternative (interval propagation on a global superset box) is sound
    but exponentially loose in $L$ (classic interval explosion: constants
    $\|W\|_\infty^L$).

    \item \textbf{Necessity (completeness).} Any family satisfying
    (P1)+(P2)+(P3) must lie in Regime 1: (P2) forces $C^2$ gates
    (minimax rate); (P3) forces globally computable $M_k$ (else
    reachability, by Regime 4) and univariate gates (else NP-hard
    tightness, by Regime 3); (P1)-with-linear-cost forces univariate
    gates (else $N^m$). The SVNN condition is thus \emph{necessary and
    sufficient} for the joint attainment of (rate, cost, tightness,
    no-reachability), under the standard conjectures
...
(P2) $\Rightarrow$ $C^2$ (Regime 2 excluded), (P3) $\Rightarrow$ global
computable $M_k$ and univariate (Regimes 3--4 excluded), (P1)
$\Rightarrow$ linear-cost LUT (Regime 3's $N^m$ excluded). Hence Regime 1.
$\square$
...
``one verifiable family among many'': it is the unique regime where
second-order rate, linear LUT cost, closed-form sharp constants, and
reachability-freedom coexist. \textit{(iv) Empirical confirmation (E66).}
