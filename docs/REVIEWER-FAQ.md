...
## Q27: "The soft-contractive safety factors differ 42.5x (validated 11.6x vs expected 493x). Which is the certificate?"

**Our Answer (three tiers, each with its semantics):**
The worst-case (sound) certificate is **2.34×** (bound 0.288): the first-principles no-cancellation propagation with corrected layer semantics (layer-0 LUT errors sum directly at the hidden nodes; layer-1 amplification uses per-edge Lipschitz constants) — no empirical floor, valid for every in-domain input including unseen ones. It strictly covers the float32 measured maxAE (0.053 over 13,714 inputs, Tier-4 simulator; 5.4× headroom). The **11.6×** figure is the empirically validated tier (measured-floor bound 0.058 = max(0.039, 0.053×1.1) on those same 13,714 inputs): it certifies the measured corpus, not unseen inputs — its extension to unseen inputs is an empirical generalization claim, and the paper labels it as such. The **493×** figure is the expected tier (M₂_char median calibration, high-probability in the random-sign sense of Lemma da_moment) — the same nature as the released student's 1.0×. We report all three with their semantics because they answer different referee questions: "can you guarantee worst case for any input?" (2.34×), "what is the realized margin on the validation corpus?" (11.6×), and "what is the typical-case margin?" (493×). The released student's 1.0× is its expected tier; its per-function envelope (2.29, corrected propagation) is not a certificate — the soft-contractive model's sound tier *is* one (0.288 << 0.675).
