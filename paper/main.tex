...
(Theorem~\ref{thm:trichotomy}): the SVNN condition is the \emph{unique}
regime where second-order rate, linear LUT cost, closed-form sharp
constants, and freedom from reachability computation coexist---ReLU
...
zero slack). Fourth, the \textbf{compile-aware generalization bound}
...
\begin{enumerate}[label=(\roman*)]
    ...
    the \emph{unique} regime where second-order rate, linear LUT cost,
    closed-form sharp constants, and reachability-freedom coexist.
...
\begin{enumerate}
    ...
    \item \textbf{Theory (Deployment Safety):} The
         ...
         independence requirements. The \textbf{WCET Theorem} (Theorem~11)
...
\begin{enumerate}[label=(\roman*)]
   ...
   actual execution time is typically $13.4$--$18$~ms, but Theorem~11's
...
\begin{enumerate}[label=\arabic*.]
   ...
   The SVNN condition is the \emph{unique} regime where second-order
   rate, linear LUT cost, closed-form sharp constants, and
   ...
   domain-dependent curvature requires reachability.
   ...
   SVNN uniquely attains (polynomial time, zero slack).
