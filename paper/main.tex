...
to an $11.6\times$ \emph{sound} (no-cancellation, measured-floor) /
$493\times$ expected certificate with $99.9\%$ Box-Continuation coverage,
Z3-verified components, and depth extension to $L=3$ ($6.1\times$);
the sound bound (0.058) is tight against the measured maxAE (0.053). Four $C^2$-BV architectures
...
checkpoint. The DA bound ($0.66$, {\S}\ref{sec:da}) provides the
certificate, with a safety factor of $1.0\times$ at the
$M_2^{\text{char}}$ calibration (per-logit-element, in-domain; the
worst-function $M_2^{\max}$ calibration pushes the bound above the
half-margin, {\S}\ref{sec:limitations}): classification correctness is
mathematically guaranteed under the LUT error distribution \emph{within
the Box-Continuation domain}, albeit with a thin margin (restorable to
$3.6\times$ via adaptive allocation, {\S}\ref{sec:adaptive-lut}, or to
$11.6\times$ (sound) / $493\times$ (expected) via soft-contractive
training, E-T9).
...
and soft-contractive training (E-T9) to $11.6\times$ (sound) with full
...
and soft-contractive training (E-T9) to $11.6\times$ (sound).
...
Soft-contractive expected (E-T9) & 15 & DA, $M_2^{\text{char}}$ & 0.0014 & $493\times^{\ddag}$ \\
Soft-contractive theory (E-T9) & 15 & first-principles propag. & 0.288 & $2.34\times$ \\
Soft-contractive float64 (E-T9) & 15 & float64 deploy., floored & 0.026 & $26\times$ \\
Soft-contractive sound (E-T9) & 15 & float32 deploy., floored & 0.058 & $11.6\times$ \\
FourierKAN sound (E-T10) & 15 & bounded-amp.\ basis, floored & 0.138 & $4.9\times$ \\
WaveletKAN sound (E-T11) & 15 & bounded-amp.\ basis, floored & 0.237 & $2.8\times$ \\
...
envelopes 1.70/2.29 (safety $0.4\times$/$0.3\times$)---the design-time
certificate does not exist for this checkpoint, and its measured maxAE
($0.52$) is a lucky $4.4\times$ below the theoretical envelope.
\textbf{DA bound} (0.66): high-probability under the random-sign model
(Lemma~\ref{lem:da_moment}), uses $|\sum W_{k,j}W_{j,i}|$; certifies
classification at the $M_2^{\text{char}}$ calibration with thin margin
($1.0\times$; see the adaptive and soft-contractive rows below).
...
certified bound. In-domain certified values are the recomputed rows
...
15 & 99.93\% & 0.66  & 1.02$\times$ & No$^{\dag}$ & 37/20,000 \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
E56& Deep KAN ($L{=}3$)       & [28,16,8,4] 120ep & \textbf{99.60\%}, 608/608 Z3, DA 5.66, sub-exp. depth; E-T9-3L: \textbf{98.6\%}, sound cert.\ $6.1\times$ \\
...
\begin{itemize}
    ...
    \item \textbf{DA error bound:} the 3-layer DA chain measures
          ...
          per-layer amplification envelope $\gamma^{2} = 28.1$ that
          would apply at $L = 3$ under worst-case amplification,
...
\textbf{Metric} & \textbf{MNIST} & \textbf{CWRU} & \textbf{XJTU-SY} & \textbf{ChebyKAN} & \textbf{FourierKAN} & \textbf{WaveletKAN} \\
 & \textbf{(Image)} & \textbf{(Vib.)} & \textbf{(Nat.Wear)} & \textbf{(Poly.)} & \textbf{(E60)} & \textbf{(E61)} \\
\midrule
 Accuracy & 0.9859$^{\sharp}$ & 0.9993 & 0.9172$^{*}$ & 1.0000 & \textbf{1.0000} & \textbf{1.0000} \\
 Training & Scratch & KD (VRM) & FT 100ep & Scratch & FT 20ep$^{\S}$ & FT 20ep$^{\S}$ \\
\midrule
 ...
 SVNN Cond.~3 & Yes & Yes & Yes & Yes & Yes & Yes \\
...
Soft-contractive & Sound IA & 15 & $11.6\times$ & SIL~3+ & E-T9 checkpoint
(98.5\%); Box-Continuation coverage 99.9\%; Z3 512/512;
expected tier $493\times$ \\
Sound + margin   & IA (global) & 25 & $1.4\times$ & SIL~3 &
$1.5\times$ safety margin \\
\bottomrule
\end{tabular}
\vspace{2pt}
...
($1.0\times$) to sound ($11.6\times$, measured-floor bound $0.058$ vs.\
measured maxAE $0.053$) with $99.9\%$ Box-Continuation coverage and depth
extension to $L{=}3$ ($6.1\times$); adaptive allocation (E15) achieves
$3.6\times$ at unchanged storage.
