...
\begin{enumerate}
    ...
    \item \textbf{Algorithm (Verification \& Optimization):} Three novel
         ...
         for substantial per-segment tightening (composing with DA for
         an improved combined safety factor; absolute values pending
         recomputation on the released checkpoint); and
...
the Box-Continuation domain}, albeit with a thin margin.
...
This represents a novel application of affine arithmetic
...
distribution ($\varepsilon$ mean $0.017$, max $0.041$, E58) the bound
exceeds the half-margin. The classification guarantee of this
checkpoint is therefore marginal, and contractive training (spectral
normalization) is the deployment recipe that restores a comfortable
margin (future work).
...
(recomputed with the audit constants $L_{\text{net}} = 160.4$, 2026-08-04),
...
below (the earlier instantiation with $\gamma \approx 0.18$ and an
expected bound of $\approx 0.71$ used stale constants).
...
$\gamma \approx 0.20$ are obsolete.
...
the DA bound \textit{tightens} after fine-tuning (absolute values
pending recomputation on the released checkpoint; the learned weights
...
(absolute values pending recomputation)
...
\midrule
\multicolumn{4}{@{}l}{DA bound: tightens after FT (abs. values pending recompute) \quad Z3 verified: \textbf{512/512} (post-FT)} \\
...
at 15~points certifies classification with a thin margin.
...
E9 / E16 Seg-DA$^{\dag}$  & 15 & DA $+$ Segment-Aware de~Boor      & 0.056 & $11.9\times$ \\
E9 / E16 Seg-DA$^{\dag}$  & 20 & DA $+$ Segment-Aware de~Boor      & 0.0215 & $\sim31\times$ \\
...
$^{\dag}$\textbf{Seg-DA} (0.056): values from the earlier weight set;
recomputation against the released checkpoint is pending (segmented
$M_2^{(k)}$ machinery, E16). \textbf{Compositional DA} matches the
recomputed network-level DA ($0.66$, 2026-08-04; cf.\
...
$\varepsilon_i$ values, the DA bound tightens further: the worst-case
logit perturbation decreases from 0.66 to
$\sim$0.056, and the safety factor improves from $1.0\times$ to
...
% against the released checkpoint (2026-08-03).
...
E16& Segment-Aware DA       & Seg-DA vs DA       & Substantial tightening (abs.\ values pending recompute) \\
...
% compositional ratio pending recomputation (2026-08-03).
...
(SIL~2 at $N{\geq}22$, SIL~3 at $N{\geq}25$). For SIL~3+, Level~2 arithmetic
...
$[28,16,4]$ on S7-1200 CPU~1211C. Segment-aware IA rows from the
pre-recomputation constants are withdrawn (recomputation pending).}
\end{table}
