...
$^{*}$Contractivity verified for the trained KAN student $[28,16,4]$ under VRM-KD distillation; polynomial-basis KANs admit structural Lipschitz bounds from coefficient magnitudes, giving $P(\text{KAN}) \leq 1$ as a design property.\\
...
depth-independent form of Eq.~\ref{eq:convergent_bound}.
...
Beyond this empirical observation, contractivity admits a structural
explanation: for KAN layers, the per-layer Lipschitz product
$P^{(\ell)} = L_f^{(\ell)} \cdot \|W^{(\ell+1)}\|_{1,\infty}$ decomposes
into a B-spline derivative factor ($L_B \approx 0.65$ for cubic
B-splines, independent of $n$) and a weight row-sum factor that
shrinks under weight decay during distillation. This establishes that
the contractivity required by Condition~3 is not merely an artifact of
our specific trained model or distillation procedure---it is a
\emph{controllable} property of KAN training, tunable via regularization.
The SVNN framework thus captures a training-amenable architectural
invariant rather than an empirical coincidence. $\square$
