...
conjecturally the unique regime where second-order rate, linear LUT cost, closed-form
...
IEC~61508 independence requirements---the first such guarantee for
...
\begin{enumerate}
    ...
    \item \textbf{Theory (Deployment Safety):} The
         ...
         $\text{WCET}(P) + \text{WCET}(N) \leq$ scan cycle---the first such
         guarantee for neural-network-on-PLC deployment, directly addressing
...
\begin{enumerate}[label=\arabic*.]
   ...
   \leq$ scan cycle---the first such guarantee for NN-on-PLC deployment.
...
depth extension to $L{=}3$ ($6.1\times$); adaptive allocation (E15)
achieves $3.6\times$ at unchanged storage.

\vspace{4pt}
\noindent\textbf{Open directions.} Mechanized proof of the IR type soundness
theorem in Coq/Lean, physical PLC WCET measurement (all current timing
