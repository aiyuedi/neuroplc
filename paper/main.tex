...
correctness certification under fixed-point (LUT) compilation, and why.
...
certified safety state across model updates. Five $C^2$-BV architectures
(B-spline, Chebyshev, Fourier, Wavelet, RBF) and three datasets validate
the framework. The theory answers not ``can we compile neural networks to
...
design-time correctness certification under fixed-point (LUT) compilation,
...
architectures}---a formal characterization of which architectures admit
design-time correctness certification under fixed-point compilation, with
...
\vspace{4pt}
\noindent\textbf{Empirical validation (74 experiments, 5 architectures).}
