...
to test-set validation. Proposition~\ref{prop:mlp_negative} (below) establishes that standard MLPs
...
\begin{enumerate}
    ...
    \item \textbf{Level 3---Mechanized Formal Verification (Strongest):}
          ...
          has been achieved for select neural network kernels in
          machine-checked settings (e.g., CompCert-style verified compilation)
          but is currently intractable for full ML compiler pipelines.
\end{enumerate}
...
\begin{theorem}[SVNN Sufficient Conditions]
...
\begin{theorem}[SVNN Compositional Closure]
...
({\S}\ref{sec:svnn-necessity}) provides the formal answer in the
$\varepsilon$-dimension: the LUT-verification \emph{decision problem}
is polynomial for SVNN and NP-hard for coupled architectures (a
self-contained 3-SAT reduction via product gates), and SVNN uniquely
attains (polynomial time, zero slack) simultaneously. This establishes
that the SVNN conditions are not an arbitrary restriction but the
precise boundary at which polynomial-time, tight compiler-computed
certification becomes achievable. We briefly summarize the three
...
curvature computable only via numerical optimization (SiLU: $\sup_{\mathbb{R}}|\text{SiLU}''| = 0.5$)
...
SiLU's second derivative is finite ($\sup_{[-3,3]}|\text{SiLU}''| = 0.5$,
attained at $x=0$)---but the key
...
\vspace{4pt}
\noindent\textbf{Remark (Relationship to Theorem~1).}
...
\vspace{4pt}
\noindent\textbf{Remark (Arithmetic Bounds vs.\ MILP-Based Verification).}
...
depth-\textit{uniform} ($\gamma < 1$, depth-independent growth) from those whose bounds
...
\begin{tabular}{@{}p{1.7cm}p{0.95cm}p{0.95cm}p{1.15cm}p{1.15cm}p{1.2cm}@{}}
\toprule
\textbf{Architecture} & \textbf{Cond.~1} & \textbf{Cond.~2} & \textbf{Contract.} & \textbf{Frontier} & \textbf{Z3-Verif.} \\
\midrule
KAN (B-spline) & \cmark & \cmark & \xmark$^{*}$ & Interior$^{\dag}$ & 512/512 \\
...
$^{*}$Condition~3 is \emph{not} satisfied by the trained KAN student $[28,16,4]$ (measured per-layer amplification $\gamma = [15.4, 5.3] > 1$, E68); the student is certified under Conditions~1--2 with the finite depth-dependent bound. Polynomial-basis KANs admit structural Lipschitz bounds from coefficient magnitudes, giving $P(\text{KAN}) \leq 1$ as a design-time property (ChebyKAN, {\S}\ref{sec:svnn-chebykan}).\\
$^{\dag}$The B-spline KAN is the only architecture in this taxonomy for which all three conditions are simultaneously satisfied \textit{and} empirically validated on the TIA Portal V21 engineering toolchain (S7-1200 target).\\
...
$^{\P}$SiLU contains $\sigma(x)=1/(1+e^{-x})$; $e^{-x}$ is transcendental and undecidable in Z3's NRA theory. On compact domain $[-3,3]$, $\sup|\text{SiLU}''| = 0.5$ is finite but requires numerical optimization---not computable from parameters alone.\\
...
The trained KAN student, however, does \emph{not} satisfy the contractivity
refinement (Condition~3): its measured per-layer amplification is
$\gamma = [15.4, 5.3] > 1$ (E68). It is therefore certified under
Conditions~1--2 with the finite depth-dependent bound; the depth-uniform
(convergent) regime is available to contractively trained instances
(future work).
\end{proposition}

\vspace{4pt}
\noindent\textit{Proof.} We verify Conditions~1 and~2, which establish SVNN
membership, and then report the Condition~3 status:
...
path, $\text{SiLU}''(x) = \sigma(x)(1-\sigma(x))\bigl[2 + x(1-2\sigma(x))\bigr]$,
which is bounded on any compact domain.
...
The empirically measured values ($L_B = 2.21$ layer-0 / $2.05$ layer-1, $M_2 = 0.177$ from E11) are
...
alone. Condition~3, \emph{when} it holds, upgrades this guarantee to the
depth-uniform, depth-independent form of Eq.~\ref{eq:convergent_bound}.
For the trained KAN student $[28,16,4]$, the honest checkpoint measurements
(E68) are: $L_B = 2.21$ (layer-0) / $2.05$ (layer-1)---the maximum of
$|\phi'(x)|$ over all 512 B-spline activations on a 1,000-point grid---
effective weight row-sum norms $\|W_{\text{eff}}\|_{1,\infty} = 12.36$
(layer-0) / $6.52$ (layer-1), and per-layer Lipschitz bounds of $25.1$
(layer-0) / $11.9$ (layer-1). The measured per-activation-layer
amplification of the trained student is $\gamma = [15.4, 5.3]$ (E68)---the
trained network is \emph{not} contractive, so the depth-uniform
(convergent) form does \emph{not} apply to this checkpoint. It remains
structurally verifiable under Conditions~1--2 with the finite
depth-dependent bound of Eq.~\ref{eq:total_bound}, and contractive
training (e.g., spectral normalization of the weight row sums) is the
deployment recipe for the convergent form---future work. (The earlier
estimate $L_B \cdot \|W\|_{1,\infty} = 0.65 \times 0.28 = 0.182$
double-counted the linear layers and is not a valid Lipschitz bound.)

Beyond this empirical finding, contractivity remains a structurally
meaningful \emph{target}: for KAN layers, the per-layer Lipschitz product
$P^{(\ell)} = L_f^{(\ell)} \cdot \|W^{(\ell+1)}\|_{1,\infty}$ decomposes
into a B-spline derivative factor and a weight row-sum factor, and both
factors are regularization-controllable (weight decay, spectral
normalization, $\lambda$-regularization of the spline coefficients).
Condition~3 is thus a \emph{trainable} property of KAN training rather
than an automatic one---achieving it is the recipe for the depth-uniform
certificate, and its absence does not affect verifiability under
Conditions~1--2. $\square$
...
\begin{proposition}[Standard MLPs Do Not Admit the SVNN Tight-Bound Structure]
...
are exact in DA) and decreases quadratically with LUT resolution $h$.
...
% ============================================================================
% Theorem 5 (Computational Necessity) + Theorem 6 (Generalization Bound)
% ============================================================================
...
\begin{enumerate}
    \item \textbf{Choose verifiable architectures.} Rather than attempting
          ...
          fundamentally limited~\cite{szasz2025floating}---compiler
          designers should target
    ...
    \item \textbf{Bridge design-time and mechanized verification.}
          ...
          is, to our knowledge, uniquely enabled by the SVNN architecture. Standard MLPs do not
...
KANs to PLCs, but as, to our knowledge, the first instance of a broader compiler design
...
deployment certification. Doubleton Arithmetic (DA) provides a 2.1$\times$
...
yielding the observed $2.1\times$ end-to-end advantage (E9). This
explains \emph{why} a modest per-segment advantage ($2.4\times$) produces
no further compounding: the inter-layer maps amplify DA and IA errors by
the \emph{same} per-layer factor (measured amplification
$\gamma = [15.4, 5.3]$ for the trained student, E68), which cancels in
the end-to-end ratio, so the observed $2.1\times$ advantage tracks the
per-segment ratio rather than compounding with depth.
\end{proposition}
...
and IA---the source of the $2.1\times$ end-to-end advantage---is
...
The $2.1\times$ end-to-end tightening (E9) is close to the per-segment
$2.4\times$ advantage because the inter-layer maps amplify DA and IA
errors by the \emph{same} per-layer factor (measured amplification
$\gamma = [15.4, 5.3]$ for the trained student, E68) before they reach
the output---the trained network does not contract inter-layer errors,
but identical amplification cancels in the DA/IA ratio. DA and IA both
benefit from this propagation equally; the advantage of DA is limited
to the \emph{input} side (Layer~0) where fresh LUT errors enter the
pipeline. This bounded advantage is a feature, not a bug: DA's role is
to provide a tighter initial bound that widens the safety margin from
below $1\times$ (IA, E9; bound $1.38$ exceeds the half-margin) to
$\sim 2.0\times$ (DA, E9; bound $0.66$)---a $2.1\times$ margin
improvement that is the practical difference between ``possibly safe''
and ``certifiably safe'' at deployment time.

\vspace{4pt}
\noindent\textbf{Per-segment vs.\ cross-layer DA advantage.}
A symbolic analysis (MATLAB Symbolic Math Toolbox,
\texttt{code/theory/\allowbreak{}da\_exactness\_symbolic.m}) derives the exact DA
...
end-to-end $2.1\times$ advantage measured in E9. The reduction from
$2.8\times$ (per-segment ideal) to $2.1\times$ (end-to-end observed)
...
\approx 0.43$---the LUT spacing $6/14$, not the B-spline knot spacing
$6/8 = 0.75$---domain $\approx [-3, 3]$ on Layer~0), all 512 B-spline
activations satisfy $M_2 \cdot h_{\text{LUT}}^2 / 8 \leq 0.040$ (maximum),
with mean $0.017$ and 95th-percentile $C(3) = 0.030$ (measured from the
checkpoint). The de~Boor guarantee therefore certifies all 512
...
falls within the per-function verification threshold. For the standard
KAN architecture
$[28,16,4]$ on domain $[-3,3]$, all 512 measured bounds are
$\leq 0.040$ (E58), so every activation is certified for any threshold
above that maximum. For ChebyKAN (E54), the looser Markov analytical
$M_2$ bound pushes 16 of 512 activations above the verification
threshold, explaining the 496/512 ($96.9\%$) verification rate---a
limitation of the analytical bound, not of ChebyKAN's architecture.
...
% ===========================================================================
\subsection{DA Optimality: Sharp k-th Order LUT Bounds}
\label{sec:da-optimality}
% ===========================================================================

Proposition~\ref{prop:da-segment-exactness} established that DA achieves
segment-exactness on affine B-spline segments. We now establish the
\textit{sharp} LUT error bounds for arbitrary interpolation order $k$ and
prove that DA---the $k=2$ instance---is the \textit{tightest possible}
sound first-order abstract domain: any bound tighter than DA's would
violate soundness on some member of the $C^2$ class. The previous
restriction of the optimality claim to cubic polynomials (an artifact of
the original verification on random cubics) is superseded by an explicit
extremal family attaining the bound exactly at every order.

\input{section_lut_sharp}

\vspace{4pt}
\begin{corollary}[DA Coefficient Optimality as the $k=2$ Special Case]
\label{cor:da-optimality}
...
\begin{enumerate}[label=(\roman*)]
    ...
    \item \textbf{Interpretation.} This coefficient-level optimality is the
    leading-order ($k=2$) manifestation of the sharp-constant optimality
    of Theorem~\ref{thm:sharp-lut}: the quadratic extremal
    $\sigma^{*}(x) = (M_2/2)(x - x_0)(x - x_1)$ of
    Theorem~\ref{thm:sharp-lut} attains the de~Boor bound $M_2 h^2/8$
    exactly, and the \emph{valid} coefficient bound follows from the
    minimax argument of Theorem~\ref{thm:sharp-lut} part~(3). The original
    empirical confirmation (10,000 random cubics, 62.4\% unsound under
    10\% tightening) is superseded by the exact extremal attainment
    (E63), which holds at machine precision.
\end{enumerate}
\end{corollary}
...
$B \geq M_2/2 = |f''(0)|/2$ (the best affine approximation
$f(0) + kx$ of the quadratic achieves minimal worst-case deviation
$M_2 r^2/2$ at $k = 0$---the true range radius of
$\frac{M_2}{2}x^2$ on $[-r, r]$ is $M_2 r^2/2$, so any sound affine
certificate must cover it);
...
(alternatively, see the sharp-constant argument of
Theorem~\ref{thm:sharp-lut}, which attains $M_2 h^2/8$ exactly via the
quadratic extremal at the window midpoint).
...
(iii) The extremal attainment of Theorem~\ref{thm:sharp-lut}
(E63: soundness ratio $\leq 1$ over 10,000 random $C^k$ functions;
extremal ratio $= 1.000000$ exactly for $k = 2, \dots, 5$)
replaces the earlier Monte-Carlo confirmation, which sampled only
random cubics and could not approach the extremal. $\square$

\vspace{4pt}
\noindent\textbf{Consequences.}
Theorem~\ref{thm:sharp-lut} resolves the question raised by
...
The $2.1\times$ end-to-end tightening (E9) is thus not an empirical
...
% ===========================================================================
\input{section_trichotomy}

% ===========================================================================
\input{section_capacity}
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/final/fig01_c2bv_basis}
    ...
    (annotated per subplot). Despite diverse functional forms, all share the
    SVNN-critical property: each $\phi_{j,i}(x_i)$ is a \textbf{univariate}
    $C^2$ function on a compact domain, enabling uniform LUT compilation with
    de~Boor error bound $M_2 h^2 / 8$ (Theorem~\ref{thm:tightness}).}
    \label{fig:c2bv_basis_functions}
\end{figure*}

Figure~\ref{fig:c2bv_verification} reports the per-family verification
results.

\vspace{4pt}
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.85\textwidth]{figures/final/fig02_verification}
    ...
    margin (ratio of the per-function margin to $\max M_2 \cdot h^2/8$).
    WaveletKAN's wide margin reflects M2-regularized training ($\lambda=0.01$).
    FourierKAN's M$_2$ is bounded by $\omega^2 \sum k^2(|c_k|+|d_k|)$ (Proposition~\ref{prop:c2bv-family}(c)).
    Margins differ across architectures (ChebyKAN lowest under its
    conservative Markov $M_2$ bound, E54); per-architecture deployability
    is reported in Table~\ref{tab:cross_domain}.}
