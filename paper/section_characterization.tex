...
The per-layer gap is $R(d) = \sqrt{d} / \gamma \geq \sqrt{d}$ for any
contractive KAN ($\gamma < 1$). The trained student $[28,16,4]$,
however, does \emph{not} realize this gap: its measured
per-activation-layer amplification is $\gamma = [15.4, 5.3] > 1$ (E68),
exceeding the MLP's $\sqrt{d} = 5.29$ at $d = 28$---the depth-uniform
advantage is a property of \emph{contractively trained} KANs, not of
the current checkpoint (contractive training is identified as the
deployment recipe in {\S}\ref{sec:svnn-kan}). The MLP-side of the
comparison remains unconditional: any MLP violating Condition~1 carries
a deterministic per-layer factor $\sqrt{d}$ (Step~1), so its $L$-layer
error amplification is $\Theta(d^{L/2})$ regardless of training---for
the MLP analog ($d=28$, $L=3$) this is $>10{,}000\times$ worse than a
contractive KAN, making MLP certification practically infeasible.
...
Figure~\ref{fig:sharp_lower_bound} shows the sharp lower-bound
construction.
\hfill $\square$
...
\begin{figure*}[t]
    \centering
    \includegraphics[width=0.88\textwidth]{figures/final/fig04_sharp_bound}
    ...
    achieves $\|W\|_{1,\infty} = \sqrt{d}$ exactly (solid red); the dashed blue
    line is the illustrative contractive case $\gamma = 0.182$ used in earlier
    versions---stale, since the trained student's measured per-layer
    amplification is $\gamma = [15.4, 5.3] > 1$ (E68).
    Right: per-layer amplification ratio $R(d) = \sqrt{d} / \gamma$ for widths
    $d$ from 4 to 256 (computed for the same illustrative $\gamma$).
    % TODO: regenerate fig04_sharp_bound with honest values before
    % submission (the trained KAN is non-contractive, gamma = [15.4, 5.3]).
    The all-ones construction is \textbf{sharp}:
...
(Corollary~\ref{cor:da-optimality}) or IA.
