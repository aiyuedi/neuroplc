...
\markboth{IEEE TRANSACTIONS ON INDUSTRIAL INFORMATICS, VOL. XX, NO. XX, 2026}%
         {Liu: NeuroPLC --- A Type Theory of Certifiable Neural Architectures}
...
error bound than interval arithmetic (safety factor $8.5\times$,
up to $3.1\times$ vs.\ $\sqrt{d}$ random-weights baseline,
...
\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/fig_da_vs_ia}
    \caption{\textbf{DA vs.\ IA Bound Comparison with LUT Resolution.}
   Left: DA achieves consistently $2.2\times$ tighter error bounds than IA across
   all LUT resolutions ($N = 8$--$20$). DA bound decreases as $\propto 1/N^2$ per
   Theorem~\ref{thm:tightness}; IA bound follows the same asymptotic but with a
    $2.2\times$ constant-factor penalty from wrapping effects in dense Hessian
   propagation. At $N{=}15$ (S7-1200 default): DA bound $= 0.079$ vs.\ IA bound
    $= 0.172$. Right: bar comparison showing DA's consistent $2.2\times$ advantage.
   The gap is \textit{not} empirically contingent---it is a consequence of DA's
   Galois-optimality (Theorem~\ref{thm:galois}) and sign-structural tightening
    (Theorem~\ref{thm:da-optimality}).}
    \label{fig:da_vs_ia}
\end{figure}
...
\begin{figure}[t]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/fig_wcet_breakdown}
    \caption{\textbf{WCET Breakdown: KAN $[28,16,4]$ Compiled SCL on S7-1200 CPU~1211C.}
   Dominated by B-spline LUT evaluation ($73.4\%$ of total): 448 Layer-0 edges
    ($16.4$~ms) + 64 Layer-1 edges ($2.3$~ms). MatMul contributes $16.3\%$,
   Softmax $0.5\%$, and FB overhead $0.3\%$. Total analysis bound $= 22.67$~ms,
   representing $22.7\%$ of the 100~ms scan cycle ($4.4\times$ margin).
   The worst-case per-edge LUT time of $36.7$~$\mu$s includes binary search
    (4 REAL comparisons), linear interpolation (3 REAL SUB, 1 REAL DIV,
    1 REAL MUL, 1 REAL ADD), and 4 array accesses.}
...
\State Emit confidence check: max\_conf $< \theta_c \Rightarrow$
     low\_confidence
\For{$j = 1$ to $d_{\text{out}}$}
    \State Emit range check: output $< -10.0$ OR output $> 10.0
          $\Rightarrow$ output\_range\_violation
\EndFor
\State Emit safety aggregation: any violation $\Rightarrow$
     safe\_state\_request := TRUE
...
\textbf{$C^2$-BV: B-spline+Fourier+Wavelet+Cheby+RBF} \\
