...
linear combination.
% TODO: the IA/DA logit bounds (0.172/0.079) and safety factors (3.9x/8.5x)
% stated in this section were computed with the stale constants
% L_B = 0.65 and ||W||_{1,inf} = 0.32/0.28; they must be recomputed from
% the current checkpoint (L_B = 2.21/2.05, ||W_eff||_{1,inf} = 12.36/6.52)
% before submission. With measured $\|W^{(0)}\|_{1,\max} = 0.32$ and
...
% TODO: the IA/DA logit bounds (0.172/0.079) and safety factors (3.9x/8.5x)
% quoted in {\S}\ref{sec:da} and Theorem~1 were computed with the stale
% L_B = 0.65 and must be recomputed from the current checkpoint.
...
$0.172$. Lemma~\ref{lem:da_moment} guarantees that the DA/IA ratio
...
The combined effect ($3{\times}5{\times}2 \approx 30\times$,
...
\textbf{DA bound} (0.079): high-probability under random-sign model (Lemma~
\ref{lem:da_moment}), uses $|\sum W_{k,j}W_{j,i}|$.
\textbf{Seg-DA} (0.056): DA with per-segment $M_2^{(k)}$ values (E16), additional $1.4\times$ tightening.
\textbf{Compositional DA} (0.1196): network-level bound from the three-proof-tier certificate ({\S}\ref{sec:compositional}),
...
\midrule
High-probability & DA & 15 & $8.5\times$ & SIL~2 & Sign-balance (Lemma~
\ref{lem:da_moment}) \\
