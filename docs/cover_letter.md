...
**Honest limitations** (stated in the paper): the trichotomy's full necessity remains a conjecture (first lattice + CP-relative theorem proven; anchored in free-knot hardness); validation is PLCSIM/Z3-based without physical PLC measurement; B-spline KANs reach γ<1 at the output layer (post-hoc projection) but the input layer stays at γ₁=1.026 (essentially contractive; the B-spline basis-amplitude constant). Bounded-amplitude bases (Fourier/Wavelet) now achieve pervasive contractivity (γ<1 in every layer) at 99.96%/99.75% accuracy with sound certificates 9.6×/4.0× (2026-08-05 v4/v2).
