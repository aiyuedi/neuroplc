...
\begin{enumerate}[label=(\roman*)]
    ...
    $N^{*} \asymp n^{1/(2k)}$: deployed LUT memory should scale as the
    $2k$-th root of the sample count---a directly actionable compiler
    rule no existing PAC bound states.
...
\midrule
\multicolumn{4}{@{}l}{DA bound: $0.411 \to 0.165$ after FT ($4.1\times$, 2026-08-04) \quad Z3 verified: \textbf{512/512} (post-FT)} \\
\multicolumn{4}{@{}l}{SCL compilation: 2,188 lines, \textbf{0 errors 0 warnings} (S7-1200)} \\
\multicolumn{4}{@{}l}{SVNN Condition~3: \textbf{not satisfied by this checkpoint} (measured per-layer amplification $\gamma = [15.4, 5.3] > 1$, E68); Conditions~1--2 \textbf{preserved} (the certificate carried post-fine-tuning is the depth-dependent bound)} \\
\bottomrule
\end{tabular}
\vspace{2pt}
