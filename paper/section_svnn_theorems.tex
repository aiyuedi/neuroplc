...
with cell width $h$. \textbf{Assume the intermediate signals of every layer
remain within the LUT's validated domain}---guaranteed by the
Box-Continuation Lemma~\ref{lem:box_continuation}, which propagates
certified boxes in polynomial time (for compact-support B-spline KAN the
domain is all of $\mathbb{R}$, so the assumption is vacuous). Let $\ell$
be the loss, Lipschitz $L_\ell$, with outputs and targets bounded in
$[-C, C]$, inputs in a compact box $\Omega \subset
...
$0.14 \pm 0.09$ vs.\ predicted $0.25$; the 0-1 loss variant is
unobservable at near-ceiling accuracy), and the empirical gap decays as
$n^{-0.84}$ (faster than the worst-case $n^{-1/2}$, as expected on a
well-separated task). These findings sharpen---rather than weaken---the
theory: the $h^k$ bias term is exact where its domain assumption holds,
