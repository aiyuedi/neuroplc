...
\begin{enumerate}
    ...
    \item \textbf{System (Multi-Architecture Validation):}
         ...
         generalization (Proposition~\ref{prop:c2bv-family}).
          % TODO: the margin ratios quoted in earlier versions (2.9x, 5.6x)
          % compared M2*h^2/8 against the stale contractivity constant 0.182,
          % which is not a measured margin; re-measure the per-function
          % margin ratio before submission.
...
+ V1--V7) plus five capacity-theory verification experiments
(E-T1--E-T6, {\S}\ref{sec:capacity}) spanning compression,
...
linear combination.
% FIXME(2026-08-03): the IA/DA bounds and safety factors stated in this
% section were recomputed from the released checkpoint (L_B = 2.21/2.05,
% ||W_eff||_{1,inf} = 12.36/6.52): DA 0.66 (safety 1.0x), IA 1.38 (exceeds
% half-margin 0.675, not a certificate). The narrative below uses the
% recomputed values. With measured $\|W^{(0)}\|_{1,\max} = 0.32$ and
...
factor of $1.0\times$ at the $M_2^{\text{char}}$ calibration. This
...
\begin{figure*}[t]
    ...
    % FIXME: fig05_da_vs_ia must be regenerated with the recomputed values
    % (DA 0.66 / IA 1.38 at N=15) before submission.
    \label{fig:da_vs_ia}
\end{figure*}
...
achieves a substantially higher safety factor (Seg-DA, E16; the
pre-recomputation $11.9\times$ figure is withdrawn pending
recomputation) at $N=15$ LUT points, providing the strongest formal
guarantee for compiled KAN inference to date; plain DA alone certifies
this checkpoint with a
thin $1.0\times$ factor at the $M_2^{\text{char}}$ calibration.
% TODO: Seg-DA absolute values (0.056, 11.9x) pending recomputation
...
% FIXME(2026-08-03): the IA/DA logit bounds quoted in {\S}\ref{sec:da} and
% Theorem~1 were recomputed from the released checkpoint: DA 0.66, IA 1.38
% (safety factors 1.0x / <1; IA is not a certificate).
...
arithmetic bound approach without an implausible theory--data gap.
% TODO: the correlation-aware aggregate bound (6.4) was computed with the
% stale L_net = 19.6; recompute with the recomputed network constants
% (2026-08-03).
...
% TODO: recompute the Theorem-4 expected/concentration instantiation
% from the current checkpoint (gamma > 1 branch) before submission.
...
% TODO: recompute the Theorem-4 depth-feasibility instantiation for the
% current checkpoint (gamma > 1 branch) before submission.
...
\begin{figure}[t]
    ...
    % FIXME(2026-08-03): regenerate fig09_wcet_breakdown with the
    % recomputed values (total 3.86 ms; LUT 82.3%, MatMul 15.9%).
...
% TODO: re-derive C_FT for the trained checkpoint from the honest
% constants (L_B = 2.21/2.05, ||W_eff||_{1,inf} = 12.36/6.52), or measure
% the DA-bound sensitivity to weight perturbation directly; the earlier
% "50x below delta_max" claim compared against a stale-derived delta_max
% and is not reinstated until delta_max is re-derived.
...
E40 compositional$^{\dag}$ & 15 & DA network-level (Tier~3)         & 0.1196 & $5.6\times$ \\
...
$^{\dag}$\textbf{Seg-DA} (0.056) and \textbf{compositional DA} (0.1196):
values from the earlier weight set; recomputation against the released
checkpoint is pending (segmented $M_2^{(k)}$ machinery, E16/E40).
...
$M_2^{\text{analytic}} \leq 12.8$, Eq.~\ref{eq:bspline_curve_bounds}).
% TODO: the analytical ``>14x'' bound was derived from the stale
% constants and needs re-derivation from the recomputed bounds (2026-08-03). Extending to
...
certified blind spot, and contractive training (spectral normalization) is the
recipe that restores a comfortable margin.
...
\begin{table*}[tb]
\centering
\caption{Results Summary: Core Experiments (E1--E20, E52--E68) and Validation Experiments (V1--V7)}
\label{tab:summary}
\scriptsize
...
E56& Deep KAN ($L{=}3$)       & [28,16,8,4] 120ep & \textbf{99.60\%}, 608/608 Z3, DA 1.90, sub-exp. depth \\
...
% TODO: E53 thresholds computed from stale constants; pending recomputation (2026-08-03)
...
% TODO: per-component (14.0x) ratio pending recomputation (2026-08-03)
...
$\varepsilon_i$ values tightens the DA error bound further---the safety factor
improves from $1.0\times$ (uniform DA, $M_2^{\text{char}}$ calibration)
...
% TODO: Seg-DA absolute values (0.056, 11.9x) and the former 1.4x
...
% TODO: the previous ``14.0x per-component / 38x end-to-end'' gap was
% computed from the stale DA bound 0.079; per-component ratio pending
% recomputation (2026-08-03).
...
% TODO: Seg-DA combined factor (11.9x) pending recomputation (2026-08-03).
...
\begin{itemize}
    ...
    \item \textbf{DA error bound:} the 3-layer DA chain measures
          $1.90$ (E56 rerun, 2026-08-04)---below the worst-case
          amplification envelope $\gamma^{L-1} = 5.3$ that would apply
          under full per-layer amplification, consistent with
          \emph{sub-exponential} depth scaling, not the exponential
          blowup expected for non-SVNN architectures (where $O(2^L)$
          growth arises from wrapping effects). (Absolute comparison
          with the 2-layer value across calibrations is pending
          unification; the sub-exponential claim uses the worst-case
          envelope as the reference.)
% TODO: 0.064 is a stale-weights DA value; recompute the 3-layer DA chain on
% the released checkpoint (the 3-layer model is e56_3layer_kan_deep_verify.py;
% recomputed DA at N=15 on the 2-layer model is 0.66, so 0.98/0.064 = 15.3x
% must be re-based once the 3-layer chain is recomputed).
...
\end{itemize}
The empirical depth-scaling ratio ($15.3\times$ for the single $2\to 3$
depth step) is substantially below the $O(2^L)$
% TODO: the earlier ``7.3x per added layer'' figure (14.6/2) was unexplained;
% only one layer was added (2 -> 3), so only the total step ratio (15.3x on the
% 0.064 base) applies. See the staleness TODO above.
...
substantially tighter than CROWN-IBP} (the CROWN-IBP comparison value
from the released-checkpoint rerun is pending; the earlier $8.3\times$
figure used a pre-recomputation baseline and is withdrawn).
% TODO: DA = 0.66 is the recomputed value at N=15 (2026-08-03); the earlier
% 0.064 (safety factor 10.5x = 1.35/(2*0.064)) was a stale-weights value. The
% CROWN-IBP bound (5.45) was computed in the same stale run---recompute the
% CROWN bound on the released checkpoint before submission.
...
characterization). The $8.3\times$ gap between DA and CROWN-IBP quantifies the
...
% TODO: compositional (Tier-3) bounds 0.2473/0.1196 are from the earlier
% weight set; recomputation against the released checkpoint pending
% (2026-08-03, cf. Table tab:da_bounds_summary dagger note).
\midrule
...
% TODO: the former per-component (14.0x) and E2E (38x) ratios were computed
% from the stale DA bound 0.079; E2E recomputed as 3.04/0.66 = 4.6x,
% per-component ratio pending (2026-08-03). Notably,
...
% TODO: per-component (14.0x) ratio basis unclear; pending recomputation (2026-08-03).
...
quantitative (MLP propagation is $4.6\times$ looser via DA end-to-end; $2.2\times$ looser via IA)% TODO: per-component ratio pending recomputation (2026-08-03)
...
% TODO: tab:cert_thresholds rows 2--4 (Seg-IA 2.9x, IA 1.4x, IA 2.3x) were
% computed from the stale constants and are not consistent with the
% recomputed IA bound (1.38 at N=15, not a certificate); pending
% recomputation (2026-08-03).
...
\vspace{4pt}
\noindent\textbf{Empirical validation (74 experiments + 6 capacity
experiments E-T1--E-T6, 4 architectures).}
