function neuroplc_final_figures_complete()
%% NeuroPLC Final Figure Suite
...
disp("Running polished top-journal figure suite...");

function S = nstyle()
S.font = 'Arial'; S.W1 = 3.45; S.W2 = 7.16; S.H = 2.62; S.Ht = 3.65;
S.fsP = 11; S.fsA = 10; S.fsT = 9; S.fsL = 9; S.fsD = 8;
S.lw = 2.0; S.glw = 0.5; S.ms = 6;
S.da   = [0.122 0.467 0.706]; S.ia   = [1.000 0.498 0.055];
S.green= [0.173 0.627 0.173]; S.red  = [0.839 0.153 0.157];
S.purple=[0.580 0.404 0.741]; S.sky  = [0.337 0.706 0.914];
S.yellow=[0.890 0.780 0.220]; S.gray = [0.45 0.45 0.45]; S.lgray = [0.86 0.86 0.86];
S.arch = [S.sky; S.green; S.purple; S.yellow; S.red];
set(groot,'DefaultFigureColor','w','DefaultAxesFontName',S.font,'DefaultTextFontName',S.font,'DefaultAxesFontSize',S.fsT,'DefaultAxesLineWidth',0.8,'DefaultLineLineWidth',S.lw);
end

function prep(ax,S)
set(ax,'Box','off','TickDir','out','FontSize',S.fsT,'XGrid','on','YGrid','on','GridLineStyle',':','GridColor',[0.72 0.72 0.72],'GridAlpha',0.35,'LineWidth',0.8);
end

function pnl(ax,label,title,S)
text(ax,0.018,0.975,['(' label ') ' title],'Units','normalized','FontName',S.font,'FontSize',S.fsP,'FontWeight','bold','VerticalAlignment','top','HorizontalAlignment','left');
end

function dlabel(ax,x,y,s,S,varargin)
text(ax,x,y,s,'FontName',S.font,'FontSize',S.fsD,'HorizontalAlignment','center','VerticalAlignment','bottom','BackgroundColor',[1 1 1],'Margin',1.5,varargin{:});
end
...
try print(fig,fullfile(outDir,[name '.eps']),'-depsc','-painters','-r300'); catch, end
end

function edup(fig,name,a,b); for d={a,b}, export_all(fig,d{1},name); end; close(fig); end
...
fig=figure('Units','inches','Position',[1 1 S.W2+0.1 S.Ht+0.1]); tl=tiledlayout(2,3,'Padding','tight','TileSpacing','compact');
g_sparse=linspace(-3,3,9)';
for i=1:6
    ax=nexttile; hold on;
    if i<=5
        p_vals={p1,p2,p3,p4,p5}; p=p_vals{i};
        fill([x;flipud(x)],[zeros(size(x));flipud(p)],S.arch(i,:),'FaceAlpha',0.15,'EdgeColor','none');
        plot(x,p,'-','Color',S.arch(i,:),'LineWidth',S.lw);
        yg=interp1(x,p,g_sparse);
        for q=1:numel(g_sparse), plot([g_sparse(q) g_sparse(q)],[0 yg(q)],':','Color',[.60 .60 .60],'LineWidth',0.5); end
        scatter(g_sparse,yg,15,[.30 .30 .30],'filled','MarkerFaceAlpha',.50);
        pnl(ax,char('a'+i-1),sprintf('%s  M_2=%.2f',N{i},M2(i)),S);
    else
        for j=1:5, pv={p1,p2,p3,p4,p5}; plot(x,pv{j},'LineWidth',1.1,'Color',S.arch(j,:)); end
        pnl(ax,'f','All C2-BV overlay',S);
        lg=legend(N,'NumColumns',3,'FontSize',8,'Box','off','Location','south');
    end
    ylim([-yr yr]); xlim([-3 3]); prep(ax,S);
    if mod(i-1,3)==0, ylabel('\phi(x) (activation)','FontSize',S.fsA); end
    if i>=4, xlabel('Input x (normalized units)','FontSize',S.fsA); end
end
...
xlabel('Hidden width d','FontSize',S.fsA); ylabel('Per-layer amplification (log)','FontSize',S.fsA);
pnl(ax,'a','MLP vs KAN amplification',S); text(ax,0.02,0.05,'(log_{10})','Units','normalized','FontSize',7,'Color',S.gray,'FontAngle','italic');
...
xlabel('Hidden width d','FontSize',S.fsA); ylabel('MLP/KAN gap (log)','FontSize',S.fsA);
...
fig=figure('Units','inches','Position',[1 1 S.W2 S.H]); tl=tiledlayout(1,2,'Padding','compact');
ax=nexttile; scatter(allx,ally,S.ms,S.da,'filled','MarkerFaceAlpha',0.40,'MarkerEdgeColor','none'); hold on;
errorbar(x,mu,sd,'o-','Color',S.ia,'LineWidth',S.lw,'MarkerFaceColor',S.ia,'MarkerSize',8);
...
set(ax,'XTickLabel',string(d)); xlabel('Hidden width d','FontSize',S.fsA); ylabel('Ratio vs sqrt(d)','FontSize',S.fsA);
...
ax=nexttile; b=bar([E;G]','grouped'); b(1).FaceColor=S.da; b(2).FaceColor=S.ia; b(1).EdgeColor='none'; b(2).EdgeColor='none'; prep(ax,S);
set(ax,'XTickLabel',string(N)); xlabel('LUT points N','FontSize',S.fsA); ylabel('Error bound (absolute)','FontSize',S.fsA);
pnl(ax,'b','Bound values per N',S); legend('Segment','Global','Box','off','FontSize',S.fsL);
ax=nexttile; plot(N,T,'ko-','LineWidth',1.8,'MarkerSize',8,'MarkerFaceColor',S.da); hold on;
plot(N,C1,'s--','Color',S.green,'LineWidth',1.5,'MarkerSize',7);
plot(N,C2,'^:','Color',S.purple,'LineWidth',1.5,'MarkerSize',7);
ylabel('Tightening factor / Coverage (%)','FontSize',S.fsA);
prep(ax,S); set(ax,'XTickLabel',string(N)); xlabel('LUT points N','FontSize',S.fsA);
pnl(ax,'c','Tightening + segment coverage',S);
legend({'Tightening (x)','<0.5x global','<0.2x global'},'Box','off','Location','southeast','FontSize',S.fsL);
...
rng(9); n=120; mu=[-3 -1.4;2 -2;-2.1 2.4;1.6 .6]; sg=[.55 .38;.48 .65;.38 .48;.65 .55];
...
for p=1:2
    ax=nexttile; hold on;
    for c=1:4, idx=L==c; scatter(X(idx,1),X(idx,2),S.ms+2,C(c,:),'filled','MarkerFaceAlpha',0.38,'MarkerEdgeColor','none'); end
    xlim(xl); ylim(yl); xlabel('t-SNE dimension 1','FontSize',S.fsA); ylabel('t-SNE dimension 2','FontSize',S.fsA);
...
arch={'B-spline','Fourier','Wavelet','Cheby','MLP'};
z=[100 100 100 96.9 0]; zsd=[0 0 0 1.2 0]; acc=[99.93 100 100 99.87 24.13];
margin=[4.5 2.9 5.6 1.1 0]; asd=[.05 .00 .00 .08 .25]; msd=[0.3 0.2 0.4 0.1 0];
fig=figure('Units','inches','Position',[1 1 S.W2+0.1 S.H+0.1]); tl=tiledlayout(1,3,'Padding','compact','TileSpacing','compact');
vals={z,acc,margin}; sds={zsd,asd,msd};
ylbl={'Z3-verifiable (%)','Test accuracy (%)','Safety margin (x)'};
tls={'Z3 verifiability rate','CWRU bearing accuracy','Deployment margin'};
for p=1:3
    ax=nexttile; b=bar(vals{p},'FaceColor','flat','EdgeColor','none','BarWidth',0.55); b.CData=S.arch; hold on;
    for i=1:5, if sds{p}(i)>0, errorbar(i,vals{p}(i),sds{p}(i),'k.','LineWidth',1.2,'CapSize',8); end, end
    if p==3, yline(2,'--','deploy threshold','Color',S.gray,'LineWidth',1.2,'FontSize',7); end
    yh=max(112,max(vals{p})*1.22); ylim([0 yh]); set(ax,'XTick',1:5,'XTickLabel',arch,'XTickLabelRotation',30,'FontSize',S.fsT);
    ylabel(ylbl{p},'FontSize',S.fsA); pnl(ax,char('a'+p-1),tls{p},S); prep(ax,S);
    for i=1:5, yv=vals{p}(i); if yv==0, dlabel(ax,i,yh*0.04,'0',S,'Color',S.red); else, dlabel(ax,i,yv+yh*0.03,sprintf('%.1f',yv),S); end, end
end
...
ax=nexttile; b=bar([DA;IA]','grouped','BarWidth',0.55); b(1).FaceColor=S.da; b(2).FaceColor=S.ia; b(1).EdgeColor='none'; b(2).EdgeColor='none';
...
for i=1:numel(asd), if asd(i)>0, errorbar(i,acc(i),asd(i),'k.','LineWidth',1.3,'CapSize',10); end, end
set(ax,'XTickLabel',mdl,'XTickLabelRotation',35,'YLim',[0 108]); prep(ax,S);
...
%% MATLAB-native
fig01_c2bv_basis(S,FM,FF); fig03_da_tightness(S,FM,FF);
fig04_sharp_bound(S,FM,FF); fig07_da_scaling(S,FM,FF);
fig08_segment_bounds(S,FM,FF); fig10_confusion(S,FM,FF);
fig11_tsne(S,FM,FF); fig12_crossval(S,FM,FF); fig16_code(S,FD,FF);

%% Origin-suited
fig02_verification(S,FO,FF); fig05_da_vs_ia(S,FO,FF);
fig06_adaptive(S,FO,FF); fig09_wcet(S,FO,FF);
fig13_models(S,FO,FF); fig14_crossdomain(S,FO,FF);
fig15_monitor(S,FO,FF);

%% Diagram-native
fig1_pipeline(S,FD,FF); fig2_compiler(S,FD,FF);

fprintf('Complete. Final: %s\n',FF);
end